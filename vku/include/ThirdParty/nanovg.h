...
// nvgEndFrame() nvgBeginFrame() defines the size of the window to render to in
// relation currently set viewport (i.e. glViewport on GL backends). Device
// pixel ration allows to control the rendering on Hi-DPI devices. For example,
// GLFW returns two dimension for an opened window: window size and frame buffer
...
// Returns a color value from red, green, blue values. Alpha will be set
// to 1.0f.
...
// Sets whether to draw antialias for nvgStroke() and nvgFill(). It's enabled by
// default.
...
// defined paths. Then you define one or more paths and sub-paths which describe
// the shape. The are functions to draw common shapes like rectangles and
// circles, and lower level step-by-step functions, which allow to define a path
// curve by curve.
//
...
int nvgCreateFontAtIndex(NVGcontext *ctx, const char *name,
                         const char *filename, const int fontIndex);
