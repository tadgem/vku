...
#pragma warning(                                                               \
    disable                                                                    \
    : 4251) // class 'xxx' needs to have dll-interface to be used by clients of
            // struct 'xxx' // when IMGUI_API is set to__declspec(dllexport)
#pragma warning(disable                                                        \
                : 26812) // The enum type 'xxx' is unscoped. Prefer 'enum class'
...
// imgui_internal.h in imgui.h As they are frequently requested, we do not want
// to encourage to many people using imgui_internal.h
#if defined(IMGUI_DEFINE_MATH_OPERATORS) &&                                    \
    !defined(IMGUI_DEFINE_MATH_OPERATORS_IMPLEMENTED)
...
// You can compile with both by defining both IMGUI_ENABLE_FREETYPE and
// IMGUI_ENABLE_STB_TRUETYPE together.
...
struct ImGuiDockNode;     // Docking system node (hold a list of Windows OR two
                          // child dock nodes)
struct ImGuiDockNodeSettings; // Storage for a dock node in .ini file (we
...
struct ImGuiInputTextState; // Internal state of the currently focused/edited
                            // text input box
struct ImGuiInputTextDeactivateData; // Short term storage to backup text of a
...
struct ImGuiMenuColumns; // Simple column measurement, currently used for
                         // MenuItem() only
struct ImGuiNavItemData; // Result of a gamepad/keyboard directional navigation
                         // move query result
struct ImGuiNavTreeNodeData; // Temporary storage for last TreeNode() being a
...
struct ImGuiTableTempData; // Temporary storage for one table (one per table in
                           // the stack), shared between tables.
struct ImGuiTableSettings; // Storage for a table .ini settings
struct ImGuiTableColumnsSettings; // Storage for a column .ini settings
struct ImGuiTypingSelectState;    // Storage for GetTypingSelectRequest()
struct ImGuiTypingSelectRequest;  // Storage for GetTypingSelectRequest() (aimed
                                  // to be public)
struct ImGuiWindow;               // Storage for one window
struct ImGuiWindowDockStyle; // Storage for window-style data which needs to be
                             // stored for docking purpose
...
enum ImGuiLocKey : int; // -> enum ImGuiLocKey              // Enum: a
                        // localization entry for translation.
typedef int
    ImGuiDataAuthority; // -> enum ImGuiDataAuthority_      // Enum: for storing
                        // the source authority (dock node vs window) of a field
typedef int ImGuiLayoutType; // -> enum ImGuiLayoutType_         // Enum:
                             // Horizontal or vertical

// Flags
typedef int ImGuiActivateFlags; // -> enum ImGuiActivateFlags_      // Flags:
                                // for navigation/focus function (will be for
                                // ActivateItem() later)
typedef int ImGuiDebugLogFlags; // -> enum ImGuiDebugLogFlags_      // Flags:
                                // for ShowDebugLogWindow(), g.DebugLogFlags
typedef int ImGuiFocusRequestFlags; // -> enum ImGuiFocusRequestFlags_  //
...
typedef int ImGuiScrollFlags; // -> enum ImGuiScrollFlags_        // Flags: for
...
typedef int ImGuiTooltipFlags; // -> enum ImGuiTooltipFlags_       // Flags: for
                               // BeginTooltipEx()
typedef int ImGuiTypingSelectFlags;  // -> enum ImGuiTypingSelectFlags_  //
                                     // Flags: for GetTypingSelectRequest()
typedef int ImGuiWindowRefreshFlags; // -> enum ImGuiWindowRefreshFlags_ //
                                     // Flags: for SetNextWindowRefreshPolicy()
...
// (since IMGUI_VERSION_NUM >= 18729: IMGUI_DEBUG_LOG was reworked into
// IMGUI_DEBUG_PRINTF (and removed framecount from it). If you were using a
// #define IMGUI_DEBUG_LOG please rename)
...
#define IMGUI_DEBUG_LOG_DOCKING(...)                                           \
  do {                                                                         \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventDocking)                     \
      IMGUI_DEBUG_LOG(__VA_ARGS__);                                            \
  } while (0)
#define IMGUI_DEBUG_LOG_VIEWPORT(...)                                          \
  do {                                                                         \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventViewport)                    \
      IMGUI_DEBUG_LOG(__VA_ARGS__);                                            \
  } while (0)
...
#define IM_NEWLINE "\n"
#endif
#ifndef IM_TABSIZE // Until we move this to runtime and/or add proper tab
                   // support, at least allow users to compile-time override
#define IM_TABSIZE (4)
#endif
#define IM_MEMALIGN(_OFF, _ALIGN)                                              \
  (((_OFF) + ((_ALIGN) - 1)) &                                                 \
   ~((_ALIGN) - 1)) // Memory align e.g. IM_ALIGN(0,4)=0, IM_ALIGN(1,4)=4,
...
// specific item. This will call IM_DEBUG_BREAK() which you may redefine
...
#define IM_DEBUG_BREAK()                                                       \
  IM_ASSERT(0) // It is expected that you define IM_DEBUG_BREAK() into something
...
static inline void ImQsort(void *base, size_t count, size_t size_of_element,
                           int(IMGUI_CDECL *compare_func)(void const *,
                                                          void const *)) {
  if (count > 1)
...
IMGUI_API int
ImStrnicmp(const char *str1, const char *str2,
...
ImStrchrRange(const char *str_begin, const char *str_end,
...
IMGUI_API const char *
ImStristr(const char *haystack, const char *haystack_end, const char *needle,
          const char *needle_end); // Find a substring in a string range.
IMGUI_API void
ImStrTrimBlanks(char *str); // Remove leading and trailing blanks from a buffer.
IMGUI_API const char *
ImStrSkipBlank(const char *str); // Find first non-blank character.
IMGUI_API int
ImStrlenW(const ImWchar *str); // Computer string length (ImWchar string)
...
IMGUI_API void ImFormatStringToTempBuffer(const char **out_buf,
...
IMGUI_API void ImParseFormatSanitizeForPrinting(const char *fmt_in,
                                                char *fmt_out,
                                                size_t fmt_out_size);
IMGUI_API const char *ImParseFormatSanitizeForScanning(const char *fmt_in,
                                                       char *fmt_out,
                                                       size_t fmt_out_size);
...
IMGUI_API int
ImTextStrToUtf8(char *out_buf, int out_buf_size, const ImWchar *in_text,
...
IMGUI_API int ImTextStrFromUtf8(
    ImWchar *out_buf, int out_buf_size, const char *in_text,
...
IMGUI_API int ImTextCountLines(
    const char *in_text,
    const char *in_text_end); // return number of lines taken by text. trailing
                              // carriage return doesn't count as an extra line.
...
// Helpers: Geometry
IMGUI_API ImVec2 ImBezierCubicCalc(const ImVec2 &p1, const ImVec2 &p2,
...
IMGUI_API ImVec2 ImBezierQuadraticCalc(const ImVec2 &p1, const ImVec2 &p2,
...
inline bool ImTriangleIsClockwise(const ImVec2 &a, const ImVec2 &b,
                                  const ImVec2 &c) {
...
#define IM_BITARRAY_TESTBIT(_ARRAY, _N)                                        \
  ((_ARRAY[(_N) >> 5] & ((ImU32)1 << ((_N) & 31))) !=                          \
   0) // Macro version of ImBitArrayTestBit(): ensure args have side-effect or
      // are costly!
...
inline void ImBitArraySetBitRange(ImU32 *arr, int n,
                                  int n2) // Works on range [n..n2)
{
  ...
  while (n <= n2) {
    ...
    ImU32 mask =
        (ImU32)(((ImU64)1 << b_mod) - 1) & ~(ImU32)(((ImU64)1 << a_mod) - 1);
    arr[n >> 5] |= mask;
    n = (n + 32) & ~31;
  }
}
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD, _MAXERROR)                  \
  ImClamp(IM_ROUNDUP_TO_EVEN((int)ImCeil(                                      \
              IM_PI / ImAcos(1 - ImMin((_MAXERROR), (_RAD)) / (_RAD)))),       \
          IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MIN,                                 \
          IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_MAX)

// Raw equation from IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC rewritten for 'r' and
// 'error'.
...
#define IM_DRAWLIST_ARCFAST_SAMPLE_MAX                                         \
  IM_DRAWLIST_ARCFAST_TABLE_SIZE // Sample index _PathArcToFastEx() for 360
                                 // angle.
...
struct IMGUI_API ImDrawListSharedData {
  ...
  ImDrawListFlags InitialFlags; // Initial flags at the beginning of the frame
  ...
  ImVec2 ArcFastVtx[IM_DRAWLIST_ARCFAST_TABLE_SIZE]; // Sample points on the
                                                     // quarter of the circle.
  float ArcFastRadiusCutoff;    // Cutoff radius after which arc drawing will
                                // fallback to slower PathArcTo()
  ImU8 CircleSegmentCounts[64]; // Precomputed segment count for given radius
...
struct ImDrawDataBuilder {
  ImVector<ImDrawList *>
      *Layers[2]; // Pointers to global layers for: regular, tooltip. LayersP[0]
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_HasDisplayRect =
      1 << 1, // g.LastItemData.DisplayRect is valid
  ...
  ImGuiItemStatusFlags_HoveredWindow =
      1 << 7, // Override the HoveredWindow test to allow cross-window hover
              // testing.
...
// Additional status + semantic for ImGuiTestEngine
  ...
  ImGuiItemStatusFlags_Openable = 1
                                  << 20, // Item is an openable (e.g. TreeNode)
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  // -> use BeginDisabled() or ImGuiItemFlags_Disabled
  ...
  ImGuiButtonFlags_NoNavFocus =
      1 << 18, // don't override navigation focus when activated (FIXME: this is
               // essentially used everytime an item uses ImGuiItemFlags_NoNav,
...
enum ImGuiSliderFlagsPrivate_ {
  ...
  ImGuiSliderFlags_ReadOnly = 1
                              << 21, // Consider using g.NextItemData.ItemFlags
                                     // |= ImGuiItemFlags_ReadOnly instead.
};
...
enum ImGuiTreeNodeFlagsPrivate_ {
  ...
  ImGuiTreeNodeFlags_UpsideDownArrow =
      1 << 21, // (FIXME-WIP) Turn Down arrow into an Up arrow, but reversed
               // trees (#6517)
};
...
// Flags for FocusWindow(). This is not called ImGuiFocusFlags to avoid
// confusion with public-facing ImGuiFocusedFlags.
...
struct IMGUI_API ImGuiMenuColumns {
  ...
  ImU16 OffsetMark;
  ImU16 Widths[4]; // Width of:   Icon, Label, Shortcut, Mark  (accumulators for
                   // current frame)
...
struct IMGUI_API ImGuiInputTextState {
  ...
  ImVector<ImWchar> TextW; // edit buffer, we need to persist but can't
  ...
  ImVector<char> InitialTextA; // value to revert to when pressing Escape =
                               // backup of end-user buffer at the time of focus
                               // (in UTF-8, unaltered)
  ...
  ImGuiInputTextFlags Flags; // copy of InputText() flags. may be used to check
                             // if e.g. ImGuiInputTextFlags_Password is set.
  bool ReloadUserBuf;        // force a reload of user buf so it may be modified
                             // externally. may be automatic in future version.
  int ReloadSelectionStart;  // POSITIONS ARE IN IMWCHAR units *NOT* UTF-8 this
                             // is why this is not exposed yet.
  ...
  // If you modify underlying user-passed const char* while active you need to
  // call this (InputText V2 may lift this)
  //   strcpy(my_buf, "hello");
  //   if (ImGuiInputTextState* state = ImGui::GetInputTextState(id)) // id may
  //   be ImGui::GetItemID() is last item
...
struct ImGuiNextWindowData {
  ...
  ImVec2 MenuBarOffsetMinVal; // (Always on) This is not exposed publicly, so we
...
struct ImGuiNextItemData {
  ...
  ImGuiItemFlags
      ItemFlags; // Currently only tested/used for ImGuiItemFlags_AllowOverlap.
  // Non-flags members are NOT cleared by ItemAdd() meaning they are still valid
  // during NavProcessItem()
  ImGuiSelectionUserData
      SelectionUserData;  // Set by SetNextItemSelectionUserData() (note that
                          // NULL/0 is a valid value, we use -1 ==
                          // ImGuiSelectionUserData_Invalid to mark invalid
                          // values)
...
struct ImGuiLastItemData {
  ...
  // corresponding ImGuiItemStatusFlags is set.
  ImRect DisplayRect; // Display rectangle (ONLY VALID IF
                      // ImGuiItemStatusFlags_HasDisplayRect is set)
  ImRect ClipRect; // Clip rectangle at the time of submitting item (ONLY VALID
                   // IF ImGuiItemStatusFlags_HasClipRect is set)

  ImGuiLastItemData() { memset(this, 0, sizeof(*this)); }
};

// Store data emitted by TreeNode() for usage by TreePop() to implement
// ImGuiTreeNodeFlags_NavLeftJumpsBackHere. This is the minimum amount of data
...
struct ImGuiPopupData {
  ...
  ImGuiWindow *Window; // Resolved on BeginPopup() - may stay unresolved if user
                       // never calls OpenPopup()
  ImGuiWindow *RestoreNavWindow; // Set on OpenPopup(), a NavWindow that will be
  ...
  ImVec2 OpenMousePos; // Set on OpenPopup(), copy of mouse position at the time
                       // of opening popup
...
typedef ImBitArray<ImGuiKey_NamedKey_COUNT, -ImGuiKey_NamedKey_BEGIN>
...
enum ImGuiInputSource {
  ...
  ImGuiInputSource_Mouse, // Note: may be Mouse or TouchScreen or Pen. See
...
// appears to be an extension? Using ImVec2() would fail on Clang 'union member
...
struct ImGuiInputEvent {
  ...
  union {
    ...
    ImGuiInputEventMouseWheel
        MouseWheel; // if Type == ImGuiInputEventType_MouseWheel
    ImGuiInputEventMouseButton
        MouseButton; // if Type == ImGuiInputEventType_MouseButton
...
// Input function taking an 'ImGuiID owner_id' argument defaults to
// (ImGuiKeyOwner_Any == 0) aka don't test ownership, which matches legacy
// behavior.
#define ImGuiKeyOwner_Any                                                      \
  ((ImGuiID)0) // Accept key that have an owner, UNLESS a call to SetKeyOwner()
               // explicitly used ImGuiInputFlags_LockThisFrame or
...
struct ImGuiKeyRoutingData {
  ...
  ImU16 Mods; // Technically we'd only need 4-bits but for simplify we store
              // ImGuiMod_ values which need 16-bits. ImGuiMod_Shortcut is
...
struct ImGuiKeyRoutingTable {
  ImGuiKeyRoutingIndex
      Index[ImGuiKey_NamedKey_COUNT]; // Index of first entry in Entries[]
  ImVector<ImGuiKeyRoutingData> Entries;
  ImVector<ImGuiKeyRoutingData>
      EntriesNext; // Double-buffer to avoid reallocation (could use a shared
                   // buffer)
...
// SetKeyOwner(), SetItemKeyOwner() Don't mistake with ImGuiInputTextFlags!
// (which is for ImGui::InputText() function)
enum ImGuiInputFlags_ {
  ...
  ImGuiInputFlags_Repeat =
      1 << 0, // Enable repeat. Return true on successive repeats. Default for
              // legacy IsKeyPressed(). NOT Default for legacy IsMouseClicked().
              // MUST BE == 1.
  ...
  // In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable
  // default, but it would break too many behavior so everything is opt-in.
  ...
  ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone =
      1
      << 6, // Stop repeating when released OR if keyboard mods are leaving the
  ...
  ImGuiInputFlags_CondHovered =
      1 << 8, // Only set if item is hovered (default to both)
  ImGuiInputFlags_CondActive =
      1 << 9, // Only set if item is active (default to both)
  ImGuiInputFlags_CondDefault_ =
      ImGuiInputFlags_CondHovered | ImGuiInputFlags_CondActive,

  // Flags for SetKeyOwner(), SetItemKeyOwner()
  // Locking is useful to make input-owner-aware code steal keys from
  // non-input-owner-aware code. If all code is input-owner-aware locking would
  // never be necessary.
  ImGuiInputFlags_LockThisFrame =
      1 << 10, // Further accesses to key data will require EXPLICIT owner ID
               // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared
               // at end of frame.
  ImGuiInputFlags_LockUntilRelease =
      1 << 11, // Further accesses to key data will require EXPLICIT owner ID
               // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared
  ...
  //      Parent   -> call Shortcut(Ctrl+S)    // When Parent is focused, Parent
  //      gets the shortcut.
  //        Child1 -> call Shortcut(Ctrl+S)    // When Child1 is focused, Child1
  //        gets the shortcut (Child1 overrides Parent shortcuts) Child2 -> no
  //        call                  // When Child2 is focused, Parent gets the
  //        shortcut.
  //   The whole system is order independent, so if Child1 does it calls before
  //   Parent results will be identical. This is an important property as it
  //   facilitate working with foreign code or larger codebase.
  // - Visualize registered routes in 'Metrics->Inputs' and submitted routes in
  // 'Debug Log->InputRouting'.
  // - When a policy (except for _RouteAlways *) is set, Shortcut() will
  // register itself with SetShortcutRouting(),
  //   allowing the system to decide where to route the input among other
  //   route-aware calls.
  //   (* Using ImGuiInputFlags_RouteAlways is roughly equivalent to calling
  //   IsKeyChordPressed(key)).
  // - Shortcut() uses ImGuiInputFlags_RouteFocused by default. Meaning that a
  // Shortcut() call will register
  //   a route and only succeed when parent window is in the focus-stack and if
  //   no-one with a higher priority is claiming the same shortcut.
  ...
  ImGuiInputFlags_RouteFocused =
      1 << 12, // (Default) Honor focus route: Accept inputs if window is in
  ...
  ImGuiInputFlags_RouteGlobalLow =
      1 << 13, // Register route globally (lowest priority: unless a focused
               // window or active item registered the route) -> recommended
               // Global priority IF you need a Global priority.
  ImGuiInputFlags_RouteGlobal =
      1 << 14, // Register route globally (medium priority: unless an active
               // item registered the route, e.g. CTRL+A registered by InputText
               // will take priority over this).
  ImGuiInputFlags_RouteGlobalHigh =
      1 << 15, // Register route globally (higher priority: unlikely you need to
               // use that: will interfere with every active items, e.g. CTRL+A
               // registered by InputText will be overriden by this)
  ImGuiInputFlags_RouteAlways =
      1 << 16, // Do not register route, poll keys directly.
               // Routing polices: extra options
  ImGuiInputFlags_RouteUnlessBgFocused =
      1 << 17, // Global routes will not be applied if underlying
...
struct ImGuiListClipperRange {
  ...
  static ImGuiListClipperRange FromPositions(float y1, float y2, int off_min,
                                             int off_max) {
    ImGuiListClipperRange r = {(int)y1, (int)y2, true, (ImS8)off_min,
                               (ImS8)off_max};
    return r;
  }
};
...
enum ImGuiActivateFlags_ {
  ...
  ImGuiActivateFlags_FromTabbing =
      1 << 3, // Activation requested by a tabbing request
  ImGuiActivateFlags_FromShortcut =
      1 << 4, // Activation requested by an item shortcut via
              // SetNextItemShortcut() function.
};
...
enum ImGuiScrollFlags_ {
  ...
  ImGuiScrollFlags_KeepVisibleCenterY =
      1 << 3, // If item is not visible: scroll to make the item centered on Y
              // axis
  ImGuiScrollFlags_AlwaysCenterX =
      1 << 4, // Always center the result item on X axis [rarely used]
  ImGuiScrollFlags_AlwaysCenterY =
      1 << 5, // Always center the result item on Y axis [default for Y axis for
              // appearing window)
...
enum ImGuiNavHighlightFlags_ {
  ...
  ImGuiNavHighlightFlags_AlwaysDraw =
      1 << 2, // Draw rectangular highlight if (g.NavId == id) _even_ when using
              // the mouse.
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_IsTabbing = 1 << 10,  // == Focus + Activate if item is
  ...
  ImGuiNavMoveFlags_NoSetNavHighlight =
      1 << 14, // Do not alter the visible state of keyboard vs mouse nav
               // highlight
  ImGuiNavMoveFlags_NoClearActiveId =
      1
      << 15, // (Experimental) Do not clear active id when applying move result
};
...
struct ImGuiNavItemData {
  ...
  float DistAxial;  //      Move    // Best candidate axial distance to current
                    //      NavId
  ImGuiSelectionUserData SelectionUserData; // I+Mov    // Best candidate
...
// ImGuiOldColumnFlags_NoPreserveWidths, ImGuiColumnsFlags_NoForceWithinWindow
// = ImGuiOldColumnFlags_NoForceWithinWindow,
...
struct ImGuiOldColumns {
  ...
  ImRect
      HostInitialClipRect;   // Backup of ClipRect at the time of BeginColumns()
...
enum ImGuiDockNodeFlagsPrivate_ {
  ...
  // ImGuiDockNodeFlags_NoDockingOverCentralNode/ImGuiDockNodeFlags_NoDockingSplit
...
struct IMGUI_API ImGuiDockNode {
  ...
  ImGuiDockNodeFlags LocalFlagsInWindows; // (Write) Flags specific to this
                                          // node, applied from windows
  ImGuiDockNodeFlags MergedFlags; // (Read)  Effective flags (== SharedFlags |
                                  // LocalFlagsInNode | LocalFlagsInWindows)
  ImGuiDockNodeState State;
  ImGuiDockNode *ParentNode;
  ImGuiDockNode *ChildNodes[2]; // [Split node only] Child nodes (left/right or
  ...
  ImVec2 SizeRef; // [Split node only] Last explicitly written-to size
  ...
  ImGuiWindow *VisibleWindow; // Generally point to window which is ID is ==
                              // SelectedTabID, but when CTRL+Tabbing this can
                              // be a different window.
  ImGuiDockNode *CentralNode; // [Root node only] Pointer to central node.
  ImGuiDockNode
      *OnlyNodeWithWindows; // [Root node only] Set when there is a single
  ...
  ImGuiID
      LastFocusedNodeId; // [Root node only] Which of our child docking node
                         // (any ancestor in the hierarchy) was last focused.
  ImGuiID
      SelectedTabId; // [Leaf node only] Which of our tab/window is selected.
  ImGuiID WantCloseTabId; // [Leaf node only] Set when closing a specific
                          // tab/window.
  ImGuiID RefViewportId;  // Reference viewport ID from visible window when
                          // HostWindow == NULL.
  ...
  bool HasCloseButton
      : 1; // Provide space for a close button (if any of the docked window has
           // one). Note that button may be hidden on window without one.
...
// log of colors in ImGuiContext + store offsets into those arrays in
// ImGuiWindow, but it would be more complex as we'd need to double-buffer both
...
// We don't store style.Alpha: dock_node->LastBgColor embeds it and otherwise it
// would only affect the docking tab, which intuitively I would say we don't
// want to.
...
// ImGuiViewport Private/Internals fields (cardinal sin: we are using
// inheritance!) Every instance of ImGuiViewport is in fact a ImGuiViewportP.
struct ImGuiViewportP : public ImGuiViewport {
  ImGuiWindow *Window; // Set when the viewport is owned by a window (and
                       // ImGuiViewportFlags_CanHostOtherWindows is NOT set)
  int Idx;
  ...
  ImDrawList *BgFgDrawLists[2]; // Convenience background (0) and foreground (1)
                                // draw lists. We use them to draw software
                                // mouser cursor when io.MouseDrawCursor is set
                                // and to draw most debug overlays.
  ImDrawData DrawDataP;
  ImDrawDataBuilder
      DrawDataBuilder; // Temporary data while building final ImDrawData
  ...
  ImVec2 BuildWorkOffsetMin; // Work Area: Offset being built during current
                             // frame. Generally >= 0.0f.
  ImVec2 BuildWorkOffsetMax; // Work Area: Offset being built during current
                             // frame. Generally <= 0.0f.
  ...
  ImVec2 CalcWorkRectSize(const ImVec2 &off_min, const ImVec2 &off_max) const {
    return ImVec2(ImMax(0.0f, Size.x - off_min.x + off_max.x),
                  ImMax(0.0f, Size.y - off_min.y + off_max.y));
  }
  void UpdateWorkRect() {
  ...
  // Helpers to retrieve ImRect (we don't need to store BuildWorkRect as every
...
struct ImGuiWindowSettings {
  ImGuiID ID;
  ImVec2ih Pos; // NB: Settings position are stored RELATIVE to the viewport!
                // Whereas runtime ones are absolute positions.
  ImVec2ih Size;
  ImVec2ih ViewportPos;
  ImGuiID ViewportId;
  ImGuiID
      DockId; // ID of last known DockNode (even if the DockNode is invisible
...
enum ImGuiDebugLogFlags_ {
  ...
  ImGuiDebugLogFlags_OutputToTestEngine =
      1 << 21, // Also send output to Test Engine
};
...
struct ImGuiDebugAllocInfo {
  ...
  ImGuiDebugAllocEntry
      LastEntriesBuf[6]; // Track last 6 frames that had allocations
...
struct ImGuiContext {
  bool Initialized;
  bool FontAtlasOwnedByContext; // IO.Fonts-> is owned by the ImGuiContext and
  ...
  ImVector<ImGuiInputEvent>
      InputEventsQueue; // Input events which will be trickled/written into IO
                        // structure.
  ImVector<ImGuiInputEvent>
  ...
  ImVec2 WindowsHoverPadding; // Padding around resizable windows for which
                              // hovering on counts as hovering the window ==
                              // ImMax(style.TouchExtraPadding,
                              // WINDOWS_HOVER_PADDING)
  ImGuiID DebugBreakInWindow; // Set to break in Begin() call.
  ImGuiWindow *CurrentWindow; // Window being drawn into
  ImGuiWindow *HoveredWindow; // Window the mouse is hovering. Will typically
                              // catch mouse inputs.
  ImGuiWindow
      *HoveredWindowUnderMovingWindow; // Hovered window ignoring MovingWindow.
                                       // Only set if MovingWindow is set.
  ImGuiWindow *MovingWindow; // Track the window we clicked on (in order to
  ...
  ImGuiID
      DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from GetID
  ...
  ImGuiID
      ActiveIdIsAlive; // Active widget has been seen this frame (we can't use a
  ...
  ImVec2
      ActiveIdClickOffset; // Clicked offset from upper-left corner, if
                           // applicable (currently only set by ButtonBehavior)
  ImGuiWindow *ActiveIdWindow;
  ImGuiInputSource
      ActiveIdSource; // Activating source: ImGuiInputSource_Mouse OR
  ...
  ImGuiID
      LastActiveId; // Store the last non-zero ActiveId, useful for animation.
  ...
  // owner.
  // - Input query can then read input by specifying ImGuiKeyOwner_Any (== 0),
  // ImGuiKeyOwner_None (== -1) or a custom ID.
  // - Routing is requested ahead of time for a given chord (Key + Mods) and
  // granted in NewFrame().
  double LastKeyModsChangeTime; // Record the last time key mods changed (affect
                                // repeat delay when using shortcut logic)
  double LastKeyModsChangeFromNoneTime; // Record the last time key mods changed
                                        // away from being 0 (affect repeat
                                        // delay when using shortcut logic)
  double LastKeyboardKeyPressTime;      // Record the last time a keyboard key
                                   // (ignore mouse/gamepad ones) was pressed.
  ImBitArrayForNamedKeys
      KeysMayBeCharInput; // Lookup to tell if a key can emit char input, see
                          // IsKeyChordPotentiallyCharInput(). sizeof() = 20
                          // bytes
  ImGuiKeyOwnerData KeysOwnerData[ImGuiKey_NamedKey_COUNT];
  ImGuiKeyRoutingTable KeysRoutingTable;
  ImU32 ActiveIdUsingNavDirMask; // Active widget will want to read those nav
  ...
  // Shared stacks
  ImGuiCol
      DebugFlashStyleColorIdx; // (Keep close to ColorStack to share cache line)
  ImVector<ImGuiColorMod>
      ColorStack; // Stack for PushStyleColor()/PopStyleColor() - inherited by
                  // Begin()
  ImVector<ImGuiStyleMod>
      StyleVarStack; // Stack for PushStyleVar()/PopStyleVar() - inherited by
                     // Begin()
  ImVector<ImFont *>
  ...
  ImVector<ImGuiItemFlags>
      ItemFlagsStack; // Stack for PushItemFlag()/PopItemFlag() - inherited by
                      // Begin()
  ImVector<ImGuiGroupData> GroupStack; // Stack for BeginGroup()/EndGroup() -
                                       // not inherited by Begin()
  ImVector<ImGuiPopupData> OpenPopupStack; // Which popups are open (persistent)
  ImVector<ImGuiPopupData> BeginPopupStack; // Which level of BeginPopup() we
                                            // are in (reset every frame)
  ImVector<ImGuiNavTreeNodeData>
      NavTreeNodeStack; // Stack for TreeNode() when a NavLeft requested is
                        // emitted.

  // Viewports
  ImVector<ImGuiViewportP *>
      Viewports; // Active viewports (always 1+, and generally 1 unless
                 // multi-viewports are enabled). Each viewports hold their copy
                 // of ImDrawData.
  ...
  ImGuiWindow *NavWindow;  // Focused window for navigation. Could be called
                           // 'FocusedWindow'
  ImGuiID NavId;           // Focused item for navigation
  ImGuiID NavFocusScopeId; // Focused focus scope (e.g. selection code often
                           // wants to "clear other items" when landing on an
                           // item of the same scope)
  ImVector<ImGuiFocusScopeData>
      NavFocusRoute; // Reversed copy focus scope stack for NavId (should
                     // contains NavFocusScopeId). This essentially follow the
                     // window->ParentWindowForFocusRoute chain.
  ImGuiID
      NavActivateId; // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) ||
                     // IsKeyDown(ImGuiKey_Enter) ||
                     // IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0,
                     // also set when calling ActivateItem()
  ImGuiID
      NavActivateDownId; // ~~ IsKeyDown(ImGuiKey_Space) ||
                         // IsKeyDown(ImGuiKey_Enter) ||
                         // IsKeyDown(ImGuiKey_NavGamepadActivate) ? NavId : 0
  ImGuiID NavActivatePressedId; // ~~ IsKeyPressed(ImGuiKey_Space) ||
  ...
  ImGuiID NavJustMovedToFocusScopeId; // Just navigated to this focus scope id
                                      // (result of a successfully MoveRequest).
  ImGuiKeyChord NavJustMovedToKeyMods;
  ImGuiID NavNextActivateId; // Set by ActivateItem(), queued until next frame.
  ImGuiActivateFlags NavNextActivateFlags;
  ImGuiInputSource
      NavInputSource;     // Keyboard or Gamepad mode? THIS CAN ONLY BE
                          // ImGuiInputSource_Keyboard or ImGuiInputSource_Mouse
  ImGuiNavLayer NavLayer; // Layer we are navigating on. For now the system is
  ...
  bool NavMousePosDirty; // When set we will update mouse position if
                         // (io.ConfigFlags &
                         // ImGuiConfigFlags_NavEnableSetMousePos) if set (NB:
  ...
  ImRect NavScoringRect;   // Rectangle used for scoring, in screen space. Based
                           // of window->NavRectRel[], modified for directional
                           // navigation scoring.
  ImRect NavScoringNoClipRect; // Some nav operations (such as PageUp/PageDown)
  ...
  ImGuiNavItemData
      NavTabbingResultFirst; // First tabbing request candidate within NavWindow
  ...
  ImGuiKeyChord ConfigNavWindowingKeyPrev; // = ImGuiMod_Ctrl | ImGuiMod_Shift |
                                           // ImGuiKey_Tab
  ImGuiWindow *NavWindowingTarget; // Target window when doing CTRL+Tab (or Pad
  ...
  ImGuiWindow *NavWindowingListWindow; // Internal window actually listing the
                                       // CTRL+Tab contents
  ...
  ImRect DragDropTargetRect; // Store rectangle of current target candidate (we
  ...
  float DragDropAcceptIdCurrRectSurface; // Target item surface (we resolve
                                         ...
                                         // the smaller surface)
  ImGuiID DragDropAcceptIdCurr; // Target item id (set at the time of accepting
                                // the payload)
  ImGuiID DragDropAcceptIdPrev; // Target item id from previous frame (we need
                                // to store this to allow for overlapping drag
                                // and drop targets)
  int DragDropAcceptFrameCount; // Last time a target expressed a desire to
                                // accept the source
  ImGuiID DragDropHoldJustPressedId; // Set when holding a payload just made
  ...
  ImVector<float> TablesLastTimeActive; // Last used timestamp of each tables
  ...
  ImGuiID HoverItemUnlockedStationaryId;   // Mouse has once been stationary on
                                           // this item. Only reset after
                                           // departing the item.
  ImGuiID HoverWindowUnlockedStationaryId; // Mouse has once been stationary on
  ...
  ImGuiID
      TempInputId; // Temporary text input when CTRL+clicking on a slider, etc.
  int BeginMenuDepth;
  int BeginComboDepth;
  ImGuiColorEditFlags
  ...
  ImVec4 ColorPickerRef; // Initial/reference color at the time of opening the
                         // color picker.
  ImGuiComboPreviewData ComboPreviewData;
  ImRect WindowResizeBorderExpectedRect; // Expected border rect, switch to
  ...
  ImVector<ImGuiID> MenusIdSubmittedThisFrame; // A list of menu IDs that were
                                               // rendered at least once
  ImGuiTypingSelectState
      TypingSelectState; // State for GetTypingSelectRequest()

  // Platform support
  ImGuiPlatformImeData PlatformImeData; // Data updated by current frame
  ImGuiPlatformImeData
      PlatformImeDataPrev; // Previous frame data (when changing we will call
  ...
  void (*DockNodeWindowMenuHandler)(ImGuiContext *ctx, ImGuiDockNode *node,
                                    ImGuiTabBar *tab_bar);
  ...
  ImGuiTextBuffer LogBuffer; // Accumulation buffer when log to clipboard. This
                             // is pointer so our GImGui static constructor
  ...
  // Debug Tools
  // (some of the highly frequently used data are interleaved in other
  // structures above: DebugBreakXXX fields, DebugHookIdInfo, DebugLocateId
  // etc.)
  ...
  ImU8 DebugLocateFrames; // For DebugLocateItemOnHover(). This is used together
                          // with DebugLocateId which is in a hot/cached spot
                          // above.
  bool DebugBreakInLocateId;           // Debug break in ItemAdd() call for
                                       // g.DebugLocateId.
  ...
  ImGuiContext(ImFontAtlas *shared_font_atlas) {
    ...
    PlatformImeDataPrev.InputPos =
        ImVec2(-1.0f, -1.0f); // Different to ensure initial submission
    ...
    // Same as DebugBreakClearData(). Those fields are scattered in their
    // respective subsystem to stay in hot-data locations
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec2 CursorStartPos; // Initial position after Begin(), generally ~ window
  ...
  bool IsSameLine;
  bool IsSetPos;
  ImVec1 Indent; // Indentation / start position from left of window (increased
  ...
  ImGuiStorage *StateStorage; // Current persistent per-window storage (store
  ...
  ImVector<float> ItemWidthStack; // Store item widths to restore (attention:
                                  // .back() is not == ItemWidth)
  ImVector<float>
      TextWrapPosStack; // Store text wrap pos to restore (attention: .back() is
                        // not == TextWrapPos)
};
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiChildFlags ChildFlags; // Set when window is a child window. See enum
                              // ImGuiChildFlags_
  ImGuiWindowClass
      WindowClass; // Advanced users only. Set with SetNextWindowClass()
  ImGuiViewportP
      *Viewport; // Always set in Begin(). Inactive windows may have a NULL
                 // value here if their viewport was discarded.
  ImGuiID
      ViewportId; // We backup the viewport id (since the viewport may disappear
                  // or never be created if the window is inactive)
  ImVec2
      ViewportPos; // We backup the viewport position (since the viewport may
  ...
  ImVec2
      ContentSize; // Size of contents/scrollable client area (calculated from
                   // the extents reach of the cursor) from previous frame. Does
                   // not include window decoration or window padding.
  ...
  ImGuiID ChildId; // ID of corresponding item in parent window (for navigation
                   // to return from child window to parent window)
  ImVec2 Scroll;
  ImVec2 ScrollMax;
  ImVec2 ScrollTarget; // target scroll position. stored as cursor position with
                       // scrolling canceled out, so the highest point is always
                       // 0.0f. (FLT_MAX for no change)
  ImVec2 ScrollTargetCenterRatio; // 0.0f = scroll so that target position is at
                                  // top, 0.5f = scroll so that target position
                                  // is centered
  ImVec2
      ScrollTargetEdgeSnapDist; // 0.0f = no snapping, >0.0f snapping threshold
  ImVec2 ScrollbarSizes; // Size taken by each scrollbars on their smaller axis.
  ...
  ImS8 HiddenFramesCannotSkipItems; // Hide the window for N frames while
                                    ...
                                    // measure their size
  ImS8 HiddenFramesForRenderOnly;   // Hide the window until frame N at Render()
                                    // time only
  ImS8 DisableInputsFrames;         // Disable window interactions for N frames
  ImGuiCond SetWindowPosAllowFlags
      : 8; // store acceptable condition flags for SetNextWindowPos() use.
  ImGuiCond SetWindowSizeAllowFlags
      : 8; // store acceptable condition flags for SetNextWindowSize() use.
  ImGuiCond SetWindowCollapsedAllowFlags
      : 8; // store acceptable condition flags for SetNextWindowCollapsed() use.
  ImGuiCond SetWindowDockAllowFlags
      : 8; // store acceptable condition flags for SetNextWindowDock() use.
  ImVec2 SetWindowPosVal; // store window position when using a non-zero Pivot
                          // (position set needs to be processed when we know
                          // the window size)
  ImVec2
      SetWindowPosPivot; // store window pivot for positioning. ImVec2(0, 0)
                         // when positioning from top-left corner; ImVec2(0.5f,
                         // 0.5f) for centering; ImVec2(1, 1) for bottom right.

  ImVector<ImGuiID> IDStack; // ID stack. ID are hashes seeded with the value at
                             // the top of the stack. (In theory this should be
                             // in the TempData structure)
  ImGuiWindowTempData DC; // Temporary per-window data, reset at the beginning
  ...
  ImRect OuterRectClipped; // == Window->Rect() just after setup in Begin(). ==
                           // window->Rect() for root window.
  ImRect InnerRect; // Inner rectangle (omit title bar, menu bar, scroll bar)
  ImRect InnerClipRect; // == InnerRect shrunk by WindowPadding*0.5f on each
                        // side, clipped within viewport or parent clip rect.
  ImRect WorkRect; // Initially covers the whole scrolling region. Reduced by
  ...
  ImRect ParentWorkRect; // Backup of WorkRect before entering a container such
  ...
  ImRect
      ContentRegionRect; // FIXME: This is currently confusing/misleading. It is
                         // essentially WorkRect but not handling of scrolling.
                         // We currently rely on it as right/bottom aligned
                         // sizing operation need some size to rely on.
  ImVec2ih HitTestHoleSize; // Define an optional rectangular hole where mouse
  ...
  ImDrawList
      *DrawList; // == &DrawListInst (for backward compatibility reason with
                 // code using imgui_internal.h we keep this a pointer)
  ImDrawList DrawListInst;
  ImGuiWindow *ParentWindow; // If we are a child _or_ popup _or_ docked window,
                             // this is pointing to our parent. Otherwise NULL.
  ImGuiWindow *ParentWindowInBeginStack;
  ImGuiWindow
      *RootWindow; // Point to ourself or first ancestor that is not a child
  ...
  ImGuiWindow
      *RootWindowDockTree; // Point to ourself or first ancestor that is not a
                           // child window. Cross through dock nodes.
  ImGuiWindow
      *RootWindowForTitleBarHighlight; // Point to ourself or first ancestor
                                       // which will display TitleBgActive color
                                       // when this window is active.
  ImGuiWindow *RootWindowForNav; // Point to ourself or first ancestor which
                                 // doesn't have the NavFlattened flag.
  ImGuiWindow
      *ParentWindowForFocusRoute; // Set to manual link a window to its logical
                                  // parent so that Shortcut() chain are honoerd
                                  // (e.g. Tool linked to Document)
  ...
  ImGuiID NavLastIds[ImGuiNavLayer_COUNT]; // Last known NavId for this window,
                                           // per layer (0/1)
  ImRect NavRectRel[ImGuiNavLayer_COUNT];  // Reference rectangle, in window
                                           // relative space
  ImVec2 NavPreferredScoringPosRel
      [ImGuiNavLayer_COUNT]; // Preferred X/Y position updated when moving on a
  ...
  // Docking
  bool DockIsActive
      : 1; // When docking artifacts are actually visible. When this is set,
           // DockNode is guaranteed to be != NULL. ~~ (DockNode != NULL) &&
           // (DockNode->Windows.Size > 1).
  ...
  ImGuiDockNode *DockNode; // Which node are we docked into. Important: Prefer
  ...
  ImGuiID
      DockId; // Backup of last valid DockNode->ID, so single window remember
...
enum ImGuiTabItemFlagsPrivate_ {
  ...
  ImGuiTabItemFlags_Button = 1 << 21, // Used by TabItemButton, change the tab
...
struct ImGuiTabItem {
  ...
  ImGuiWindow *Window; // When TabItem is part of a DockNode's TabBar, we hold
                       // on to a window.
  ...
  ImS16 IndexDuringLayout; // Index only used during TabBarLayout(). Tabs gets
...
struct IMGUI_API ImGuiTabBar {
  ...
  ImGuiID NextSelectedTabId; // Next selected tab/window. Will also trigger a
                             // scrolling animation
  ...
  ImVec2 BackupCursorPos;
  ImGuiTextBuffer TabsNames; // For non-docking tab bar we re-append names in a
                             // contiguous buffer.
...
struct ImGuiTableColumn {
  ImGuiTableColumnFlags
      Flags; // Flags after some patching (not directly same as provided by
  ...
  ImGuiTableColumnIdx
      DisplayOrder; // Index within Table's IndexToDisplayOrder[] (column may be
                    // reordered by users)
  ImGuiTableColumnIdx IndexWithinEnabledSet; // Index within enabled/visible set
                                             // (<= IndexToDisplayOrder)
  ImGuiTableColumnIdx
  ...
  ImGuiTableDrawChannelIdx
      DrawChannelUnfrozen; // Draw channels for unfrozen rows
  bool IsEnabled; // IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled)
                  // == 0
  ...
  ImU8 SortDirectionsAvailCount
      : 2; // Number of available sort directions (0 to 3)
  ImU8 SortDirectionsAvailMask
      : 4; // Mask of available sort directions (1-bit each)
...
// could be moved to ImGuiTableTempData?) sizeof() ~ 24 bytes
...
// ImGuiTableTempData: e.g. SortSpecs, incoming RowData sizeof() ~ 580 bytes +
...
struct IMGUI_API ImGuiTable {
  ...
  void *RawData; // Single allocation to hold Columns[], DisplayOrderToIndex[]
                 // and RowCellData[]
  ImGuiTableTempData *TempData; // Transient data while table is active. Point
  ...
  ImSpan<ImGuiTableCellData>
      RowCellData; // Point within RawData[]. Store cells background requests
                   // for current row.
  ImBitArrayPtr
      EnabledMaskByDisplayOrder; // Column DisplayOrder -> IsEnabled map
  ImBitArrayPtr
  ...
  ImBitArrayPtr VisibleMaskByIndex; // Column Index -> IsVisibleX|IsVisibleY map
  ...
  ImS16 InstanceInteracted; // Mark which instance (generally 0) of the same ID
  ...
  ImGuiTableRowFlags RowFlags
      : 16; // Current row flags, see ImGuiTableRowFlags_
  ...
  float AngledHeadersSlope;  // Set by TableAngledHeadersRow(), used in
                             // TableUpdateLayout()
  ImRect OuterRect; // Note: for non-scrolling table, OuterRect.Max.y is often
                    // FLT_MAX until EndTable(), unless a height has been
                    // specified in BeginTable().
  ImRect InnerRect; // InnerRect but without decoration. As with OuterRect, for
                    // non-scrolling tables, InnerRect.Max.y is
  ImRect WorkRect;
  ImRect InnerClipRect;
  ImRect
      BgClipRect; // We use this to cpu-clip cell background color fill, evolve
                  // during the frame as we cross frozen rows boundaries
  ImRect Bg0ClipRectForDrawCmd; // Actual ImDrawCmd clip rect for BG0/1 channel.
                                // This tends to be == OuterWindow->ClipRect at
                                // BeginTable() because output in BG0/BG1 is
                                // cpu-clipped
  ImRect Bg2ClipRectForDrawCmd; // Actual ImDrawCmd clip rect for BG2 channel.
                                // This tends to be a correct, tight-fit,
                                // because output to BG2 are done by widgets
                                // relying on regular ClipRect.
  ImRect HostClipRect; // This is used to check if we can eventually merge our
  ...
  ImGuiWindow *InnerWindow; // Window holding the table data (== OuterWindow or
                            // a child window)
  ImGuiTextBuffer ColumnsNames; // Contiguous buffer holding columns names
  ImDrawListSplitter
      *DrawSplitter; // Shortcut to TempData->DrawSplitter while in table.
                     // Isolate draw commands per columns to avoid switching
                     // clip rect constantly
  ...
  ImGuiTableSortSpecs SortSpecs; // Public facing sorts specs, this is what we
                                 // return in TableGetSortSpecs()
  ImGuiTableColumnIdx SortSpecsCount;
  ImGuiTableColumnIdx
      ColumnsEnabledCount; // Number of enabled columns (<= ColumnsCount)
  ImGuiTableColumnIdx
      ColumnsEnabledFixedCount; // Number of enabled columns using fixed width
                                // (<= ColumnsCount)
  ...
  ImGuiTableColumnIdx HoveredColumnBorder; // Index of column whose right-border
  ...
  ImGuiTableColumnIdx ResizedColumn; // Index of column being resized. Reset
  ...
  ImGuiTableColumnIdx
      LeftMostEnabledColumn; // Index of left-most non-hidden column.
  ImGuiTableColumnIdx
  ...
  ImGuiTableColumnIdx FreezeColumnsCount;   // Actual frozen columns count (==
  ...
  bool IsSettingsDirty; // Set when table settings have changed and needs to be
                        // reported into ImGuiTableSetttings data.
...
// useful for debugging/metrics. sizeof() ~ 136 bytes.
struct IMGUI_API ImGuiTableTempData {
  int TableIndex;                // Index in g.Tables.Buf[] pool
  float LastTimeActive;          // Last timestamp this structure was used
  float AngledHeadersExtraWidth; // Used in EndTable()
  ImVector<ImGuiTableHeaderData>
      AngledHeadersRequests; // Used in TableAngledHeadersRow()

  ImVec2 UserOuterSize; // outer_size.x passed to BeginTable()
  ImDrawListSplitter DrawSplitter;

  ImRect HostBackupWorkRect; // Backup of InnerWindow->WorkRect at the end of
                             // BeginTable()
  ImRect HostBackupParentWorkRect; // Backup of InnerWindow->ParentWorkRect at
                                   // the end of BeginTable()
  ImVec2 HostBackupPrevLineSize;   // Backup of InnerWindow->DC.PrevLineSize at
                                   // the end of BeginTable()
  ImVec2 HostBackupCurrLineSize;   // Backup of InnerWindow->DC.CurrLineSize at
                                   // the end of BeginTable()
  ImVec2 HostBackupCursorMaxPos;   // Backup of InnerWindow->DC.CursorMaxPos at
                                   // the end of BeginTable()
  ImVec1 HostBackupColumnsOffset;  // Backup of OuterWindow->DC.ColumnsOffset at
                                   // the end of BeginTable()
...
IMGUI_API bool IsWindowWithinBeginStackOf(ImGuiWindow *window,
                                          ImGuiWindow *potential_parent);
...
IMGUI_API void BringWindowToDisplayBehind(ImGuiWindow *window,
                                          ImGuiWindow *above_window);
IMGUI_API int FindWindowDisplayIndex(ImGuiWindow *window);
IMGUI_API ImGuiWindow *
...
IMGUI_API void Shutdown(); // Since 1.60 this is a _private_ function. You can
...
IMGUI_API void StartMouseMovingWindowOrNode(ImGuiWindow *window,
                                            ImGuiDockNode *node, bool undock);
IMGUI_API void UpdateMouseMovingWindowNewFrame();
IMGUI_API void UpdateMouseMovingWindowEndFrame();

// Generic context hooks
IMGUI_API ImGuiID AddContextHook(ImGuiContext *context,
...
// Viewports
IMGUI_API void TranslateWindowsInViewport(ImGuiViewportP *viewport,
                                          const ImVec2 &old_pos,
                                          const ImVec2 &new_pos);
IMGUI_API void ScaleWindowsInViewport(ImGuiViewportP *viewport, float scale);
IMGUI_API void DestroyPlatformWindow(ImGuiViewportP *viewport);
IMGUI_API void SetWindowViewport(ImGuiWindow *window, ImGuiViewportP *viewport);
IMGUI_API void SetCurrentViewport(ImGuiWindow *window,
                                  ImGuiViewportP *viewport);
IMGUI_API const ImGuiPlatformMonitor *
GetViewportPlatformMonitor(ImGuiViewport *viewport);
IMGUI_API ImGuiViewportP *
FindHoveredViewportFromPlatformWindowStack(const ImVec2 &mouse_platform_pos);
...
IMGUI_API void ScrollToRect(ImGuiWindow *window, const ImRect &rect,
...
IMGUI_API bool IsWindowContentHoverable(ImGuiWindow *window,
                                        ImGuiHoveredFlags flags = 0);
IMGUI_API bool IsClippedEx(const ImRect &bb, ImGuiID id);
IMGUI_API void SetLastItemData(ImGuiID item_id, ImGuiItemFlags in_flags,
...
IMGUI_API bool BeginTooltipEx(ImGuiTooltipFlags tooltip_flags,
...
// Menus
IMGUI_API bool BeginViewportSideBar(const char *name, ImGuiViewport *viewport,
                                    ImGuiDir dir, float size,
                                    ImGuiWindowFlags window_flags);
IMGUI_API bool BeginMenuEx(const char *label, const char *icon,
                           bool enabled = true);
IMGUI_API bool MenuItemEx(const char *label, const char *icon,
                          const char *shortcut = NULL, bool selected = false,
                          bool enabled = true);

// Combos
IMGUI_API bool BeginComboPopup(ImGuiID popup_id, const ImRect &bb,
...
IMGUI_API void
NavMoveRequestResolveWithPastTreeNode(ImGuiNavItemData *result,
...
IMGUI_API void SetNavID(ImGuiID id, ImGuiNavLayer nav_layer,
...
IMGUI_API void ActivateItemByID(
    ImGuiID id); // Activate an item by ID (button, checkbox, tree node etc.).
                 // Activation is queued and processed on the next frame when
                 // the item is encountered again.
...
inline bool IsNamedKeyOrModKey(ImGuiKey key) {
  return (key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END) ||
         key == ImGuiMod_Ctrl || key == ImGuiMod_Shift || key == ImGuiMod_Alt ||
         key == ImGuiMod_Super || key == ImGuiMod_Shortcut;
}
...
inline ImGuiKey ConvertSingleModFlagToKey(ImGuiContext *ctx, ImGuiKey key) {
  ...
  if (key == ImGuiMod_Shortcut)
    return (g.IO.ConfigMacOSXBehaviors ? ImGuiKey_ReservedForModSuper
...
IMGUI_API ImVec2 GetKeyMagnitude2d(ImGuiKey key_left, ImGuiKey key_right,
                                   ImGuiKey key_up, ImGuiKey key_down);
IMGUI_API float GetNavTweakPressedAmount(ImGuiAxis axis);
IMGUI_API int CalcTypematicRepeatAmount(float t0, float t1, float repeat_delay,
                                        float repeat_rate);
IMGUI_API void GetTypematicRepeatRate(ImGuiInputFlags flags,
...
// - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0),
// ImGuiKeyOwner_None (== -1) or a custom ID.
// - Legacy input queries (without specifying an owner or _Any or _None) are
// equivalent to using ImGuiKeyOwner_Any (== 0).
...
IMGUI_API void SetKeyOwner(ImGuiKey key, ImGuiID owner_id,
...
SetItemKeyOwner(ImGuiKey key,
                ImGuiInputFlags flags =
                    0); // Set key owner to last item if it is hovered or
                        // active. Equivalent to 'if (IsItemHovered() ||
                        // IsItemActive()) { SetKeyOwner(key, GetItemID());'.
IMGUI_API bool
TestKeyOwner(ImGuiKey key,
             ImGuiID owner_id); // Test that key is either not owned, either
                                // owned by 'owner_id'
...
// Ownership
// - Important: legacy IsKeyPressed(ImGuiKey, bool repeat=true) _DEFAULTS_ to
// repeat, new IsKeyPressed() requires _EXPLICIT_ ImGuiInputFlags_Repeat flag.
...
// - Specifying a value for 'ImGuiID owner' will test that EITHER the key is NOT
// owned (UNLESS locked), EITHER the key is owned by 'owner'.
//   Legacy functions use ImGuiKeyOwner_Any meaning that they typically ignore
...
IMGUI_API bool IsMouseClicked(ImGuiMouseButton button, ImGuiID owner_id,
                              ImGuiInputFlags flags = 0);
IMGUI_API bool IsMouseReleased(ImGuiMouseButton button, ImGuiID owner_id);
IMGUI_API bool IsMouseDoubleClicked(ImGuiMouseButton button, ImGuiID owner_id);

// [EXPERIMENTAL] Shortcut Routing
// - ImGuiKeyChord = a ImGuiKey optionally OR-red with
// ImGuiMod_Alt/ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Super.
//     ImGuiKey_C                 (accepted by functions taking ImGuiKey or
//     ImGuiKeyChord) ImGuiKey_C | ImGuiMod_Ctrl (accepted by functions taking
//     ImGuiKeyChord)
//   ONLY ImGuiMod_XXX values are legal to 'OR' with an ImGuiKey. You CANNOT
//   'OR' two ImGuiKey values.
// - When using one of the routing flags (e.g. ImGuiInputFlags_RouteFocused):
...
IMGUI_API bool IsKeyChordPressed(ImGuiKeyChord key_chord, ImGuiID owner_id,
                                 ImGuiInputFlags flags = 0);
IMGUI_API void SetNextItemShortcut(ImGuiKeyChord key_chord);
IMGUI_API bool Shortcut(ImGuiKeyChord key_chord, ImGuiID owner_id = 0,
                        ImGuiInputFlags flags = 0);
IMGUI_API bool SetShortcutRouting(
    ImGuiKeyChord key_chord, ImGuiID owner_id,
    ImGuiInputFlags flags = 0); // owner_id needs to be explicit and cannot be 0
...
IMGUI_API void
DockContextClearNodes(ImGuiContext *ctx, ImGuiID root_id,
...
IMGUI_API void DockContextQueueDock(ImGuiContext *ctx, ImGuiWindow *target,
                                    ImGuiDockNode *target_node,
                                    ImGuiWindow *payload, ImGuiDir split_dir,
                                    float split_ratio, bool split_outer);
IMGUI_API void DockContextQueueUndockWindow(ImGuiContext *ctx,
                                            ImGuiWindow *window);
IMGUI_API void DockContextQueueUndockNode(ImGuiContext *ctx,
                                          ImGuiDockNode *node);
IMGUI_API void
DockContextProcessUndockWindow(ImGuiContext *ctx, ImGuiWindow *window,
                               bool clear_persistent_docking_ref = true);
IMGUI_API void DockContextProcessUndockNode(ImGuiContext *ctx,
                                            ImGuiDockNode *node);
IMGUI_API bool DockContextCalcDropPosForDocking(
    ImGuiWindow *target, ImGuiDockNode *target_node,
...
IMGUI_API void DockNodeWindowMenuHandler_Default(ImGuiContext *ctx,
                                                 ImGuiDockNode *node,
                                                 ImGuiTabBar *tab_bar);
...
IMGUI_API void SetWindowDock(ImGuiWindow *window, ImGuiID dock_id,
                             ImGuiCond cond);
...
// - Do not hold on ImGuiDockNode* pointers! They may be invalidated by any
// split/merge/remove operation and every frame.
// - To create a DockSpace() node, make sure to set the
// ImGuiDockNodeFlags_DockSpace flag when calling DockBuilderAddNode().
...
IMGUI_API void DockBuilderRemoveNode(
    ImGuiID node_id); // Remove node and all its child, undock all windows
IMGUI_API void
DockBuilderRemoveNodeDockedWindows(ImGuiID node_id,
...
IMGUI_API ImGuiID DockBuilderSplitNode(
    ImGuiID node_id, ImGuiDir split_dir, float size_ratio_for_node_at_dir,
    ImGuiID *out_id_at_dir,
    ImGuiID
        *out_id_at_opposite_dir); // Create 2 child nodes in this parent node.
IMGUI_API void
DockBuilderCopyDockSpace(ImGuiID src_dockspace_id, ImGuiID dst_dockspace_id,
                         ImVector<const char *> *in_window_remap_pairs);
IMGUI_API void DockBuilderCopyNode(ImGuiID src_node_id, ImGuiID dst_node_id,
                                   ImVector<ImGuiID> *out_node_remap_pairs);
IMGUI_API void DockBuilderCopyWindowSettings(const char *src_name,
                                             const char *dst_name);
...
IMGUI_API int
TypingSelectFindMatch(ImGuiTypingSelectRequest *req, int items_count,
                      const char *(*get_item_name_func)(void *, int),
                      void *user_data, int nav_item_idx);
IMGUI_API int TypingSelectFindNextSingleCharMatch(
    ImGuiTypingSelectRequest *req, int items_count,
    const char *(*get_item_name_func)(void *, int), void *user_data,
    int nav_item_idx);
IMGUI_API int
TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest *req, int items_count,
                                 const char *(*get_item_name_func)(void *, int),
                                 void *user_data);
...
IMGUI_API int
TableGetHoveredRow(); // Retrieve *PREVIOUS FRAME* hovered row. This difference
                      // with TableGetHoveredColumn() is the reason why this is
                      // not public yet.
IMGUI_API float TableGetHeaderRowHeight();
IMGUI_API float TableGetHeaderAngledMaxLabelWidth();
IMGUI_API void TablePushBackgroundChannel();
IMGUI_API void TablePopBackgroundChannel();
IMGUI_API void TableAngledHeadersRowEx(ImGuiID row_id, float angle,
...
IMGUI_API void
TableDrawDefaultContextMenu(ImGuiTable *table,
...
IMGUI_API void TableFixColumnSortDirection(ImGuiTable *table,
                                           ImGuiTableColumn *column);
IMGUI_API float TableGetColumnWidthAuto(ImGuiTable *table,
                                        ImGuiTableColumn *column);
...
IMGUI_API ImGuiID TableGetColumnResizeID(ImGuiTable *table, int column_n,
                                         int instance_no = 0);
...
IMGUI_API ImGuiTabItem *
TabBarFindMostRecentlySelectedTabForActiveWindow(ImGuiTabBar *tab_bar);
...
IMGUI_API void TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags,
                            ImGuiWindow *window);
IMGUI_API void TabBarRemoveTab(ImGuiTabBar *tab_bar, ImGuiID tab_id);
IMGUI_API void TabBarCloseTab(ImGuiTabBar *tab_bar, ImGuiTabItem *tab);
IMGUI_API void TabBarQueueFocus(ImGuiTabBar *tab_bar, ImGuiTabItem *tab);
IMGUI_API void TabBarQueueReorder(ImGuiTabBar *tab_bar, ImGuiTabItem *tab,
                                  int offset);
IMGUI_API void TabBarQueueReorderFromMousePos(ImGuiTabBar *tab_bar,
                                              ImGuiTabItem *tab,
                                              ImVec2 mouse_pos);
...
IMGUI_API ImVec2 TabItemCalcSize(const char *label,
...
IMGUI_API void RenderColorRectWithAlphaCheckerboard(
    ...
    ImDrawFlags flags = 0);
IMGUI_API void
RenderNavHighlight(const ImRect &bb, ImGuiID id,
                   ImGuiNavHighlightFlags flags =
                       ImGuiNavHighlightFlags_None); // Navigation highlight
IMGUI_API const char *FindRenderedTextEnd(
    const char *text,
    const char *text_end =
        NULL); // Find the optional ## from which we stop displaying text.
IMGUI_API void RenderMouseCursor(ImVec2 pos, float scale,
...
IMGUI_API void RenderArrowPointingAt(ImDrawList *draw_list, ImVec2 pos,
                                     ...
                                     ImU32 col);
IMGUI_API void RenderArrowDockMenu(ImDrawList *draw_list, ImVec2 p_min,
                                   float sz, ImU32 col);
...
IMGUI_API void RenderRectFilledWithHole(ImDrawList *draw_list,
                                        const ImRect &outer,
                                        const ImRect &inner, ImU32 col,
                                        float rounding);
IMGUI_API ImDrawFlags CalcRoundingFlagsForRectInRect(const ImRect &r_in,
                                                     const ImRect &r_outer,
                                                     float threshold);
...
IMGUI_API void SeparatorTextEx(ImGuiID id, const char *label,
                               const char *label_end, float extra_width);
IMGUI_API bool CheckboxFlags(const char *label, ImS64 *flags,
                             ImS64 flags_value);
IMGUI_API bool CheckboxFlags(const char *label, ImU64 *flags,
                             ImU64 flags_value);
...
IMGUI_API ImGuiID GetWindowResizeCornerID(ImGuiWindow *window,
                                          int n); // 0..3: corners
...
IMGUI_API bool DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
...
IMGUI_API void
DebugAllocHook(ImGuiDebugAllocInfo *info, int frame_count, void *ptr,
               size_t size); // size >= 0 : alloc, size = -1 : free

// Debug Tools
IMGUI_API void ErrorCheckEndFrameRecover(ImGuiErrorLogCallback log_callback,
                                         void *user_data = NULL);
IMGUI_API void ErrorCheckEndWindowRecover(ImGuiErrorLogCallback log_callback,
                                          void *user_data = NULL);
...
IMGUI_API void
DebugLocateItem(ImGuiID target_id); // Call sparingly: only 1 at the same time!
IMGUI_API void DebugLocateItemOnHover(
    ImGuiID target_id); // Only call on reaction to a mouse Hover: because only
                        // 1 at the same time!
IMGUI_API void DebugLocateItemResolveWithLastItem();
IMGUI_API void DebugBreakClearData();
IMGUI_API bool DebugBreakButton(const char *label,
                                const char *description_of_location);
IMGUI_API void DebugBreakButtonTooltip(bool keyboard_only,
                                       const char *description_of_location);
IMGUI_API void ShowFontAtlas(ImFontAtlas *atlas);
IMGUI_API void DebugHookIdInfo(ImGuiID id, ImGuiDataType data_type,
...
IMGUI_API void DebugNodeDrawCmdShowMeshAndBoundingBox(
    ImDrawList *out_draw_list, const ImDrawList *draw_list,
...
IMGUI_API void DebugNodeWindowsList(ImVector<ImGuiWindow *> *windows,
                                    const char *label);
IMGUI_API void
DebugNodeWindowsListByBeginStackParent(ImGuiWindow **windows, int windows_size,
                                       ImGuiWindow *parent_in_begin_stack);
IMGUI_API void DebugNodeViewport(ImGuiViewportP *viewport);
IMGUI_API void DebugRenderKeyboardPreview(ImDrawList *draw_list);
IMGUI_API void DebugRenderViewportThumbnail(ImDrawList *draw_list,
                                            ImGuiViewportP *viewport,
                                            const ImRect &bb);
...
// copy-and-pasted widgets which used FocusableItemRegister():
//  (Old) IMGUI_VERSION_NUM  < 18209: using 'ItemAdd(....)' and 'bool
//  tab_focused = FocusableItemRegister(...)' (Old) IMGUI_VERSION_NUM >= 18209:
//  using 'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and 'bool tab_focused =
//  (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Focused) != 0' (New)
//  IMGUI_VERSION_NUM >= 18413: using 'ItemAdd(..., ImGuiItemFlags_Inputable)'
//  and 'bool tab_focused = (g.NavActivateId == id && (g.NavActivateFlags &
//  ImGuiActivateFlags_PreferInput))'
// inline bool   FocusableItemRegister(ImGuiWindow* window, ImGuiID id) // ->
// pass ImGuiItemAddFlags_Inputable flag to ItemAdd() inline void
// FocusableItemUnregister(ImGuiWindow* window)                        // ->
...
IMGUI_API void ImFontAtlasBuildRender8bppRectFromString(
...
extern const char *ImGuiTestEngine_FindItemDebugLabel(ImGuiContext *ctx,
                                                      ImGuiID id);
...
#define IMGUI_TEST_ENGINE_ITEM_ADD(_ID, _BB, _ITEM_DATA)                       \
  ...
  ImGuiTestEngineHook_ItemAdd(&g, _ID, _BB,                                    \
                              _ITEM_DATA) // Register item bounding box
