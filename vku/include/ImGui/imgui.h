...
ImGuiSizeCallbackData, ImGuiWindowClass, ImGuiPayload)
...
// [SECTION] Platform Dependent Interfaces (ImGuiPlatformIO,
ImGuiPlatformMonitor, ImGuiPlatformImeData)
...
#pragma warning(disable                                                        \
                : 26495) // [Static Analyzer] Variable 'XXX' is uninitialized.
...
// Temporary storage to output draw commands out of order, used by
// ImDrawListSplitter and ImDrawList::ChannelsSplit()
struct ImDrawCmd;
// A single draw command within a parent ImDrawList (generally maps to 1 GPU
// draw call, unless it is a callback)
struct ImDrawData;
// All draw command lists required to render the frame + pos/size coordinates to
// use for the projection matrix.
struct ImDrawList;
// A single draw command list (generally one per window, conceptually you may
// see this as a dynamic "mesh" builder)
struct ImDrawListSharedData;
// Data shared among multiple draw lists (typically owned by parent ImGui
// context, but you may create one yourself)
struct ImDrawListSplitter;
// Helper to split a draw list into different layers which can be drawn into out
// of order, then flattened back.
struct ImDrawVert;
// A single vertex (pos + uv + col = 20 bytes by default. Override layout with
...
struct ImFontGlyphRangesBuilder; // Helper to build glyph ranges from
                                 // text/string data
struct ImColor;
// Helper functions to create a color that can be converted to either u32 or
...
// Shared state of InputText() when using custom ImGuiInputTextCallback
// (rare/advanced use)
...
struct ImGuiPlatformIO;     // Multi-viewport support: interface for
                            // Platform/Renderer backends + viewports to render
struct ImGuiPlatformMonitor;
// Multi-viewport support: user-provided bounds for each connected
// monitor/display. Used when positioning popups and tooltips to avoid them
// straddling monitors
...
// A Platform Window (always 1 unless multi-viewport are enabled. One per
// platform window to output to). In the future may represent Platform Monitor
struct ImGuiWindowClass;
// Window class (rare/advanced uses: provide hints to the platform backend via
// altered viewport flags and parent/child info)
...
// -> enum ImGuiMouseSource      // Enum; A mouse input source identifier
...
// -> enum ImGuiMouseButton_     // Enum: A mouse button identifier (0=left,
// 1=right, 2=middle)
...
typedef int ImGuiDockNodeFlags; // -> enum ImGuiDockNodeFlags_   // Flags: for
                                // DockSpace()
typedef int ImGuiDragDropFlags;
// -> enum ImGuiDragDropFlags_   // Flags: for BeginDragDropSource(),
...
// -> enum ImGuiTreeNodeFlags_   // Flags: for TreeNode(), TreeNodeEx(),
// CollapsingHeader()
...
// Default: store a pointer or an integer fitting in a pointer (most renderer
...
// A single decoded U32 character/code point. We encode them as multi bytes
// UTF-8 when used in strings.
typedef unsigned short ImWchar16;
// A single decoded U16 character/code point. We encode them as multi bytes
// UTF-8 when used in strings.
...
// access the Style structure (colors, sizes). Always use PushStyleColor(),
...
// valid after Render() and until the next call to NewFrame(). this is what you
// have to render.

// Demo, Debug, Information
IMGUI_API void ShowDemoWindow(bool *p_open = NULL);
// create Demo window. demonstrate most ImGui features. call this to learn about
...
// add style editor block (not a window). you can pass in a reference ImGuiStyle
// structure to compare to, revert to and save to (else it uses the default
// style)
...
// add basic help/info block (not a window): how to manipulate ImGui as an
...
// get draw list associated to the current window, to append your own drawing
// primitives
IMGUI_API float GetWindowDpiScale(); // get DPI scale currently associated to
...
// call before Begin()
IMGUI_API void
SetNextWindowSizeConstraints(const ImVec2 &size_min, const ImVec2 &size_max,
...
// itself is a constraint). Use callback to apply non-trivial programmatic
// constraints.
...
// etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call
// before Begin()
...
IMGUI_API void
SetNextWindowViewport(ImGuiID viewport_id); // set next window viewport
...
// (not recommended) set current window size - call within Begin()/End(). set to
// ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this
...
// [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all
...
// content boundaries min for the full window (roughly (0,0)-Scroll), in window
// coordinates
IMGUI_API ImVec2 GetWindowContentRegionMax();
// content boundaries max for the full window (roughly (0,0)+Size-Scroll) where
// Size can be overridden with SetNextWindowContentSize(), in window coordinates
...
// modify a style color. always use this if you modify the style after
// NewFrame().
IMGUI_API void PushStyleColor(ImGuiCol idx, const ImVec4 &col);
IMGUI_API void PopStyleColor(int count = 1);
IMGUI_API void PushStyleVar(ImGuiStyleVar idx, float val);
// modify a style float variable. always use this if you modify the style after
// NewFrame().
IMGUI_API void PushStyleVar(ImGuiStyleVar idx, const ImVec2 &val);
// modify a style ImVec2 variable. always use this if you modify the style after
// NewFrame().
IMGUI_API void PopStyleVar(int count = 1);
IMGUI_API void PushTabStop(bool tab_stop);
// == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default
...
// push width of items for common large "item+label" widgets. >0.0f: width in
...
// set width of the _next_ common large "item+label" widget. >0.0f: width in
// pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always
// align width to the right side)
IMGUI_API float CalcItemWidth();
// width of item given pushed settings and current cursor position. NOT
// necessarily the width of last item unlike most 'Item' functions.
IMGUI_API void PushTextWrapPos(float wrap_local_pos_x = 0.0f);
// push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f:
// wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in
// window local space
...
// get UV coordinate for a while pixel, useful to draw custom shapes via the
// ImDrawList API
...
// cursor position in absolute coordinates (prefer using this, also more useful
// to work with ImDrawList API).
IMGUI_API void SetCursorScreenPos(
    const ImVec2 &pos); // cursor position in absolute coordinates
IMGUI_API ImVec2 GetCursorPos();
// [window-local] cursor position in window coordinates (relative to window
// position)
...
IMGUI_API void SameLine(float offset_from_start_x = 0.0f,
                        float spacing = -1.0f);
// call between widgets or groups to layout them horizontally. X position given
// in window coordinates.
...
// vertically align upcoming text baseline to FramePadding.y so that it will
// align properly to regularly framed items (call if you have text on a line
// before a framed item)
IMGUI_API float GetTextLineHeight(); // ~ FontSize
IMGUI_API float GetTextLineHeightWithSpacing();
// ~ FontSize + style.ItemSpacing.y (distance in pixels between 2 consecutive
// lines of text)
IMGUI_API float GetFrameHeight(); // ~ FontSize + style.FramePadding.y * 2
IMGUI_API float GetFrameHeightWithSpacing();
// ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y (distance in
...
// calculate unique ID (hash of whole ID stack + given parameter). e.g. if you
...
// raw text without formatting. Roughly equivalent to Text("%s", text) but: A)
...
// shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note
// that this won't work on an auto-resizing window if there's no other widgets
// to extend the window width, yoy may need to set a size using
// SetNextWindowSize().
...
IMGUI_API bool SliderFloat(const char *label, float *v, float v_min,
                           ...
                           ImGuiSliderFlags flags = 0);
// adjust format to decorate the value with a prefix or a suffix for in-slider
// labels or unit display.
...
// initialize current options (generally on application startup) if you want to
...
// helper variation to easily decorelate the id from the displayed string. Read
...
// ~ Indent()+PushID(). Already called by TreeNode() when returning true, but
// you can call TreePush/TreePop yourself if desired.
...
IMGUI_API bool CollapsingHeader(const char *label, bool *p_visible,
                                ImGuiTreeNodeFlags flags = 0);
// when 'p_visible != NULL': if '*p_visible==true' display an additional small
// close button on upper right of the header which will set the bool to false
// when clicked, if '*p_visible==false' don't display the header.
...
IMGUI_API bool Selectable(const char *label, bool selected = false,
                          ...
                          const ImVec2 &size = ImVec2(0, 0));
// "bool selected" carry the selection state (read-only). Selectable() is
// clicked is returns true so you can modify your selection state. size.x==0.0:
// use remaining width, size.x>0.0: specify width. size.y==0.0: use label
// height, size.y>0.0: specify height
IMGUI_API bool Selectable(const char *label, bool *p_selected,
                          ImGuiSelectableFlags flags = 0,
                          const ImVec2 &size = ImVec2(0, 0));
// "bool* p_selected" point to the selection state (read-write), as a convenient
// helper.
...
// append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag
...
// set a text-only tooltip if preceeding item was hovered. override any previous
// call to SetTooltip().
...
IMGUI_API bool BeginPopupContextItem(const char *str_id = NULL,
                                     ImGuiPopupFlags popup_flags = 1);
// open+begin popup when clicked on last item. Use str_id==NULL to associate the
// popup to previous item. If you want to use that on a non-interactive item
// such as Text() you need to pass in an explicit ID here. read comments in
// .cpp!
...
// get latest sort specs for the table (NULL if not sorting).  Lifetime: don't
...
IMGUI_API void TableSetBgColor(ImGuiTableBgTarget target, ImU32 color,
                               int column_n = -1);
// change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for
// details.
...
// get position of column line (in pixels, from the left side of the contents
// region). pass -1 to use current column, otherwise 0..GetColumnsCount()
// inclusive. column 0 is typically 0.0f
IMGUI_API void SetColumnOffset(int column_index, float offset_x);
// set position of column line (in pixels, from the left side of the contents
...
//   This is often used with ImGuiDockNodeFlags_PassthruCentralNode to make it
//   transparent.
...
IMGUI_API ImGuiID DockSpace(ImGuiID id, const ImVec2 &size = ImVec2(0, 0),
...
IMGUI_API ImGuiID DockSpaceOverViewport(
    ...
    const ImGuiWindowClass *window_class = NULL);
IMGUI_API void
SetNextWindowDockID(ImGuiID dock_id,
                    ImGuiCond cond = 0); // set next window dock id
IMGUI_API void SetNextWindowClass(const ImGuiWindowClass *window_class);
// set next window class (control docking compatibility + provide hints to
// platform backend via custom viewport flags and platform parent/child
// relationship)
...
IMGUI_API bool SetDragDropPayload(const char *type, const void *data, size_t sz,
                                  ImGuiCond cond = 0);
// type is a user defined string of maximum 32 characters. Strings starting with
// '_' are reserved for dear imgui internal types. Data is copied and held by
// imgui. Return true when payload has been accepted.
IMGUI_API void EndDragDropSource(); // only call EndDragDropSource() if
                                    // BeginDragDropSource() returns true!
IMGUI_API bool BeginDragDropTarget();
// call after submitting an item that may receive a payload. If this returns
...
// peek directly into the current payload from anywhere. returns NULL when drag
// and drop is finished or inactive. use ImGuiPayload::IsDataType() to test for
// the payload type.
...
// buttons, selectable, treenode covering an area where subsequent items may
// need to be added. Note that both Selectable() and TreeNode() have dedicated
// flags doing this.
...
// equivalent to the behavior of e.g. Button(). Read comments in function
// definition.
...
// get background draw list for the given viewport. this draw list will be the
// first rendering one. Useful to quickly draw shapes/text behind dear imgui
// contents.
...
// last rendered one. Useful to quickly draw shapes/text over dear imgui
// contents.
...
// replace current window storage with our own (if you want to manipulate it
// yourself, typically clear subsection of it)
...
// was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl |
...
// [DEBUG] returns English name of the key. Those names a provided for debugging
...
// Override io.WantCaptureKeyboard flag next frame (said flag is left for your
...
// did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note
...
// [WILL OBSOLETE] is any mouse button held? This was designed for backends, but
...
// shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with
// other calls
...
// is mouse dragging? (if lock_threshold < -1.0f, uses
// io.MouseDraggingThreshold)
...
// moves past a distance threshold at least once (if lock_threshold < -1.0f,
// uses io.MouseDraggingThreshold)
IMGUI_API void ResetMouseDragDelta(ImGuiMouseButton button = 0); //
IMGUI_API ImGuiMouseCursor GetMouseCursor();
// get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this
// is updated during the frame. valid before Render(). If you use software
// rendering by setting io.MouseDrawCursor ImGui will render those for you
IMGUI_API void
SetMouseCursor(ImGuiMouseCursor cursor_type); // set desired mouse cursor shape
IMGUI_API void SetNextFrameWantCaptureMouse(bool want_capture_mouse);
// Override io.WantCaptureMouse flag next frame (said flag is left for your
// application to handle, typical when true it instucts your app to ignore
...
IMGUI_API void LoadIniSettingsFromMemory(const char *ini_data,
                                         size_t ini_size = 0);
// call after CreateContext() and before the first call to NewFrame() to provide
// .ini data from your own data source.
IMGUI_API void SaveIniSettingsToDisk(const char *ini_filename);
// this is automatically called (if io.IniFilename is not empty) a few seconds
// after any modification that should be reflected in the .ini file (and also by
// DestroyContext).
IMGUI_API const char *SaveIniSettingsToMemory(size_t *out_ini_size = NULL);
// return a zero-terminated string with the .ini data which you can save by your
...
GetPlatformIO(); // platform/renderer functions, for backend to setup +
                 // viewports list.
IMGUI_API void UpdatePlatformWindows();
// call in main loop. will call CreateWindow/ResizeWindow/etc. platform
// functions for each secondary viewport, and DestroyWindow for each inactive
// viewport.
IMGUI_API void RenderPlatformWindowsDefault(void *platform_render_arg = NULL,
                                            void *renderer_render_arg = NULL);
// call in main loop. will call RenderWindow/SwapBuffers platform functions for
...
// call DestroyWindow platform functions for all viewports. call from backend
// Shutdown() if you need to close platform windows before imgui shutdown.
// otherwise will be called by DestroyContext().
IMGUI_API ImGuiViewport *
FindViewportByID(ImGuiID id); // this is a helper for backends.
IMGUI_API ImGuiViewport *FindViewportByPlatformHandle(void *platform_handle);
// this is a helper for backends. the type platform_handle is decided by the
// backend (e.g. HWND, MyWindow*, GLFWwindow* etc.)
} // namespace ImGui
...
enum ImGuiWindowFlags_ {
  ...
  // specify width. Read code in imgui_demo in the "Horizontal Scrolling"
  // section.
  ...
  // navigation to cross over parent border to this child or between sibling
  // child windows.
...
// Obsoleted in 1.90: Use ImGuiChildFlags_AlwaysUseWindowPadding in BeginChild()
// call.
#endif
};
...
enum ImGuiChildFlags_ {
  ...
  // Show an outer border and enable WindowPadding. (IMPORTANT: this is always
  // == 1 == true for legacy reason)
  ...
  // Enable auto-resizing width. Read "IMPORTANT: Size measurement" details
  // above.
  ImGuiChildFlags_AutoResizeY = 1 << 5,
  // Enable auto-resizing height. Read "IMPORTANT: Size measurement" details
  // above.
  ImGuiChildFlags_AlwaysAutoResize = 1 << 6,
  // Combined with AutoResizeX/AutoResizeY. Always measure size even when child
  // is hidden, always return true, always disable clipping optimization! NOT
  // RECOMMENDED.
  ImGuiChildFlags_FrameStyle = 1 << 7,
  // Style the child window like a framed item: use FrameBg, FrameRounding,
  // FrameBorderSize, FramePadding instead of ChildBg, ChildRounding,
  // ChildBorderSize, WindowPadding.
};
...
enum ImGuiInputTextFlags_ {
  ...
  // Callback on each iteration. User code may query cursor position, modify
  // text buffer.
  ...
  // Disable undo/redo. Note that input text owns the text data while active, if
  // you want to provide your own undo/redo stack you need e.g. to call
  // ClearActiveID().
  ImGuiInputTextFlags_CharsScientific =
      1 << 17, // Allow 0123456789.+-*/eE (Scientific notation input)
  ImGuiInputTextFlags_CallbackResize = 1 << 18,
  // Callback on buffer capacity changes request (beyond 'buf_size' parameter
  ...
  // Escape key clears content if not empty, and deactivate otherwise (contrast
...
enum ImGuiTreeNodeFlags_ {
  ...
  // ImGuiTreeNodeFlags_OpenOnDoubleClick is also set, single-click arrow or
  ...
  // Extend hit box to the left-most and right-most edges (cover the indent
  // area).
  ...
  // (WIP) Nav: left direction may move to this TreeNode() from any of its child
  // (items submitted between TreeNode and TreePop)
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 16,  // FIXME: TODO: Disable
...
enum ImGuiPopupFlags_ {
  ...
  // For BeginPopupContext*(): open on Left Mouse release. Guaranteed to always
  // be == 0 (same as ImGuiMouseButton_Left)
  ImGuiPopupFlags_MouseButtonRight = 1,
  // For BeginPopupContext*(): open on Right Mouse release. Guaranteed to always
  ...
  // For OpenPopup*(), BeginPopupContext*(): don't reopen same popup if already
  ...
  // For BeginPopupContextWindow(): don't return true when hovering items, only
  ...
  // For IsPopupOpen(): search/test at any level of the popup stack (default
  // test in the current level)
...
enum ImGuiComboFlags_ {
  ...
  // size you can use SetNextWindowSizeConstraints() prior to calling
  // BeginCombo()
...
enum ImGuiTabBarFlags_ {
  ...
  // Disable behavior of closing tabs (that are submitted with p_open != NULL)
...
enum ImGuiTabItemFlags_ {
  ...
  // Enforce the tab position to the left of the tab bar (after the tab list
  // popup button)
  ...
  // (will wait for user to stop submitting the tab). Otherwise closure is
  // assumed when pressing the X, so if you keep submitting the tab may reappear
  // at end of tab bar.
};
...
enum ImGuiHoveredFlags_ {
  ...
  // IsWindowHovered() only: Return true if any children of the window is
  // hovered
  ImGuiHoveredFlags_RootWindow = 1 << 1,
  // IsWindowHovered() only: Test from root window (top most parent of the
  // current hierarchy)
  ...
  // IsItemHovered() only: Return true even if the item uses AllowOverlap mode
  // and is overlapped by another hoverable item.
  ...
  // IsItemHovered() only: Disable using gamepad/keyboard navigation state when
  // active, always query mouse
  ...
  // Shortcut for standard flags when using IsItemHovered() + SetTooltip()
  // sequence.
  ...
  // IsItemHovered() only: Return true immediately (default). As this is the
  ...
  // (~0.15 sec) (shared between items) + requires mouse to be stationary for
  ...
  // (~0.40 sec) (shared between items) + requires mouse to be stationary for
  // style.HoverStationaryDelay (once per item).
  ImGuiHoveredFlags_NoSharedDelay = 1 << 17,
  // IsItemHovered() only: Disable shared delay system where moving from one
  // item to the next keeps the previous timer for a short time (standard for
  // tooltips with long delays)
};

// Flags for ImGui::DockSpace(), shared/inherited by child nodes.
// (Some flags can be applied to individual nodes directly)
// FIXME-DOCK: Also see ImGuiDockNodeFlagsPrivate_ which may involve using the
// WIP and internal DockBuilder api.
enum ImGuiDockNodeFlags_ {
  ImGuiDockNodeFlags_None = 0,
  ImGuiDockNodeFlags_KeepAliveOnly = 1 << 0,
  //       // Don't display the dockspace node but keep it alive. Windows docked
  //       into this dockspace node won't be undocked.
  ...
  //       // Disable docking over the Central Node, which will be always kept
  //       empty.
  ...
  //       // Tab bar will automatically hide when there is a single window in
  //       the dock node.
  ImGuiDockNodeFlags_NoUndocking =
      1 << 7, //       // Disable undocking this node.

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiDockNodeFlags_NoSplit =
      ImGuiDockNodeFlags_NoDockingSplit, // Renamed in 1.90
  ImGuiDockNodeFlags_NoDockingInCentralNode =
      ImGuiDockNodeFlags_NoDockingOverCentralNode, // Renamed in 1.90
#endif
};
...
enum ImGuiDragDropFlags_ {
  ...
  // false, to avoid subsequent user code submitting tooltips. This flag
  // disables this behavior so you can still call IsItemHovered() on the source
  // item.
  ImGuiDragDropFlags_SourceNoHoldToOpenOthers = 1 << 2,
  // Disable the behavior that allows to open tree nodes and collapsing header
  // by holding over them while dragging a source item.
  ImGuiDragDropFlags_SourceAllowNullID = 1 << 3,
  // Allow items such as Text(), Image() that have no unique identifier to be
  // used as drag source, by manufacturing a temporary identifier based on their
...
enum ImGuiKey : int {
  ...
  // X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu. Hold:
  // Windowing mode (Focus/Move/Resize windows)
  ...
  // D-pad Left                                       // Move / Tweak / Resize
  // Window (in Windowing mode)
  ImGuiKey_GamepadDpadRight,
  // D-pad Right                                      // Move / Tweak / Resize
  // Window (in Windowing mode)
  ImGuiKey_GamepadDpadUp,
  // D-pad Up                                         // Move / Tweak / Resize
  // Window (in Windowing mode)
  ImGuiKey_GamepadDpadDown,
  // D-pad Down                                       // Move / Tweak / Resize
  // Window (in Windowing mode)
  ImGuiKey_GamepadL1,
  // L Bumper (Xbox)  L (Switch)   L1 (PS)            // Tweak Slower / Focus
  // Previous (in Windowing mode)
  ImGuiKey_GamepadR1,
  // R Bumper (Xbox)  R (Switch)   R1 (PS)            // Tweak Faster / Focus
...
enum ImGuiConfigFlags_ {
  ...
  // Instruct navigation to move the mouse cursor. May be useful on TV/console
  // systems where moving a virtual mouse is awkward. Will update io.MousePos
  // and set io.WantSetMousePos=true. If enabled you MUST honor
  // io.WantSetMousePos requests in your backend, otherwise ImGui will react as
  ...
  // Instruct imgui to clear mouse position/buttons in NewFrame(). This allows
  // ignoring the mouse information set by the backend.
  ImGuiConfigFlags_NoMouseCursorChange = 1 << 5,
  // Instruct backend to not alter mouse cursor shape and visibility. Use if the
  // backend cursor changes are interfering with yours and you don't want to use
  // SetMouseCursor() to change mouse cursor. You may want to honor requests
  // from imgui by reading GetMouseCursor() yourself instead.
  ...
  // ImGuiCol_WindowBg is opaque (Alpha=1.0) so transition to a viewport won't
  // be noticeable.
  ImGuiConfigFlags_ViewportsEnable = 1 << 10,
  // Viewport enable flags (require both ImGuiBackendFlags_PlatformHasViewports
  // + ImGuiBackendFlags_RendererHasViewports set by the respective backends)
  ImGuiConfigFlags_DpiEnableScaleViewports = 1 << 14,
  // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui windows when the
  // DpiScale of a viewport changed (mostly useful for the main viewport hosting
  // other window). Note that resizing the main window itself is up to your
  // application.
  ImGuiConfigFlags_DpiEnableScaleFonts = 1 << 15,
  // [BETA: Don't use] FIXME-DPI: Request bitmap-scaled fonts to match DpiScale.
  // This is a very low-quality workaround. The correct way to handle DPI is
  // _currently_ to replace the atlas and/or fonts in the
  // Platform_OnChangedViewport callback, but this is all early work in
  // progress.
...
enum ImGuiBackendFlags_ {
  ...
  // Backend Platform supports honoring GetMouseCursor() value to change the OS
  // cursor shape.
  ImGuiBackendFlags_HasSetMousePos = 1 << 2,
  // Backend Platform supports io.WantSetMousePos requests to reposition the OS
  ...
  // ImGuiViewportFlags_NoInputs flag (Win32 backend, GLFW 3.30+ backend can do
  // this, SDL backend cannot). If this cannot be done, Dear ImGui needs to use
  // a flawed heuristic to find the viewport under.
...
enum ImGuiStyleVar_ {
  ...
  ImGuiStyleVar_TableAngledHeadersTextAlign, // ImVec2
...
enum ImGuiColorEditFlags_ {
  ...
  //              // ColorEdit, ColorPicker: disable display of inline text
  //              label (the label is still forwarded to the tooltip and
  //              picker).
  ...
  //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient in
  //              picker.
  ...
  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values
  // formatted as 0..255.
  ImGuiColorEditFlags_Float = 1 << 24,
  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values
  // formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of
  // value via integers.
  ...
  // [Input]      // ColorEdit, ColorPicker: input and output data in RGB
  // format.
  ImGuiColorEditFlags_InputHSV = 1 << 28,
  // [Input]      // ColorEdit, ColorPicker: input and output data in HSV
  // format.
...
enum ImGuiSliderFlags_ {
  ...
  // ImGuiSliderFlags_NoRoundToFormat with this if using a format-string with
  ...
  // [Internal] We treat using those bits as being potentially a 'float power'
...
enum ImGuiTableFlags_ {
  ...
  // default it is available in TableHeadersRow(). Decorations
  ...
  // [ALPHA] Disable vertical borders in columns Body (borders will always
  // appear in Headers). -> May move to style
  ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12,
  // [ALPHA] Disable vertical borders in columns Body until hovered for resize
  // (borders will always appear in Headers). -> May move to style Sizing Policy
  ...
  // Columns default to _WidthFixed or _WidthAuto (if resizable or not
  // resizable), matching the maximum contents width of all columns. Implicitly
  // enable ImGuiTableFlags_NoKeepColumnsVisible.
  ...
  // available when ScrollX/ScrollY are disabled and Stretch columns are not
  // used.
  ImGuiTableFlags_NoHostExtendY = 1 << 17,
  // Make outer height stop exactly at outer_size.y (prevent auto-extending
  // table past the limit). Only available when ScrollX/ScrollY are disabled.
  ...
  // Default if BordersOuterV is on. Enable outermost padding. Generally
  ...
  // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable()
  // to specify the container size. Changes default sizing policy. Because this
  // creates a child window, ScrollY is currently generally recommended when
  // using ScrollX.
  ImGuiTableFlags_ScrollY = 1 << 25,
  // Enable vertical scrolling. Require 'outer_size' parameter of BeginTable()
  ...
  // Allow no sorting, disable default sorting. TableGetSortSpecs() may return
...
enum ImGuiTableColumnFlags_ {
  ...
  // (unlike calling TableSetColumnEnabled() which manipulates the user
  // accessible state)
  ...
  // Disable ability to sort on this field (even if ImGuiTableFlags_Sortable is
  // set on the table).
  ...
  // TableHeadersRow() will submit an angled header row for this column. Note
  ...
  // [Internal] Disable user resizing this column directly (it may however we
  // resized indirectly from its left edge)
};
...
enum ImGuiTableBgTarget_ {
  ...
  // Set row background color 0 (generally used for background, automatically
  // set when ImGuiTableFlags_RowBg is used)
...
struct ImGuiTableSortSpecs {
  ...
  int SpecsCount;
  // Sort spec count. Most often 1. May be > 1 when ImGuiTableFlags_SortMulti is
  // enabled. May be == 0 when ImGuiTableFlags_SortTristate is enabled.
  bool SpecsDirty;
  // Set to true when specs have changed since last time! Use this to sort
  // again, then clear the flag.
...
struct ImGuiTableColumnSortSpecs {
  ...
  ImS16 SortOrder;
  // Index within parent ImGuiTableSortSpecs (always stored in order starting
  // from 0, tables sorted on a single criteria will always have a 0 here)
  ImGuiSortDirection SortDirection
      : 8; // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending
...
struct ImGuiStyle {
  ...
  // Thickness of border around windows. Generally set to 0.0f or 1.0f. (Other
  ...
  // Thickness of border around frames. Generally set to 0.0f or 1.0f. (Other
  // values are not well tested and more CPU/GPU costly).
  ...
  // Expand reactive bounding box for touch-based system where touch position is
  // not accurate enough. Unfortunately we don't sort widgets so priority on
  // overlap will always be given to the first widget. So don't grow this too
  // much!
  float IndentSpacing;
  ...
  // Set to 0.0f to always show when hovering, set to FLT_MAX to never show
  // close button unless selected.
  ...
  // +50.0f degrees).
  ImVec2 TableAngledHeadersTextAlign; // Alignment of angled headers within the
                                      // cell
  ImGuiDir ColorButtonPosition;
  // Side of the color button in the ColorEdit4 widget (left/right). Defaults to
  // ImGuiDir_Right.
  ...
  // If you cannot see the edges of your screen (e.g. on a TV) increase the safe
  // area padding. Apply to popups/tooltips as well regular windows. NB: Prefer
  ...
  // We apply per-monitor DPI scaling over this scale. May be removed later.
  ...
  // backend to render with bilinear filtering (NOT point/nearest filtering).
  ...
  // circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the
  // beginning of the frame (copied to ImDrawList).
  float CurveTessellationTol;
  // Tessellation tolerance when using PathBezierCurveTo() without a specific
  // number of segments. Decrease for highly tessellated curves (higher quality,
  // more polygons), increase to reduce quality.
  float CircleTessellationMaxError;
  // Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled()
  ...
  // Delay for IsItemHovered(ImGuiHoveredFlags_DelayShort). Usually used along
  // with HoverStationaryDelay.
...
// file for general usage. It is generally expected that:
// - initialization: backends and user code writes to ImGuiIO.
// - main loop: backends writes to ImGuiIO, user code and imgui code reads from
// ImGuiIO.
//-----------------------------------------------------------------------------
...
struct ImGuiIO {
  ...
  // = 0              // See ImGuiConfigFlags_ enum. Set by user/application.
  // Gamepad/keyboard navigation options, etc.
  ImGuiBackendFlags BackendFlags;
  // = 0              // See ImGuiBackendFlags_ enum. Set by backend
  // (imgui_impl_xxx files or custom backend) to communicate features supported
  // by the backend.
  ImVec2 DisplaySize;
  // <unset>          // Main display size, in pixels (generally ==
  ...
  // = "imgui.ini"    // Path to .ini file (important: default "imgui.ini" is
  // relative to current working dir!). Set NULL to disable automatic .ini
  // loading/saving or if you want to manually call LoadIniSettingsXXX() /
  // SaveIniSettingsXXX() functions.
  const char *LogFilename;
  // = "imgui_log.txt"// Path to .log file (default parameter to
  // ImGui::LogToFile when no file is specified).
  void *UserData; // = NULL           // Store your own data.

  ImFontAtlas *Fonts;
  // <auto>           // Font atlas: load, rasterize and pack one or more fonts
  ...
  // = (1, 1)         // For retina display or other situations where window
  // coordinates are different from framebuffer coordinates. This generally ends
  // up in ImDrawData::FramebufferScale.

  // Docking options (when ImGuiConfigFlags_DockingEnable is set)
  bool ConfigDockingNoSplit;
  // = false          // Simplified docking mode: disable window splitting, so
  // docking is limited to merging multiple windows together into tab-bars.
  bool ConfigDockingWithShift;
  // = false          // Enable docking with holding Shift key (reduce visual
  // noise, allows dropping in wider space)
  bool ConfigDockingAlwaysTabBar;
  // = false          // [BETA] [FIXME: This currently creates regression with
  // auto-sizing and general overhead] Make every single floating window display
  // within a docking node.
  bool ConfigDockingTransparentPayload;
  // = false          // [BETA] Make window or viewport transparent when docking
  ...
  // = false;         // Set to make all floating imgui windows always create
  // their own viewport. Otherwise, they are merged into the main host viewports
  // when overlapping it. May also set ImGuiViewportFlags_NoAutoMerge on
  // individual viewport.
  bool ConfigViewportsNoTaskBarIcon;
  // = false          // Disable default OS task bar icon flag for secondary
  // viewports. When a viewport doesn't want a task bar icon,
  // ImGuiViewportFlags_NoTaskBarIcon will be set on it.
  bool ConfigViewportsNoDecoration;
  // = true           // Disable default OS window decoration flag for secondary
  ...
  // = false          // Disable default OS parenting to main viewport for
  ...
  // = false          // Request ImGui to draw a mouse cursor for you (if you
  // are on a platform without a mouse cursor). Cannot be easily renamed to
  // 'io.ConfigXXX' because this is frequently used by backend implementations.
  bool ConfigMacOSXBehaviors;
  // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt
  // instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start
  // and End using Cmd+Arrows instead of Home/End, Double click selects by word
  // instead of selecting whole text, Multi-selection in lists uses Cmd/Super
  // instead of Ctrl.
  bool ConfigInputTrickleEventQueue;
  // = true           // Enable input queue trickling: some types of events
  ...
  // = true           // Enable blinking cursor (optional as some users consider
  // it to be distracting).
  bool ConfigInputTextEnterKeepActive;
  // = false          // [BETA] Pressing Enter will keep item active and select
  // contents (single-line only).
  bool ConfigDragClickToInputText;
  // = false          // [BETA] Enable turning DragXXX widgets into text input
  // with a simple mouse click-release (without moving). Not desirable on
  // devices without a keyboard.
  bool ConfigWindowsResizeFromEdges;
  // = true           // Enable resizing of windows from their edges and from
  // the lower-left corner. This requires (io.BackendFlags &
  ...
  // = false       // Enable allowing to move windows only when clicking on
  // their title bar. Does not apply to windows without a title bar.
  float ConfigMemoryCompactTimer;
  // = 60.0f          // Timer (in seconds) to free transient windows/tables
  // memory buffers when unused. Set to -1.0f to disable.
  ...
  // = 6.0f           // Distance threshold to stay in to validate a
  // double-click, in pixels.
  float MouseDragThreshold; // = 6.0f           // Distance threshold before
                            // considering we are dragging.
  float KeyRepeatDelay;
  // = 0.275f         // When holding a key/button, time before it starts
  ...
  // = false          // First-time calls to Begin()/BeginChild() will return
  ...
  // = false          // Ignore io.AddFocusEvent(false), consequently not
  // calling io.ClearInputKeys() in input processing.

  // Option to audit .ini data
  bool ConfigDebugIniSettings;
  // = false          // Save .ini data with extra comments (particularly
  ...
  // '.'              // [Experimental] Configure decimal point e.g. '.' or ','
  // useful for some languages (e.g. German), generally pulled from
  ...
  // Queue a new key down/up event. Key should be "translated" (as in, generally
  ...
  // Queue a mouse wheel update. wheel_y<0: scroll down, wheel_y>0: scroll up,
  ...
  // Queue a mouse hovered viewport. Requires backend to set
  // ImGuiBackendFlags_HasMouseHoveredViewport to call this (for multi-viewport
  // support).
  ...
  IMGUI_API void SetKeyEventNativeData(ImGuiKey key, int native_keycode,
                                       int native_scancode,
                                       int native_legacy_index = -1);
  // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native
  ...
  // Useful if you have native dialog boxes that are interrupting your
  // application loop/refresh, and you want to disable events being queued while
  // your app is frozen.
  IMGUI_API void ClearEventsQueue(); // Clear all incoming events.
  IMGUI_API void ClearInputKeys();
  // Clear current keyboard/mouse/gamepad state + current frame text input
  // buffer. Equivalent to releasing all keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  IMGUI_API void ClearInputCharacters();
  // [Obsoleted in 1.89.8] Clear the current frame text input buffer. Now
  ...
  // Set when Dear ImGui will use mouse inputs, in this case do not dispatch
  // them to your main game/application (either way, always pass on mouse inputs
  // to imgui). (e.g. unclicked mouse is hovering over an imgui window, widget
  // is active, mouse was clicked over an imgui window, etc.).
  bool WantCaptureKeyboard;
  // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch
  // them to your main game/application (either way, always pass keyboard inputs
  // to imgui). (e.g. InputText active, or an imgui window is focused and
  // navigation is enabled, etc.).
  bool WantTextInput;
  // Mobile/console: when set, you may display an on-screen keyboard. This is
  // set by Dear ImGui when it wants textual keyboard input to happen (e.g. when
  // a InputText widget is active).
  bool WantSetMousePos;
  // MousePos has been altered, backend should reposition mouse on next frame.
  // Rarely used! Set only when ImGuiConfigFlags_NavEnableSetMousePos flag is
  // enabled.
  bool WantSaveIniSettings;
  // When manual .ini load/save is active (io.IniFilename == NULL), this will be
  // set to notify your application that you can call SaveIniSettingsToMemory()
  // and save yourself. Important: clear io.WantSaveIniSettings yourself after
  // saving!
  bool NavActive;
  // Keyboard/Gamepad navigation is currently allowed (will handle
  // ImGuiKey_NavXXX events) = a window is focused and it doesn't use the
  ...
  // io.DeltaTime), in frame per second. Solely for convenience. Slow
  // applications may not want to use a moving average or may want to reset
  // underlying buffers occasionally.
  ...
  // moving anywhere)
  ImVec2 MousePos;
  // Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is
  // unavailable (on another screen, etc.)
  bool MouseDown[5];
  // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT
  ...
  // (Optional) Modify using io.AddMouseViewportEvent(). With multi-viewports:
  // viewport the OS mouse is hovering. If possible _IGNORING_ viewports with
  // the ImGuiViewportFlags_NoInputs flag is much better (few backends can
  // handle that). Set io.BackendFlags |=
  // ImGuiBackendFlags_HasMouseHoveredViewport if you can provide this info. If
  // you don't imgui will infer the value using the rectangles and last focused
  ...
  // Key mods flags (any of
  // ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags, same as
  // io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. DOES NOT
  // CONTAINS ImGuiMod_Shortcut which is pretranslated). Read-only, updated by
  // NewFrame()
  ...
  // == 0 (not clicked), == 1 (same as MouseClicked[]), == 2 (double-clicked),
  // == 3 (triple-clicked) etc. when going from !Down to Down
  ImU16 MouseClickedLastCount[5];
  // Count successive number of clicks. Stays valid after mouse release. Reset
  // after another click is done.
  bool MouseReleased[5]; // Mouse button went from Down to !Down
  bool MouseDownOwned[5];
  // Track if button was clicked inside a dear imgui window or over void blocked
  // by a popup. We don't request mouse capture from the application if click
  // started outside ImGui bounds.
  bool MouseDownOwnedUnlessPopupClose[5]; // Track if button was clicked inside
                                          // a dear imgui window.
  bool MouseWheelRequestAxisSwap;
  // On a non-Mac system, holding SHIFT requests WheelY to perform the
  // equivalent of a WheelX event. On a Mac system this is already enforced by
  // the system.
  ...
  // Maximum distance, absolute, on each axis, of how much mouse has traveled
  // from the clicking point
  float MouseDragMaxDistanceSqr[5];
  // Squared maximum distance of how much mouse has traveled from the clicking
  // point (used for moving thresholds)
  float PenPressure;
  // Touch/Pen pressure (0.0f to 1.0f, should be >0.0f only when MouseDown[0] ==
  ...
  // Queue of _characters_ input (obtained by platform backend). Fill using
  ...
  // [LEGACY] Input: map of indices into the KeysDown[512] entries array which
  // represent your "native" keyboard state. The first 512 are now unused and
  // should be kept zero. Legacy backend will write into KeyMap[] using
  // ImGuiKey_ indices which are always >512.
  bool KeysDown[ImGuiKey_COUNT];
  // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the
  // "native" order your engine has access to keyboard keys, so you can use your
  // own defines/enums for keys). This used to be [512] sized. It is now
  // ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work
  // without an overflow.
  float NavInputs[ImGuiNavInput_COUNT];
  // [LEGACY] Since 1.88, NavInputs[] was removed. Backends from 1.60 to 1.86
  // won't build. Feed gamepad inputs via io.AddKeyEvent() and
...
struct ImGuiInputTextCallbackData {
  ...
  // - However, during Completion/History/Always callback, Buf always points to
  // our own internal data (it is not the same as your buffer)! Changes to it
  // will be reflected into your own buffer shortly after the callback.
  ...
  ImWchar EventChar;
  // Character input                      // Read-write   // [CharFilter]
  // Replace character with another one, or set to zero to drop. return 1 is
  // equivalent to setting EventChar=0;
  ImGuiKey EventKey; // Key pressed (Up/Down/TAB)            // Read-only    //
                     // [Completion,History]
  char *Buf;
  // Text buffer                          // Read-write   // [Resize] Can
  // replace pointer / [Completion,History,Always] Only write to pointed data,
  // don't replace the actual pointer!
  int BufTextLen;
  // Text length (in bytes)               // Read-write   //
  // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C
  // land: == strlen(some_text), in C++ land: string.length()
  int BufSize;
  // Buffer size (in bytes) = capacity+1  // Read-only    //
  // [Resize,Completion,History,Always] Include zero-terminator storage. In C
  // land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1
...
struct ImGuiSizeCallbackData {
  void *UserData;
  // Read-only.   What user passed to SetNextWindowSizeConstraints(). Generally
  // store an integer or float in here (need reinterpret_cast<>).
  ImVec2 Pos;         // Read-only.   Window position, for reference.
  ImVec2 CurrentSize; // Read-only.   Current window size.
  ImVec2 DesiredSize;
  // Read-write.  Desired size, based on user's mouse position. Write to this
...
struct ImGuiWindowClass {
  ImGuiID ClassId;
  // User data. 0 = Default class (unclassed). Windows of different classes
  // cannot be docked with each others.
  ImGuiID ParentViewportId;
  // Hint for the platform backend. -1: use default. 0: request platform backend
  // to not parent the platform. != 0: request platform backend to create a
  // parent<>child relationship between the platform windows. Not conforming
  // backends are free to e.g. parent every viewport to the main viewport or
  // not.
  ImGuiID FocusRouteParentWindowId;
  // ID of parent window for shortcut focus route evaluation, e.g. Shortcut()
  // call from Parent Window will succeed when this window is focused.
  ImGuiViewportFlags ViewportFlagsOverrideSet;
  // Viewport flags to set when a window of this class owns a viewport. This
  // allows you to enforce OS decoration or task bar icon, override the defaults
  // on a per-window basis.
  ImGuiViewportFlags ViewportFlagsOverrideClear;
  // Viewport flags to clear when a window of this class owns a viewport. This
  // allows you to enforce OS decoration or task bar icon, override the defaults
  // on a per-window basis.
  ImGuiTabItemFlags TabItemFlagsOverrideSet;
  // [EXPERIMENTAL] TabItem flags to set when a window of this class gets
  // submitted into a dock node tab bar. May use with ImGuiTabItemFlags_Leading
  // or ImGuiTabItemFlags_Trailing.
  ImGuiDockNodeFlags DockNodeFlagsOverrideSet;
  // [EXPERIMENTAL] Dock node flags to set when a window of this class is hosted
  // by a dock node (it doesn't have to be selected!)
  bool DockingAlwaysTabBar;
  // Set to true to enforce single floating windows of this class always having
  // their own docking node (equivalent of setting the global
  // io.ConfigDockingAlwaysTabBar)
  bool DockingAllowUnclassed;
  // Set to true to allow windows of this class to be docked/merged with an
  // unclassed window. // FIXME-DOCK: Move to DockNodeFlags override?
...
struct ImGuiPayload {
  ...
  char DataType[32 + 1];  // Data type tag (short user-supplied string, 32
                          // characters max)
  bool Preview;
  // Set when AcceptDragDropPayload() was called and mouse has been hovering the
...
struct ImGuiListClipper {
  ...
  IMGUI_API bool Step();
  // Call until it returns false. The DisplayStart/DisplayEnd fields will be set
  // and you can process/draw those items.
...
struct ImDrawCmd {
  ImVec4 ClipRect;
  // 4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract
  // ImDrawData->DisplayPos to get clipping rectangle in "viewport" coordinates
  ImTextureID TextureId;
  // 4-8  // User-provided texture ID. Set by user in ImfontAtlas::SetTexID()
  // for fonts or passed to Image*() functions. Ignore if never using images or
  // multiple fonts atlas.
  unsigned int VtxOffset;
  // 4    // Start offset in vertex buffer.
  // ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to
  // support meshes larger than 64K vertices with 16-bit indices.
  unsigned int IdxOffset; // 4    // Start offset in index buffer.
  unsigned int ElemCount;
  // 4    // Number of indices (multiple of 3) to be rendered as triangles.
  // Vertices are stored in the callee ImDrawList's vtx_buffer[] array, indices
  // in idx_buffer[].
...
enum ImDrawFlags_ {
  ...
  // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right corner
  // only (when rounding > 0.0f, we default to all corners). Was 0x02.
  ImDrawFlags_RoundCornersBottomLeft = 1 << 6,
  // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-left corner
  // only (when rounding > 0.0f, we default to all corners). Was 0x04.
  ImDrawFlags_RoundCornersBottomRight = 1 << 7,
  // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-right corner
...
enum ImDrawListFlags_ {
  ...
  // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f wide
  // line or lines thin enough to be drawn using textures, otherwise *3 the
  // number of triangles)
  ImDrawListFlags_AntiAliasedLinesUseTex = 1 << 1,
  // Enable anti-aliased lines/borders using textures when possible. Require
  // backend to render with bilinear filtering (NOT point/nearest filtering).
  ImDrawListFlags_AntiAliasedFill = 1 << 2,
  // Enable anti-aliased edge around filled shapes (rounded rectangles,
  // circles).
...
struct ImDrawList {
  ...
  // Draw commands. Typically 1 command = 1 GPU draw call, unless the command is
  // a callback.
  ...
  // [Internal] generally == VtxBuffer.Size unless we are past 64K vertices, in
  // which case this gets reset to 0.
  ImDrawListSharedData *_Data;
  // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to
  // get the one from current ImGui context)
  ImDrawVert *_VtxWritePtr;
  // [Internal] point within VtxBuffer.Data after each add command (to avoid
  // using the ImVector<> operators too much)
  ImDrawIdx *_IdxWritePtr;
  // [Internal] point within IdxBuffer.Data after each add command (to avoid
  // using the ImVector<> operators too much)
  ImVector<ImVec2> _Path; // [Internal] current path building
  ImDrawCmdHeader _CmdHeader;
  // [Internal] template of active commands. Fields should match those of
  // CmdBuffer.back().
  ImDrawListSplitter _Splitter;
  // [Internal] for channels api (note: prefer using your own persistent
  // instance of ImDrawListSplitter!)
  ImVector<ImVec4> _ClipRectStack;       // [Internal]
  ImVector<ImTextureID> _TextureIdStack; // [Internal]
  float _FringeScale;
  // [Internal] anti-alias fringe is scaled by this value, this helps to keep
  ...
  // Render-level scissoring. This is passed down to your render function but
  ...
  IMGUI_API void AddNgonFilled(const ImVec2 &center, float radius, ImU32 col,
                               int num_segments);
  IMGUI_API void AddEllipse(const ImVec2 &center, const ImVec2 &radius,
                            ImU32 col, float rot = 0.0f, int num_segments = 0,
                            float thickness = 1.0f);
  IMGUI_API void AddEllipseFilled(const ImVec2 &center, const ImVec2 &radius,
                                  ImU32 col, float rot = 0.0f,
                                  int num_segments = 0);
  ...
  // General polygon
  // - Only simple polygons are supported by filling functions (no
  // self-intersections, no holes).
  // - Concave polygon fill is more expensive than convex one: it has O(N^2)
  // complexity. Provided as a convenience fo user but not used by main library.
  ...
  IMGUI_API void AddConvexPolyFilled(const ImVec2 *points, int num_points,
                                     ImU32 col);
  IMGUI_API void AddConcavePolyFilled(const ImVec2 *points, int num_points,
                                      ImU32 col);
  ...
  // PathStroke()
  // - Important: filled shapes must always use clockwise winding order! The
  // anti-aliasing fringe depends on it. Counter-clockwise shapes will have
  // "inward" anti-aliasing.
  //   so e.g. 'PathArcTo(center, radius, PI * -0.5f, PI)' is ok, whereas
  ...
  IMGUI_API void PathArcToFast(const ImVec2 &center, float radius,
                               int a_min_of_12, int a_max_of_12);
  // Use precomputed angles for a 12 steps circle
  IMGUI_API void PathEllipticalArcTo(const ImVec2 &center, const ImVec2 &radius,
                                     float rot, float a_min, float a_max,
                                     int num_segments = 0); // Ellipse
  IMGUI_API void PathBezierCubicCurveTo(const ImVec2 &p2, const ImVec2 &p3,
                                        const ImVec2 &p4, int num_segments = 0);
  ...
  // Your rendering function must check for 'UserCallback' in ImDrawCmd and call
  // the function instead of rendering triangles.
  IMGUI_API void AddDrawCmd();
  // This is useful if you need to forcefully create a new draw call (to allow
  // for dependent rendering / blending). Otherwise primitives are merged into
  // the same draw-call as much as possible
  ...
  // radius_y, ImU32 col, float rot = 0.0f, int num_segments = 0, float
  // thickness = 1.0f) { AddEllipse(center, ImVec2(radius_x, radius_y), col,
  // rot, num_segments, thickness); } // OBSOLETED in 1.90.5 (Mar 2024) inline
  // void  AddEllipseFilled(const ImVec2& center, float radius_x, float
  // radius_y, ImU32 col, float rot = 0.0f, int num_segments = 0) {
  // AddEllipseFilled(center, ImVec2(radius_x, radius_y), col, rot,
  // num_segments); }                        // OBSOLETED in 1.90.5 (Mar 2024)
  // inline  void  PathEllipticalArcTo(const ImVec2& center, float radius_x,
  // float radius_y, float rot, float a_min, float a_max, int num_segments = 0)
  // { PathEllipticalArcTo(center, ImVec2(radius_x, radius_y), rot, a_min,
  // a_max, num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void
  // AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const
  // ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) {
...
struct ImDrawData {
  ...
  // Array of ImDrawList* to render. The ImDrawLists are owned by ImGuiContext
  // and only pointed to from here.
  ImVec2 DisplayPos;
  // Top-left position of the viewport to render (== top-left of the orthogonal
  // projection matrix to use) (== GetMainViewport()->Pos for the main viewport,
  // == (0.0) in most single-viewport applications)
  ImVec2 DisplaySize;
  // Size of the viewport to render (== GetMainViewport()->Size for the main
  // viewport, == io.DisplaySize in most single-viewport applications)
  ImVec2 FramebufferScale;
  // Amount of pixels for each unit of DisplaySize. Based on
  // io.DisplayFramebufferScale. Generally (1,1) on normal display, (2,2) on OSX
  // with Retina display.
  ImGuiViewport *OwnerViewport;
  // Viewport carrying the ImDrawData instance, might be of use to the renderer
  // (generally not).
  ...
  // Helper to convert all buffers from indexed to non-indexed, in case you
  ...
  // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final
...
struct ImFontConfig {
  ...
  // true     // TTF/OTF data ownership taken by the container ImFontAtlas (will
  // delete memory itself).
  int FontNo;       // 0        // Index of font within TTF/OTF file
  float SizePixels; //          // Size in pixels for rasterizer (more or less
                    //          maps to the resulting font height).
  int OversampleH;
  // 2        // Rasterize at higher quality for sub-pixel positioning. Note the
  // difference between 2 and 3 is minimal. You can reduce this to 1 for large
  // glyphs save memory. Read
  // https://github.com/nothings/stb/blob/master/tests/oversample/README.md for
  // details.
  int OversampleV;
  // 1        // Rasterize at higher quality for sub-pixel positioning. This is
  // not really useful as we don't use sub-pixel positions on the Y axis.
  bool PixelSnapH;
  // false    // Align every glyph to pixel boundary. Useful e.g. if you are
  ...
  // Pointer to a user-provided list of Unicode range (2 value per range, values
  // are inclusive, zero-terminated list).
  float GlyphMinAdvanceX;
  // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set
  // both Min/Max to enforce mono-space font
  float GlyphMaxAdvanceX; // FLT_MAX  // Maximum AdvanceX for glyphs
  bool MergeMode;
  // false    // Merge into previous ImFont, so you can combine multiple inputs
  ...
  // 0        // Settings for custom font builder. THIS IS BUILDER
  ...
  // 1.0f     // DPI scale for rasterization, not altering other font metrics:
  // make it easy to swap between e.g. a 100% and a 400% fonts for a zooming
  // display. IMPORTANT: If you increase this it is expected that you increase
  // font scale accordingly, otherwise quality may look lowered.
...
struct ImFontGlyph {
  ...
  // Flag to indicate glyph is colored and should generally ignore tinting (make
  // it usable with no shift on little-endian as this is used in loops)
  unsigned int Visible : 1;
  // Flag to indicate glyph has no visible pixels (e.g. space). Allow early out
  // when rendering.
...
struct ImFontGlyphRangesBuilder {
  ...
  // Add ranges, e.g. builder.AddRanges(ImFontAtlas::GetGlyphRangesDefault()) to
  // force add all of ASCII/Latin+Ext
  IMGUI_API void
...
enum ImFontAtlasFlags_ {
  ...
  // Don't build software mouse cursors into the atlas (save a little texture
  // memory)
  ...
  // AntiAliasedLinesUseTex features uses them, otherwise they will be rendered
  // using polygons (more expensive for CPU/GPU).
};
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont *AddFontFromMemoryTTF(void *font_data, int font_data_size,
                                         float size_pixels,
                                         const ImFontConfig *font_cfg = NULL,
                                         const ImWchar *glyph_ranges = NULL);
  // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted
  ...
  // 'compressed_font_data_base85' still owned by caller. Compress with
  // binary_to_compressed_c.cpp with -base85 parameter.
  IMGUI_API void ClearInputData();
  // Clear input data (all ImFontConfig structures including sizes, TTF data,
  // glyph ranges, etc.) = all the data used to build the texture and fonts.
  IMGUI_API void ClearTexData();
  // Clear output texture data (CPU side). Saves RAM once the texture has been
  ...
  // Default + Half-Width + Japanese Hiragana/Katakana + full set of about 21000
  // CJK Unified Ideographs
  IMGUI_API const ImWchar *GetGlyphRangesChineseSimplifiedCommon();
  // Default + Half-Width + Japanese Hiragana/Katakana + set of 2500 CJK Unified
  ...
  ImTextureID TexID;
  // User data to refer to the texture once it has been uploaded to user's
  // graphic systems. It is passed back to you during rendering via the
  // ImDrawCmd structure.
  int TexDesiredWidth;
  // Texture width desired by user before Build(). Must be a power-of-two. If
  // have many glyphs your graphics API have texture size restrictions you may
  // want to increase texture width to decrease height.
  int TexGlyphPadding;
  // Padding between glyphs within texture in pixels. Defaults to 1. If your
  // rendering method doesn't rely on bilinear filtering you may set this to 0
  // (will also need to set AntiAliasedLinesUseTex = false).
  ...
  // Tell whether our texture data is known to use colors (rather than just
  // alpha channel), in order to help backend select a format.
  ...
  // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon
  // calling ImGui::NewFrame(), use ImGui::PushFont()/PopFont() to change the
  // current font.
  ...
  // Opaque interface to a font builder (default to stb_truetype, can be changed
  // to use FreeType by defining IMGUI_ENABLE_FREETYPE).
  unsigned int FontBuilderFlags;
  // Shared flags (for all fonts) for custom font builder. THIS IS BUILD
  // IMPLEMENTATION DEPENDENT. Per-font override is also available in
  // ImFontConfig.
...
struct ImFont {
  ...
  // 12-16 // out //            // Sparse. Glyphs->AdvanceX in a directly
  ...
  float FontSize;
  // 4     // in  //            // Height of characters/line, set during loading
  ...
  // 2     // in  // ~ 1        // Number of ImFontConfig involved in creating
  // this font. Bigger than 1 when merging multiple font sources into one
  // ImFont.
  ...
  float Scale;
  // 4     // in  // = 1.f      // Base font scale, multiplied by the per-window
  // font scale which you can adjust with SetWindowFontScale()
  float Ascent, Descent;
  // 4+4   // out //            // Ascent: distance from top to bottom of e.g.
  // 'A' [0..FontSize]
  int MetricsTotalSurface;
  // 4     // out //            // Total surface in pixels to get an idea of the
  // font rasterization/texture cost (not exact, we approximate the cost of
  // padding between glyphs)
  ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 / 8];
  // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store 1-bit
  // for each block of 4K codepoints that has one active glyph. This is mainly
  // used to facilitate iterations across all used codepoints.
  ...
  IMGUI_API void AddRemapChar(ImWchar dst, ImWchar src,
                              bool overwrite_dst = true);
  // Makes 'dst' character/glyph points to 'src' character/glyph. Currently
  // needs to be called AFTER fonts have been built.
...
enum ImGuiViewportFlags_ {
  ...
  // Platform Window: Was created/managed by the user application? (rather than
  // our backend)
  ImGuiViewportFlags_NoDecoration = 1 << 3,
  // Platform Window: Disable platform decorations: title bar, borders, etc.
  // (generally set all windows, but if ImGuiConfigFlags_ViewportsDecoration is
  // set we only set this on popups/tooltips)
  ImGuiViewportFlags_NoTaskBarIcon = 1 << 4,
  // Platform Window: Disable platform task bar icon (generally set on
  // popups/tooltips, or all windows if ImGuiConfigFlags_ViewportsNoTaskBarIcon
  // is set)
  ...
  // Platform Window: Make mouse pass through so we can drag this window while
  // peaking behind it.
  ...
  // Platform Window: Avoid merging this window into another host window. This
  // can only be set via ImGuiWindowClass viewport flags override (because we
  // need to now ahead if we are going to create a viewport in the first
  // place!).
  ...
  // Platform Window: Window is minimized, can skip render. When minimized we
  // tend to avoid using the viewport pos/size for clipping window or testing if
  // they are contained in the viewport.
  ImGuiViewportFlags_IsFocused = 1 << 13,
  // Platform Window: Window is focused (last call to Platform_GetWindowFocus()
  // returned true)
};
...
struct ImGuiViewport {
  ...
  ImVec2 Pos;
  // Main Area: Position of the viewport (Dear ImGui coordinates are the same as
  // OS desktop/native coordinates)
  ...
  ImDrawData *DrawData;
  // The ImDrawData corresponding to this viewport. Valid after Render() and
  ...
  // void* to hold custom data structure for the renderer (e.g. swap chain,
  // framebuffers etc.). generally set by your Renderer_CreateWindow function.
  void *PlatformUserData;
  // void* to hold custom data structure for the OS / platform (e.g. windowing
  // info, render context). generally set by your Platform_CreateWindow
  // function.
  void *PlatformHandle;
  // void* for FindViewportByPlatformHandle(). (e.g. suggested to use natural
  // platform handle such as HWND, GLFWWindow*, SDL_Window*)
  void *PlatformHandleRaw;
  // void* to hold lower-level, platform-native window handle (under Win32 this
  // is expected to be a HWND, unused for other platforms), when using an
  // abstraction layer like GLFW or SDL (where PlatformHandle would be a
  // SDL_Window*)
  bool PlatformWindowCreated;
  // Platform window has been created (Platform_CreateWindow() has been called).
  // This is false during the first frame where a viewport is being created.
  bool PlatformRequestMove;
  // Platform window requested move (e.g. window was moved by the OS / host
  // window manager, authoritative position will be OS window position)
  bool PlatformRequestResize;
  // Platform window requested resize (e.g. window was resized by the OS / host
  // window manager, authoritative size will be OS window size)
  bool PlatformRequestClose;
  // Platform window requested closure (e.g. window was moved by the OS / host
  // window manager, e.g. pressing ALT-F4)
...
// If you are new to Dear ImGui and trying to integrate it into your engine, you
// can probably ignore this for now.
//
// This feature allows you to seamlessly drag Dear ImGui windows outside of your
// application viewport. This is achieved by creating new Platform/OS windows on
// the fly, and rendering into them. Dear ImGui manages the viewport structures,
...
// - When multi-viewports are enabled, all Dear ImGui coordinates become
// absolute coordinates (same as OS coordinates!)
// - So e.g. ImGui::SetNextWindowPos(ImVec2(0,0)) will position a window
// relative to your primary monitor!
// - If you want to position windows relative to your main application viewport,
// use ImGui::GetMainViewport()->Pos as a base position.
//
...
// ImGuiPlatformIO's functions and update monitors info every frame.
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(),
// ImGui::RenderPlatformWindowsDefault() after EndFrame() or Render().
// - Application:  Fix absolute coordinates used in ImGui::SetWindowPos() or
// ImGui::SetNextWindowPos() calls.
//
...
// - Application:  Enable feature with 'io.ConfigFlags |=
// ImGuiConfigFlags_ViewportsEnable'.
// - Backend:      Hook ImGuiPlatformIO's Platform_* and Renderer_* callbacks
// (see below).
//                 Set 'io.BackendFlags |=
//                 ImGuiBackendFlags_PlatformHasViewports' and 'io.BackendFlags
//                 |= ImGuiBackendFlags_PlatformHasViewports'. Update
//                 ImGuiPlatformIO's Monitors list every frame. Update MousePos
...
//   setup in ImGuiPlatformIO, if available:
...
struct ImGuiPlatformIO {
  ...
  //   N = ImGui::NewFrame()                        ~ beginning of the dear
  //   imgui frame: read info from platform/OS windows (latest size/position) F
  //   = ImGui::Begin(), ImGui::EndFrame()        ~ during the dear imgui frame
  //   U = ImGui::UpdatePlatformWindows()           ~ after the dear imgui
  //   frame: create and update all platform/OS windows R =
  //   ImGui::RenderPlatformWindowsDefault()    ~ render D =
  ...
  //
  // The functions are designed so we can mix and match 2 imgui_impl_xxxx files,
  ...
  // . . U . .  // Newly created windows are initially hidden so
  // SetWindowPos/Size/Title can be called on them before showing the window
  void (*Platform_SetWindowPos)(ImGuiViewport *vp, ImVec2 pos);
  // . . U . .  // Set platform window position (given the upper-left corner of
  // client area)
  ImVec2 (*Platform_GetWindowPos)(ImGuiViewport *vp); // N . . . .  //
  void (*Platform_SetWindowSize)(ImGuiViewport *vp, ImVec2 size);
  // . . U . .  // Set platform window client area size (ignoring OS decorations
  // such as OS title bar etc.)
  ImVec2 (*Platform_GetWindowSize)(
      ImGuiViewport *vp); // N . . . .  // Get platform window client area size
  void (*Platform_SetWindowFocus)(
      ImGuiViewport
          *vp); // N . . . .  // Move window to front and set input focus
  bool (*Platform_GetWindowFocus)(ImGuiViewport *vp); // . . U . .  //
  bool (*Platform_GetWindowMinimized)(ImGuiViewport *vp);
  // N . . . .  // Get platform window minimized state. When minimized, we
  // generally won't attempt to get/set size and contents will be culled more
  // easily
  ...
  // . . U . .  // (Optional) Setup global transparency (not per-pixel
  // transparency)
  void (*Platform_UpdateWindow)(ImGuiViewport *vp);
  // . . U . .  // (Optional) Called by UpdatePlatformWindows(). Optional hook
  // to allow the platform backend from doing general book-keeping every frame.
  void (*Platform_RenderWindow)(ImGuiViewport *vp, void *render_arg);
  // . . . R .  // (Optional) Main rendering (platform side! This is often
  ...
  // . . . R .  // (Optional) Call Present/SwapBuffers (platform side! This is
  // often unused!). 'render_arg' is the value passed to
  // RenderPlatformWindowsDefault().
  float (*Platform_GetWindowDpiScale)(ImGuiViewport *vp);
  // N . . . .  // (Optional) [BETA] FIXME-DPI: DPI handling: Return DPI scale
  // for this viewport. 1.0f = 96 DPI.
  void (*Platform_OnChangedViewport)(ImGuiViewport *vp);
  // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI handling: Called during
  // Begin() every time the viewport we are outputting into changes, so backend
  // has a chance to swap fonts to adjust style.
  int (*Platform_CreateVkSurface)(ImGuiViewport *vp, ImU64 vk_inst,
                                  const void *vk_allocators,
                                  ImU64 *out_vk_surface);
  // (Optional) For a Vulkan Renderer to call into Platform code (since the
  // surface creation needs to tie them both).

  // (Optional) Renderer functions (e.g. DirectX, OpenGL, Vulkan)
  void (*Renderer_CreateWindow)(ImGuiViewport *vp);
  // . . U . .  // Create swap chain, frame buffers etc. (called after
  // Platform_CreateWindow)
  void (*Renderer_DestroyWindow)(ImGuiViewport *vp);
  // N . U . D  // Destroy swap chain, frame buffers etc. (called before
  // Platform_DestroyWindow)
  void (*Renderer_SetWindowSize)(ImGuiViewport *vp, ImVec2 size);
  // . . U . .  // Resize swap chain, frame buffers etc. (called after
  // Platform_SetWindowSize)
  void (*Renderer_RenderWindow)(ImGuiViewport *vp, void *render_arg);
  // . . . R .  // (Optional) Clear framebuffer, setup render target, then
  // render the viewport->DrawData. 'render_arg' is the value passed to
  // RenderPlatformWindowsDefault().
  void (*Renderer_SwapBuffers)(ImGuiViewport *vp, void *render_arg);
  // . . . R .  // (Optional) Call Present/SwapBuffers. 'render_arg' is the
  // value passed to RenderPlatformWindowsDefault().
  ...
  // Viewports list (the list is updated by calling ImGui::EndFrame or
  // ImGui::Render) (in the future we will attempt to organize this feature to
...
struct ImGuiPlatformMonitor {
  ...
  // Coordinates of the area displayed on this monitor (Min = upper left, Max =
  // bottom right)
  ImVec2 WorkPos, WorkSize;
  // Coordinates without task bars / side bars / menu bars. Used to avoid
  // positioning popups/tooltips inside this region. If you don't have this
  // info, please copy the value for MainPos/MainSize.
...
IMGUI_API bool ImageButton(ImTextureID user_texture_id, const ImVec2 &size,
                           const ImVec2 &uv0 = ImVec2(0, 0),
                           const ImVec2 &uv1 = ImVec2(1, 1),
                           int frame_padding = -1,
                           const ImVec4 &bg_col = ImVec4(0, 0, 0, 0),
...
// Map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]. When
// using a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex() with ANY
// ImGuiKey_XXXX values will return the same value!
// static inline ImGuiKey GetKeyIndex(ImGuiKey key) { IM_ASSERT(key >=
// ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); return key; }
...
// == int. We generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any
// number of ImGuiMod_XXX value", but you may store only mods in there.
enum ImGuiModFlags_ {
