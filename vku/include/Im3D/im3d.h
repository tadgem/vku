#pragma once
// Define IM3D_CONFIG "myfilename.h" from your build system if you do not want
// to modify im3d_config.h directly.
#ifdef IM3D_CONFIG
#include IM3D_CONFIG
#else
...
// Call after all Im3d calls have been made for the current frame, before
// accessing draw data.
IM3D_API void EndFrame();

// Access draw data. Draw lists are valid after calling EndFrame() and before
// calling NewFrame().
IM3D_API const DrawList *GetDrawLists();
IM3D_API U32 GetDrawListCount();

// Access to text draw data. Draw lists are valid after calling EndFrame() and
// before calling NewFrame().
...
// Begin/end primitive. End() must be called before starting each new primitive
// type.
...
// Alpha draw state, multiplies the alpha set by the color draw state (per
// vertex).
...
// Size draw state, for points/lines this is the radius/width in pixels (per
// vertex).
...
// based on the distance to the view origin (as specified via the AppData
// struct).
IM3D_API void DrawXyzAxes();
IM3D_API void DrawPoint(const Vec3 &_position, float _size, Color _color);
IM3D_API void DrawLine(const Vec3 &_a, const Vec3 &_b, float _size,
                       Color _color);
IM3D_API void DrawQuad(const Vec3 &_a, const Vec3 &_b, const Vec3 &_c,
                       const Vec3 &_d);
IM3D_API void DrawQuad(const Vec3 &_origin, const Vec3 &_normal,
                       const Vec2 &_size);
IM3D_API void DrawQuadFilled(const Vec3 &_a, const Vec3 &_b, const Vec3 &_c,
                             const Vec3 &_d);
IM3D_API void DrawQuadFilled(const Vec3 &_origin, const Vec3 &_normal,
                             const Vec2 &_size);
IM3D_API void DrawCircle(const Vec3 &_origin, const Vec3 &_normal,
                         float _radius, int _detail = -1);
IM3D_API void DrawCircleFilled(const Vec3 &_origin, const Vec3 &_normal,
                               float _radius, int _detail = -1);
IM3D_API void DrawSphere(const Vec3 &_origin, float _radius, int _detail = -1);
IM3D_API void DrawSphereFilled(const Vec3 &_origin, float _radius,
                               int _detail = -1);
...
IM3D_API void DrawCylinder(const Vec3 &_start, const Vec3 &_end, float _radius,
                           int _detail = -1);
IM3D_API void DrawCapsule(const Vec3 &_start, const Vec3 &_end, float _radius,
                          int _detail = -1);
IM3D_API void DrawPrism(const Vec3 &_start, const Vec3 &_end, float _radius,
                        int _sides);
IM3D_API void DrawArrow(const Vec3 &_start, const Vec3 &_end,
                        float _headLength = -1.0f,
                        float _headThickness = -1.0f);
IM3D_API void DrawCone(const Vec3 &_origin, const Vec3 &_normal, float height,
                       float _radius, int _detail);
IM3D_API void DrawConeFilled(const Vec3 &_origin, const Vec3 &_normal,
                             float height, float _radius, int _detail);

// Add text. See TextFlags_ enum for _textFlags. _size is a hint to the
// application-side text rendering.
IM3D_API void Text(const Vec3 &_position, U32 _textFlags, const char *_text,
                   ...); // use the current draw state for size/color
IM3D_API void Text(const Vec3 &_position, float _size, Color _color,
                   U32 _textFlags, const char *_text, ...);

// IDs are used to uniquely identify gizmos and layers. Gizmo should have a
// unique ID during a frame. Note that ids are a hash of the whole ID stack, see
// PushId(), PopId().
IM3D_API Id MakeId(const char *_str);
IM3D_API Id MakeId(const void *_ptr);
IM3D_API Id MakeId(int _i);

// PushId(), PopId() affect the result of subsequent calls to MakeId(), use when
// creating gizmos in a loop.
...
// Layer id state, subsequent primitives are added to a separate draw list
// associated with the id (per primitive).
...
// Manipulate translation/rotation/scale via a gizmo. Return true if the gizmo
// is 'active' (if it modified the output parameter). If _local is true, the
// Gizmo* functions expect that the local matrix is on the matrix stack; in
// general the application should push the local matrix before calling any of
// the following.
IM3D_API bool GizmoTranslation(const char *_id, float _translation_[3],
                               bool _local = false);
IM3D_API bool GizmoRotation(const char *_id, float _rotation_[3 * 3],
                            bool _local = false);
IM3D_API bool GizmoScale(const char *_id, float _scale_[3]); // local scale only
// Unified gizmo, selects local/global, translation/rotation/scale based on the
// context-global gizmo modes. Return true if the gizmo is active.
IM3D_API bool
Gizmo(const char *_id, float _translation_[3], float _rotation_[3 * 3],
      float _scale_[3]); // any of _translation_/_rotation_/_scale_ may be null.
IM3D_API bool Gizmo(const char *_id, float _transform_[4 * 4]);

// Gizmo* overloads which take an ID directly. In some cases the app may want to
// call MakeId() separately, usually to change the gizmo appearance if
// hot/active.
IM3D_API bool GizmoTranslation(Id _id, float _translation_[3],
                               bool _local = false);
IM3D_API bool GizmoRotation(Id _id, float _rotation_[3 * 3],
                            bool _local = false);
IM3D_API bool GizmoScale(Id _id, float _scale_[3]);
IM3D_API bool Gizmo(Id _id, float _transform_[4 * 4]);
IM3D_API bool Gizmo(Id _id, float _translation_[3], float _rotation_[3 * 3],
                    float _scale_[3]);

// Active gizmo ID. This will match the _id parameter passed to a Gizmo*
// function. Return Id_Invalid if no gizmo is in use.
IM3D_API Id GetActiveId();
// ID of the current current 'hot' gizmo (nearest intersecting gizmo along the
// cursor ray).
...
IM3D_API bool IsVisible(const Vec3 &_min,
                        const Vec3 &_max); // axis-aligned bounding box

// Get/set the current context. All Im3d calls affect the currently bound
// context.
IM3D_API Context &GetContext();
IM3D_API void SetContext(Context &_ctx);

// Merge vertex data from _src into _dst_. Layers are preserved. Call before
// EndFrame().
...
struct alignas(IM3D_VERTEX_ALIGNMENT) VertexData {
  ...
  VertexData(const Vec3 &_position, float _size, Color _color)
      : m_positionSize(_position, _size), m_color(_color) {}
};
#pragma warning(pop)
...
struct AppData {
  ...
  Vec4 m_cullFrustum[FrustumPlane_Count] = {
      Vec4(0.0f)}; // Frustum planes for culling (if culling enabled).
  ...
  Vec3 m_viewOrigin =
      Vec3(0.0f); // World space render origin (camera position).
  Vec3 m_viewDirection = Vec3(0.0f); // World space view direction.
  Vec2 m_viewportSize = Vec2(0.0f);  // Viewport size (pixels).
  float m_projScaleY =
      1.0f; // Scale factor used to convert from pixel size -> world scale; use
            // tan(fov) for perspective projections, far plane height for ortho.
  ...
  float m_snapTranslation =
      0.0f; // Snap value for translation gizmos (world units). 0 = disabled.
  float m_snapRotation =
      0.0f; // Snap value for rotation gizmos (radians). 0 = disabled.
  ...
  DrawPrimitivesCallback *drawCallback =
      nullptr; // e.g. void Im3d_Draw(const DrawList& _drawList)

  // Extract cull frustum planes from the view-projection matrix.
  // Set _ndcZNegativeOneToOne = true if the proj matrix maps z from [-1,1]
  // (OpenGL style).
...
// Context stores all relevant state - main interface affects the context
// currently bound via SetCurrentContext().
struct IM3D_API Context {
  ...
  void vertex(const Vec3 &_position) {
    vertex(_position, getSize(), getColor());
  }

  void text(const Vec3 &_position, float _size, Color _color, TextFlags _flags,
            const char *_textStart, const char *_textEnd);
  void text(const Vec3 &_position, float _size, Color _color, TextFlags _flags,
  ...
  // Low-level interface for internal and app-defined gizmos. May be subject to
  // breaking changes.

  bool gizmoAxisTranslation_Behavior(Id _id, const Vec3 &_origin,
                                     const Vec3 &_axis, float _snap,
                                     float _worldHeight, float _worldSize,
                                     Vec3 *_out_);
  void gizmoAxisTranslation_Draw(Id _id, const Vec3 &_origin, const Vec3 &_axis,
                                 float _worldHeight, Color _color);

  bool gizmoPlaneTranslation_Behavior(Id _id, const Vec3 &_origin,
                                      const Vec3 &_normal, float _snap,
                                      float _worldSize, Vec3 *_out_);
  void gizmoPlaneTranslation_Draw(Id _id, const Vec3 &_origin,
                                  const Vec3 &_normal, float _worldSize,
                                  Color _color);

  bool gizmoAxislAngle_Behavior(Id _id, const Vec3 &_origin, const Vec3 &_axis,
                                float _snap, float _worldRadius,
                                float _worldSize, float *_out_);
  void gizmoAxislAngle_Draw(Id _id, const Vec3 &_origin, const Vec3 &_axis,
                            float _worldRadius, Color _color, float _minAlpha);

  bool gizmoAxisScale_Behavior(Id _id, const Vec3 &_origin, const Vec3 &_axis,
                               float _snap, float _worldHeight,
                               float _worldSize, float *_out_);
  void gizmoAxisScale_Draw(Id _id, const Vec3 &_origin, const Vec3 &_axis,
                           float _worldHeight, Color _color);

  // Convert pixels -> world space size based on distance between _position and
  // view origin.
  float pixelsToWorldSize(const Vec3 &_position, float _pixels);
  // Convert world space size -> pixels based on distance between _position and
  // view origin.
  float worldSizeToPixels(const Vec3 &_position, float _pixels);
  // Blend between _min and _max based on distance betwen _position and view
  // origin.
  int estimateLevelOfDetail(const Vec3 &_position, float _worldSize,
                            int _min = 4, int _max = 256);
  ...
  bool wasKeyPressed(Key _key) const {
    return m_keyDownCurr[_key] && !m_keyDownPrev[_key];
  }

  // Visibiity tests for culling.
  bool isVisible(const VertexData *_vdata,
                 DrawPrimitiveType _prim);            // per-vertex
  ...
  Id m_hotId; // ID of the current 'hot' gizmo (nearest intersecting gizmo along
              // the cursor ray). NB this is the id of the *sub* gizmo, not the
              // app-specified ID.
  float m_hotDepth; // Depth of the current hot gizmo along the cursor ray, for
                    // handling occlusion.
  Id m_appId; // Current ID *without* the hashing the ID stack (= _id arg to
              // Gizmo* functions).
  ...
  // Return the total number of primitives (sorted + unsorted) of the given
  // _type in all layers.
...
private:
  ...
  // Vertex data: one list per layer, per primitive type, *2 for
  // sorted/unsorted.
  typedef Vector<VertexData> VertexList;
  Vector<VertexList *>
      m_vertexData[2];   // Each layer is DrawPrimitive_Count consecutive lists.
  ...
  Vector<DrawList>
      m_drawLists;   // All draw lists for the current frame, available after
                     // calling endFrame() before calling reset().
  bool m_sortCalled; // Avoid calling sort() during every call to draw().
  bool m_endFrameCalled; // For assert, if vertices are pushed after endFrame()
                         // was called.
  ...
  DrawPrimitiveType m_primType;
  U32 m_firstVertThisPrim; // Index of the first vertex pushed during this
                           // primitive.
  U32 m_vertCountThisPrim; // # calls to vertex() since the last call to
                           // begin().
  ...
  Vec4 m_cullFrustum[FrustumPlane_Count]; // Optimized frustum planes from
                                          // m_appData.m_cullFrustum.
...
inline const TextDrawList *GetTextDrawLists() {
  return GetContext().getTextDrawLists();
}
inline U32 GetTextDrawListCount() {
...
inline void BeginTriangleStrip() {
  GetContext().begin(PrimitiveMode_TriangleStrip);
}
inline void End() { GetContext().end(); }

inline void Vertex(const Vec3 &_position) {
  GetContext().vertex(_position, GetContext().getSize(),
                      GetContext().getColor());
}
inline void Vertex(const Vec3 &_position, Color _color) {
  GetContext().vertex(_position, GetContext().getSize(), _color);
}
inline void Vertex(const Vec3 &_position, float _size) {
  GetContext().vertex(_position, _size, GetContext().getColor());
}
inline void Vertex(const Vec3 &_position, float _size, Color _color) {
  GetContext().vertex(_position, _size, _color);
}
inline void Vertex(float _x, float _y, float _z) { Vertex(Vec3(_x, _y, _z)); }
inline void Vertex(float _x, float _y, float _z, Color _color) {
  Vertex(Vec3(_x, _y, _z), _color);
}
inline void Vertex(float _x, float _y, float _z, float _size) {
  Vertex(Vec3(_x, _y, _z), _size);
}
inline void Vertex(float _x, float _y, float _z, float _size, Color _color) {
  Vertex(Vec3(_x, _y, _z), _size, _color);
}

inline void PushDrawState() {
...
inline void SetColor(float _r, float _g, float _b, float _a) {
  GetContext().setColor(Color(_r, _g, _b, _a));
}
...
inline bool GizmoTranslation(const char *_id, float _translation_[3],
                             bool _local) {
  return GizmoTranslation(MakeId(_id), _translation_, _local);
}
inline bool GizmoRotation(const char *_id, float _rotation_[3 * 3],
                          bool _local) {
  return GizmoRotation(MakeId(_id), _rotation_, _local);
}
inline bool GizmoScale(const char *_id, float _scale_[3]) {
  return GizmoScale(MakeId(_id), _scale_);
}
inline bool Gizmo(const char *_id, float _translation_[3],
                  float _rotation_[3 * 3], float _scale_[3]) {
  return Gizmo(MakeId(_id), _translation_, _rotation_, _scale_);
}
inline bool Gizmo(const char *_id, float _transform_[4 * 4]) {
...
inline bool IsVisible(const Vec3 &_origin, float _radius) {
  return GetContext().isVisible(_origin, _radius);
}
inline bool IsVisible(const Vec3 &_min, const Vec3 &_max) {
...
inline void MergeContexts(Context &_dst_, const Context &_src) {
  _dst_.merge(_src);
}
