...
public:
  ...
  public:
    ...
    ShaderBufferBindingData(uint32_t set, uint32_t binding, VkDeviceSize size,
                            Buffer::BufferType bufferType,
                            ShaderBufferFrameData &buffer);
  ...
  void AttachBuffer(VkState &vk, uint32_t frameIndex, uint32_t set,
                    uint32_t binding, ShaderBufferFrameData &buffer);
  void CreateBuffer(VkState &vk, uint32_t set, uint32_t binding);

  void UpdateDescriptors(VkState &vk);

  template <typename _Ty>
  bool SetBuffer(uint32_t frameIndex, uint32_t set, uint32_t binding,
                 const _Ty &value) {
  ...
  template <typename _Ty>
  bool SetBufferArrayElement(uint32_t frameIndex, uint32_t set,
                             uint32_t binding, uint32_t index, const _Ty &value,
                             uint32_t innerElementOffset = 0) {
  ...
  bool SetSampler(VkState &vk, const char *name, const VkImageView &imageView,
                  const VkSampler &sampler, bool isAttachment = false);
  bool SetSampler(VkState &vk, const char *name, Texture &texture);
  bool SetColourAttachment(VkState &vk, const String &name,
                           Framebuffer &framebuffer,
                           uint32_t colourAttachmentIndex);
  bool SetColourAttachment(VkState &vk, const char *name,
                           Framebuffer &framebuffer,
                           uint32_t colourAttachmentIndex);
  bool SetDepthAttachment(VkState &vk, const String &name,
                          Framebuffer &framebuffer);
  bool SetDepthAttachment(VkState &vk, const char *name,
                          Framebuffer &framebuffer);
