...
namespace vku
{
    ...
    struct ShaderStage
    {
        ...
        {
            String name(VKU_FILESYSTEM_NS::filesystem::path(stagePath).filename().string(), *vk.m_CPUAllocator);

            auto stageBin = vk.m_Backend->LoadBinaryFromPath(vk, stagePath);
            return CreateFromBinary(vk, stageBin, stageType, VKU_FILESYSTEM_NS::filesystem::path(stagePath).filename().string().c_str());
        }
        ...
        static ShaderStage CreateFromSourcePath(VkState & vk, const char* path, const ShaderStageType& type)
        {
            ...
            return CreateFromSource(vk, source, type, path, VKU_FILESYSTEM_NS::filesystem::path(path).filename().string().c_str());
        }
    };
