...
public:
  ...
  void Init(IAllocator &alloc, VkDevice logical_device,
            uint32_t initialSetAmount, Vector<PoolSizeRatio> ratios);
  void Reset(VkDevice device);
  void Free(VkDevice device);

  VkDescriptorSet Allocate(IAllocator &alloc, VkDevice device,
                           VkDescriptorSetLayout layout, void *pNext = nullptr);

  VkDescriptorPool GetPool(IAllocator &alloc, VkDevice device);
  VkDescriptorPool CreatePool(IAllocator &alloc, VkDevice device,
                              uint32_t setCount);
