...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered,
  ...
  // We report navigated and navigation-activated items as hovered but we don't
  // set g.HoveredId to not interfere with mouse.
  ...
  if (g.NavActivateDownId == id) {
    ...
    if (!nav_activated_by_inputs &&
      ...
      const float t1 = ImMax(ImMax(key1->DownDuration, key2->DownDuration),
                             key3->DownDuration);
  ...
  if (g.ActiveId == id) {
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (mouse_button == -1) {
        ...
        bool release_in =
            hovered && (flags & ImGuiButtonFlags_PressedOnClickRelease) != 0;
      ...
      // button is released
      if (g.NavActivateDownId == id)
        held = true; // hovered == true not true as we are already likely
                     // hovered on direct activation.
      else
        ClearActiveID();
    }
    if (pressed)
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos) {
  ...
  if (hovered)
    window->DrawList->AddCircleFilled(
        center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos,
  ...
  ImU32 text_col = GetColorU32(ImGuiCol_Text);
  if (hovered || held)
    window->DrawList->AddCircleFilled(bb.GetCenter() + ImVec2(0.0f, -0.5f),
...
void ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  ScrollbarEx(bb, id, axis, &scroll, (ImS64)size_avail, (ImS64)size_contents,
              rounding_corners);
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis,
                        ImS64 *p_scroll_v, ImS64 size_avail_v,
  ...
  ImRect bb = bb_frame;
  bb.Expand(
      ImVec2(-ImClamp(IM_TRUNC((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
  ...
  const ImS64 win_size_v =
      ImMax(ImMax(size_contents_v, size_avail_v), (ImS64)1);
  const float grab_h_pixels =
      ImClamp(scrollbar_size_v * ((float)size_avail_v / (float)win_size_v),
...
bool ImGui::Checkbox(const char *label, bool *v) {
  ...
  if (mixed_value) {
    ...
    // ImGuiItemFlags_MixedValue supported by all widgets (not just checkbox)
...
bool ImGui::RadioButton(const char *label, bool active) {
  ...
  if (style.FrameBorderSize > 0.0f) {
    window->DrawList->AddCircle(center + ImVec2(1, 1), radius,
                                GetColorU32(ImGuiCol_BorderShadow), num_segment,
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg,
  ...
  RenderRectFilledRangeH(window->DrawList, bb,
                         GetColorU32(ImGuiCol_PlotHistogram), fill_n0, fill_n1,
                         style.FrameRounding);

  // Default displaying the fraction as percentage string, but user can override
  // it Don't display text for indeterminate bars by default
  char overlay_buf[32];
  if (!is_indeterminate || overlay != NULL) {
    ...
    if (overlay_size.x > 0.0f) {
      ...
      RenderTextClipped(
          ImVec2(ImClamp(text_x, bb.Min.x,
...
void ImGui::SeparatorTextEx(ImGuiID id, const char *label,
  ...
  const float text_baseline_y =
      ImTrunc((bb.GetHeight() - label_size.y) * style.SeparatorTextAlign.y +
              0.99999f); // ImMax(padding.y, ImTrunc((style.SeparatorTextSize -
                         // label_size.y) * 0.5f));
...
bool ImGui::SplitterBehavior(const ImRect &bb, ImGuiID id, ImGuiAxis axis,
  ...
  // ImGuiButtonFlags_AllowOverlap here is to allow caller of SplitterBehavior()
...
bool ImGui::BeginCombo(const char *label, const char *preview_value,
  ...
  if (!(flags & ImGuiComboFlags_NoArrowButton)) {
    ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered
                                                       : ImGuiCol_Button);
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    window->DrawList->AddRectFilled(
    ...
    if (popup_window->WasActive) {
      ...
      popup_window->AutoPosLastDirection =
          (flags & ImGuiComboFlags_PopupAlignLeft)
              ? ImGuiDir_Left
              : ImGuiDir_Down; // Left = "Below, Toward Left", Down = "Below,
  ...
  case ImGuiDataType_U32:
    if (op == '+') {
      *(ImU32 *)output = ImAddClampOverflow(
          *(const ImU32 *)arg1, *(const ImU32 *)arg2, IM_U32_MIN, IM_U32_MAX);
    }
    if (op == '-') {
...
bool ImGui::DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
  ...
  if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
    ...
    format = ImParseFormatSanitizeForScanning(format, format_sanitized,
                                              IM_ARRAYSIZE(format_sanitized));
...
bool ImGui::DragFloatRange2(const char *label, float *v_current_min,
  ...
  bool value_changed =
      DragScalar("##min", ImGuiDataType_Float, v_current_min, v_speed, &min_min,
  ...
  value_changed |=
      DragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min,
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
  ...
  if (is_logarithmic) {
        ...
        0.0f) // Range crosses zero, so we have to do this in two parts
    {
      ...
      result =
          (TYPE) - (-v_max_fudged * ImPow(-v_min_fudged / -v_max_fudged,
...
bool ImGui::InputTextWithHint(const char *label, const char *hint, char *buf,
                              ...
                              void *user_data) {
  IM_ASSERT(
      !(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline() or
...
static void stb_textedit_replace(ImGuiInputTextState *str,
  ...
  IM_ASSERT(0); // Failed to insert character, normally shouldn't happen because
                // of how we currently use stb_textedit_replace()
}
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  IM_ASSERT(!(
      ...
      (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together
                                                 // (they both use up/down keys)
  IM_ASSERT(!(
      (flags & ImGuiInputTextFlags_CallbackCompletion) &&
      (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together
  ...
  if (is_multiline) {
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding,
                 ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach
                                // FramePadding edges
    ...
    // Support for internal ImGuiInputTextFlags_MergedItem flag, which could be
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs ||
    ...
    // From the moment we focused we are normally ignoring the content of 'buf'
    ...
    bool recycle_state =
        (state->ID == id && !init_changed_specs && !init_reload_from_user_buf);
    ...
    state->CurLenA =
        (int)(buf_end -
              buf); // We can't get the result from ImStrncpy() above because it
  ...
  if (g.ActiveId == id) {
    ...
    if (select_all) {
      ...
      if ((multiclick_count % 2) == 0) {
        ...
        state->Stb.cursor =
            ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(state, state->Stb.cursor);
        state->Stb.select_end = state->Stb.cursor;
        ImStb::stb_textedit_clamp(state, &state->Stb);
      } else {
        // Triple-click: Select line
        const bool is_eol =
            ImStb::STB_TEXTEDIT_GETCHAR(state, state->Stb.cursor) == '\n';
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    // Using Shortcut() with ImGuiInputFlags_RouteFocused (default policy) to
    ...
    const bool is_cut =
        (Shortcut(ImGuiMod_Shortcut | ImGuiKey_X, id, f_repeat) ||
         Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, id, f_repeat)) &&
        !is_readonly && !is_password &&
        (!is_multiline || state->HasSelection());
    const bool is_copy = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_C, id) ||
                          Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, id)) &&
                         !is_password &&
                         (!is_multiline || state->HasSelection());
    const bool is_paste =
        (Shortcut(ImGuiMod_Shortcut | ImGuiKey_V, id, f_repeat) ||
         Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, id, f_repeat)) &&
        !is_readonly;
    const bool is_undo =
    ...
    const bool is_redo =
        (Shortcut(ImGuiMod_Shortcut | ImGuiKey_Y, id, f_repeat) ||
         (is_osx && Shortcut(ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z,
                             id, f_repeat))) &&
        !is_readonly && is_undoable;
    ...
    if (IsKeyPressed(ImGuiKey_LeftArrow)) {
      ...
      if (io.SetClipboardTextFn) {
        const int ib = state->HasSelection() ? ImMin(state->Stb.select_start,
                                                     state->Stb.select_end)
                                             : 0;
        const int ie = state->HasSelection() ? ImMax(state->Stb.select_start,
                                                     state->Stb.select_end)
                                             : state->CurLenW;
  ...
  if (g.ActiveId == id) {
    ...
    if (revert_edit && !is_readonly) {
      if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
        ...
        if (apply_new_text_length > 0) {
          w_text.resize(
              ImTextCountCharsFromUtf8(apply_new_text,
    ...
    // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we
    ...
    // user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue without
    // maintaining any user-side storage (please note that if you use this
    // property along ImGuiInputTextFlags_CallbackResize you can end up with
  ...
  if (render_cursor || render_selection) {
...

    {
      ...
      int line_count = 0;
      // for (const ImWchar* s = text_begin; (s = (const ImWchar*)wcschr((const
      ...
      if (searches_result_line_no[1] >= 0) {
        select_start_offset.x =
            InputTextCalcTextSizeW(&g,
                                   ImStrbolW(searches_input_ptr[1], text_begin),
                                   searches_input_ptr[1])
                .x;
    ...
    if (render_cursor && state->CursorFollow) {
      ...
      if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
        ...
        if (cursor_offset.x < state->ScrollX)
          state->ScrollX =
              IM_TRUNC(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
        else if (cursor_offset.x - visible_width >= state->ScrollX)
          state->ScrollX =
              IM_TRUNC(cursor_offset.x - visible_width + scroll_increment_x);
      } else {
        state->ScrollX = 0.0f;
      }
    ...
    if (render_selection) {
      ...
      for (const ImWchar *p = text_selected_begin; p < text_selected_end;) {
        ...
        if (rect_pos.y < clip_rect.y) {
          ...
          if (rect_size.x <= 0.0f)
            rect_size.x = IM_TRUNC(g.Font->GetCharAdvance((ImWchar)' ') *
    ...
    // cases (e.g. single-line 1 MB string) which would make ImDrawList crash.
    if (is_multiline ||
    ...
    if (is_multiline)
      text_size = ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(
  ...
  if (is_multiline) {
    ...
    // ItemAdd() call specifies the ImGuiItemFlags_Inputable (ref issue
    // #4761)...
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB |
    ...
    const bool hide_prefix =
        (IM_TRUNC(w_items / components) <=
         CalcTextSize((flags & ImGuiColorEditFlags_Float) ? "M:0.000" : "M:000")
             .x);
    ...
    for (int n = 0; n < components; n++) {
      ...
      // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values snap in
...
bool ImGui::ColorPicker4(const char *label, float col[4],
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    InvisibleButton(
        "hsv", ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width,
                      sv_picker_size));
  ...
  if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
    ...
    if (flags & ImGuiColorEditFlags_DisplayHSV ||
        (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
      value_changed |=
          ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
    if (flags & ImGuiColorEditFlags_DisplayHex ||
        (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
      value_changed |=
          ColorEdit4("##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex);
    PopItemWidth();
  }
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    draw_list->AddRectFilledMultiColor(
        ...
        col_black, col_black);
    RenderFrameBorder(
        picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0.0f);
    sv_cursor_pos.x =
...
bool ImGui::ColorButton(const char *desc_id, const ImVec4 &col,
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
    ...
    RenderColorRectWithAlphaCheckerboard(
        window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y),
        bb_inner.Max, GetColorU32(col_rgb), grid_step,
        ImVec2(-grid_step + off, off), rounding, ImDrawFlags_RoundCornersRight);
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  if ((flags & ImGuiColorEditFlags_DisplayMask_) == 0)
    flags |=
        ImGuiColorEditFlags_DefaultOptions_ & ImGuiColorEditFlags_DisplayMask_;
  if ((flags & ImGuiColorEditFlags_DataTypeMask_) == 0)
    flags |=
        ImGuiColorEditFlags_DefaultOptions_ & ImGuiColorEditFlags_DataTypeMask_;
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags) {
  ...
  if (BeginPopup("Copy")) {
    ...
    if (Selectable(buf))
      SetClipboardText(buf);
    if (!(flags & ImGuiColorEditFlags_NoAlpha)) {
      ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X%02X", cr, cg, cb,
                     ca);
      if (Selectable(buf))
        SetClipboardText(buf);
    }
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags) {
  ...
  if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen) {
    if (g.NextItemData.OpenCond & ImGuiCond_Always) {
      ...
      // We treat ImGuiCond_Once and ImGuiCond_FirstUseEver the same because
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // ImGuiTreeNodeFlags_NavLeftJumpsBackHere enabled: Store data for the current
  ...
  // ImGuiStyle. Currently only supports 32 level deep and we are fine with (1
  ...
  // ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we want
...
bool ImGui::CollapsingHeader(const char *label, ImGuiTreeNodeFlags flags) {
  ...
  return TreeNodeBehavior(window->GetID(label),
                          flags | ImGuiTreeNodeFlags_CollapsingHeader, label);
}
...
bool ImGui::CollapsingHeader(const char *label, bool *p_visible,
  ...
  if (p_visible != NULL) {
    ...
    float button_x =
        ImMax(g.LastItemData.Rect.Min.x,
...
ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags) {
  ...
  if ((flags & ImGuiTypingSelectFlags_AllowBackspace) &&
      IsKeyPressed(ImGuiKey_Backspace, 0, ImGuiInputFlags_Repeat)) {
...
//   from ImGuiMultiSelectIO::NavIdItem, otherwise from
...
int ImGui::TypingSelectFindBestLeadingMatch(
  ...
  for (int idx = 0; idx < items_count; idx++) {
    const char *item_name = get_item_name_func(user_data, idx);
    const int match_len = ImStrimatchlen(
...
bool ImGui::BeginMenuBar() {
  ...
  ImRect clip_rect(
      ImFloor(bar_rect.Min.x + window->WindowBorderSize),
      ImFloor(bar_rect.Min.y + window->WindowBorderSize),
      ImFloor(ImMax(bar_rect.Min.x,
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    while (
        nav_earliest_child->ParentWindow &&
  ...
  IM_MSVC_WARNING_SUPPRESS(6011); // Static Analysis false positive "warning
...
bool ImGui::BeginViewportSideBar(const char *name, ImGuiViewport *viewport_p,
  ...
  if (bar_window == NULL || bar_window->BeginCount == 0) {
    ...
    ImGuiAxis axis = (dir == ImGuiDir_Up || dir == ImGuiDir_Down) ? ImGuiAxis_Y
                                                                  : ImGuiAxis_X;
  ...
  SetNextWindowViewport(
      viewport->ID); // Enforce viewport so we don't create our own viewport
                     // when ImGuiConfigFlags_ViewportsNoMerge is set.
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
  ...
  // ever becoming a problem we can switch to use e.g. ImGuiStorage mapping key
  // to last frame used.
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    pressed =
        Selectable("", menu_is_open, selectable_flags, ImVec2(w, label_size.y));
  ...
  if (menu_is_open) {
    ...
    if (menu_is_open) {
      ...
      // Perform an init request in the case the popup was already open (via a
      // previous mouse hover)
...
bool ImGui::MenuItemEx(const char *label, const char *icon,
  ...
  // We've been using the equivalent of ImGuiSelectableFlags_SetNavIdOnHover on
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
      ...
      if (shortcut_w > 0.0f) {
        ...
        RenderText(pos + ImVec2(offsets->OffsetShortcut + stretch_w, 0.0f),
                   shortcut, NULL, false);
        PopStyleColor();
      }
      if (selected)
...
bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb,
  ...
  // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flag, or
  ...
  window->DC.CursorPos = ImVec2(tab_bar->BarRect.Min.x,
...
ImGuiTabItem *
ImGui::TabBarFindMostRecentlySelectedTabForActiveWindow(ImGuiTabBar *tab_bar) {
  ImGuiTabItem *most_recently_selected_tab = NULL;
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ImGuiTabItem *tab = &tab_bar->Tabs[tab_n];
    if (most_recently_selected_tab == NULL ||
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags,
                         ImGuiWindow *window) {
  ImGuiContext &g = *GImGui;
  ...
  IM_ASSERT(
      ...
      tab_bar); // Can't work while the tab bar is active as our tab doesn't
                ...
                // something like (tab_bar->CurrFrameVisible < g.FrameCount) but
                // we'd need to solve why triggers the commented early-out
                // assert in BeginTabBarEx() (probably dock node going from
                // implicit to explicit in same frame)

  if (!window->HasCloseButton)
    tab_flags |=
        ImGuiTabItemFlags_NoCloseButton; // Set _NoCloseButton immediately
...
static ImGuiTabItem *ImGui::TabBarTabListPopupButton(ImGuiTabBar *tab_bar) {
  ...
  const ImVec2 backup_cursor_pos = window->DC.CursorPos;
  window->DC.CursorPos = ImVec2(tab_bar->BarRect.Min.x - g.Style.FramePadding.y,
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  if (g.DragDropActive &&
      !g.DragDropPayload.IsDataType(
          IMGUI_PAYLOAD_TYPE_WINDOW)) // FIXME: May be an opt-in property of the
  ...
  if (held && single_floating_window_node && IsMouseDragging(0, 0.0f)) {
    ...
    const bool can_undock =
        docked_window != NULL &&
        !(docked_window->Flags & ImGuiWindowFlags_NoMove) &&
    ...
    if (can_undock) {
      ...
      if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
      {
        ...
        float threshold_y = (threshold_base * 1.5f) +
                            ImClamp((ImFabs(g.IO.MouseDragMaxDistanceAbs[0].x) -
                                     threshold_base * 2.0f) *
                                        0.20f,
                                    0.0f, threshold_base * 4.0f);
        // GetForegroundDrawList()->AddRect(ImVec2(bb.Min.x - threshold_x,
        // bb.Min.y - threshold_y), ImVec2(bb.Max.x + threshold_x, bb.Max.y +
        // threshold_y), IM_COL32_WHITE); // [DEBUG]
...
ImVec2 ImGui::TabItemCalcSize(ImGuiWindow *window) {
  return TabItemCalcSize(
      window->Name, window->HasCloseButton ||
...
void ImGui::TabItemBackground(ImDrawList *draw_list, const ImRect &bb,
  ...
  draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding, y1 + rounding), rounding,
                           6, 9);
  draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y1 + rounding), rounding,
                           9, 12);
  ...
  if (g.Style.TabBorderSize > 0.0f) {
    ...
    draw_list->PathArcToFast(
        ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f), rounding, 6,
        9);
    draw_list->PathArcToFast(
        ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f), rounding, 9,
        12);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb,
  ...
  // In Style V2 we'll have full override of all colors per state (e.g. focused,
  // selected) But right now if you want to alter text color of tabs this is
  // what you need to do.
#if 0
