...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count,
  ...
  // of that rectangle, in order for ImDrawListSplitter::Merge() to merge the
  ...
  if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
    ...
    // IMGUI_DEBUG_PRINT("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width
...
static void TableSetupColumnFlags(ImGuiTable *table, ImGuiTableColumn *column,
  ...
  if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of each
                                                    // set is used.
  }
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
  ...
  if (table->AngledHeadersHeight > 0.0f)
    if (g.IO.MousePos.y >= table->OuterRect.Min.y &&
      ...
      mouse_skewed_x += ImTrunc((table->OuterRect.Min.y +
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    column->WidthGiven = ImMax(
        column->WidthGiven, ImMin(column->WidthRequest, table->MinColumnWidth));
    ...
    if (table->InstanceCurrent == 0) {
      ...
      // As we store an absolute value to make per-cell updates faster, we need
      // to offset values used for width computation.
...
void ImGui::TableUpdateBorders(ImGuiTable *table) {
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    // TableDrawBorders()
    const float border_y2_hit = (table->Flags & ImGuiTableFlags_NoBordersInBody)
                                    ? hit_y2_head
                                    : hit_y2_body;
...
void ImGui::EndTable() {
  ...
  if (temp_data->UserOuterSize.y <= 0.0f) {
    const float decoration_size = (table->Flags & ImGuiTableFlags_ScrollY)
    ...
    outer_window->DC.CursorMaxPos.y =
        ImMax(backup_outer_max_pos.y,
              ImMin(table->OuterRect.Max.y, inner_content_max_y));
  } else {
    // OuterRect.Max.y may already have been pushed downward from the initial
    // value (unless ImGuiTableFlags_NoHostExtendY is set)
    outer_window->DC.CursorMaxPos.y =
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags,
  ...
  // users of ImGuiTableFlags_ScrollX.
  ...
  // FIXME: If we recorded the number of \n in names we could compute header row
  // height
  column->NameOffset = -1;
...
void ImGui::TableEndRow(ImGuiTable *table) {
  ...
  if (is_visible) {
    ...
    if (table->RowBgColor[0] != IM_COL32_DISABLE)
      ...
      bg_col0 =
          GetColorU32((table->RowBgColorCounter & 1) ? ImGuiCol_TableRowBgAlt
                                                     : ImGuiCol_TableRowBg);
    ...
    if (top_border_col && bg_y1 >= table->BgClipRect.Min.y &&
      ...
      window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y1),
                                ImVec2(table->BorderX2, bg_y1), top_border_col,
                                border_size);
...
void ImGui::TableEndCell(ImGuiTable *table) {
  ...
  *p_max_pos_x = ImMax(*p_max_pos_x, window->DC.CursorMaxPos.x);
  if (column->IsEnabled)
    table->RowPosY2 = ImMax(table->RowPosY2,
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
  ...
  float column_1_width =
      ImMax(column_1->WidthRequest - (column_0_width - column_0->WidthRequest),
            min_width);
  column_0_width =
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable *table) {
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    if (!column->IsEnabled &&
        !(column->Flags &
          ImGuiTableColumnFlags_WidthStretch)) // Cannot reset weight of hidden
                                               // stretch column
      continue;
    column->CannotSkipItemsQueue = (1 << 0);
  ...
  if (merge_group_mask != 0) {
    ...
    for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups);
      ...
      if (int merge_channels_count =
        ...
        // merge_group->ClipRect.Max, IM_COL32(255, 0, 0, 200), 0.0f, 0, 1.0f);
        // // [DEBUG]
...
void ImGui::TableDrawBorders(ImGuiTable *table) {
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
      ...
      if (draw_y2 > draw_y1)
        inner_drawlist->AddLine(
            ImVec2(column->MaxX, draw_y1), ImVec2(column->MaxX, draw_y2),
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
      ...
      inner_drawlist->AddRect(outer_border.Min, outer_border.Max + ImVec2(1, 1),
...
void ImGui::TableHeadersRow() {
  ...
  for (int column_n = 0; column_n < columns_count; column_n++) {
    ...
    const char *name =
        (TableGetColumnFlags(column_n) & ImGuiTableColumnFlags_NoHeaderLabel)
            ? ""
...
void ImGui::TableHeader(const char *label) {
  ...
  float label_height =
      ImMax(label_size.y, table->RowMinHeight - table->RowCellPaddingY * 2.0f);
  ...
  ItemSize(ImVec2(0.0f, label_height)); // Don't declare unclipped width, it'll
  ...
  if (held && (table->Flags & ImGuiTableFlags_Reorderable) &&
    ...
    // that is marked with ImGuiTableColumnFlags_NoReorder.
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle,
  ...
  // Angle argument expressed in (-IM_PI/2 .. +IM_PI/2) as it is easier to think
  // about for user.
  ...
  if (table->FreezeColumnsCount > 0)
    clip_rect_min_x = ImMax(clip_rect_min_x,
  ...
  const float ascent_scaled =
      g.Font->Ascent *
      (g.FontSize /
       g.Font->FontSize); // FIXME: Standardize those scaling factors better
  ...
  for (int pass = 0; pass < 2; pass++)
    for (int order_n = 0; order_n < data_count; order_n++) {
      ...
      if (pass == 0) {
        ...
        // Draw label
        // - First draw at an offset where RenderTextXXX() function won't meddle
        // with applying current ClipRect, then transform to final offset.
        // - Handle multiple lines manually, as we want each lines to follow on
        // the horizontal border, rather than see a whole block rotated.
        ...
        column->ContentMaxXHeadersUsed = column->ContentMaxXHeadersIdeal =
            column->WorkMinX +
            ImCeil(label_lines * line_off_step_x - line_off_for_align_x);
        ...
        while (label_name < label_name_end) {
          ...
          // FIXME: Individual line clipping for right-most column is broken for
          // negative angles.
          ImVec2 label_size = CalcTextSize(label_name, label_name_eol);
          float clip_width =
              max_label_width - padding.y; // Using padding.y*2.0f would be
                                           // symetrical but hide more text.
          float clip_height =
              ImMin(label_size.y, column->ClipRect.Max.x - column->WorkMinX -
                                      line_off_curr_x);
          ImRect clip_r(window->ClipRect.Min,
                        window->ClipRect.Min + ImVec2(clip_width, clip_height));
          int vtx_idx_begin = draw_list->_VtxCurrentIdx;
          PushStyleColor(ImGuiCol_Text, request->TextColor);
          RenderTextEllipsis(draw_list, clip_r.Min, clip_r.Max, clip_r.Max.x,
                             clip_r.Max.x, label_name, label_name_eol,
                             &label_size);
          PopStyleColor();
          int vtx_idx_end = draw_list->_VtxCurrentIdx;

          // Up<>Down alignment
          const float available_space = ImMax(
              clip_width - label_size.x + ImAbs(padding.x * cos_a) * 2.0f -
          ...
          if (flip_label)
            pivot_out += unit_right *
                         (clip_width - ImMax(0.0f, clip_width - label_size.x));
          pivot_out.x +=
              flip_label ? line_off_curr_x + line_off_step_x : line_off_curr_x;
          ShadeVertsTransformPos(draw_list, vtx_idx_begin, vtx_idx_end,
                                 pivot_in, label_cos_a, label_sin_a,
                                 pivot_out); // Rotate and offset
          // if (g.IO.KeyShift) { ImDrawList* fg_dl = GetForegroundDrawList();
          // vtx_idx_begin = fg_dl->_VtxCurrentIdx; fg_dl->AddRect(clip_r.Min,
          // clip_r.Max, IM_COL32(0, 255, 0, 255), 0.0f, 0, 1.0f);
          // ShadeVertsTransformPos(fg_dl, vtx_idx_begin, fg_dl->_VtxCurrentIdx,
...
void ImGui::TableOpenContextMenu(int column_n) {
  ...
  if (table->Flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable |
  ...
  if (flags_for_section_to_display & ImGuiTableFlags_Resizable) {
    ...
    if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount &&
        (table->Flags & ImGuiTableFlags_SizingMask_) !=
            ImGuiTableFlags_SizingFixedSame)
      size_all_desc =
          LocalizeGetMsg(ImGuiLocKey_TableSizeAllFit); // "###SizeAll" All fixed
    else
      size_all_desc = LocalizeGetMsg(
          ImGuiLocKey_TableSizeAllDefault); // "###SizeAll" All stretch or mixed
...
void ImGui::DebugNodeTable(ImGuiTable *table) {
  ...
  const bool is_active =
      (table->LastFrameActive >=
       g.FrameCount - 2); // Note that fully clipped early out scrolling tables
                          // will appear as inactive here.
  if (!is_active) {
...
void ImGui::SetColumnOffset(int column_index, float offset) {
  ...
  if (!(columns->Flags & ImGuiOldColumnFlags_NoForceWithinWindow))
    offset =
        ImMin(offset, columns->OffMaxX - g.Style.ColumnsMinSpacing *
