...
// imconfig file has '#define ImTextureID ImU64'. This is because we need
...
// 'IMGUI_USER_CONFIG="my_imgui_config.h"' and inside 'my_imgui_config.h' add
// '#define ImTextureID ImU64' and as many other options as you like.
// - [Solution 3] IDE/msbuild: edit imconfig.h and add '#define ImTextureID
// ImU64' (prefer solution 2 to create your own config file!)
// - [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe
...
//  ImGuiPlatformIO interface. 2024-04-19: Vulkan: Added convenience support for
//  Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via
//  ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved
//  RenderPass parameter from ImGui_ImplVulkan_Init() function to
//  ImGui_ImplVulkan_InitInfo structure. Not required when using dynamic
//  rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require
//  filling PipelineRenderingCreateInfo structure. 2024-01-19: Vulkan: Fixed
//  vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating
//  one extra semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed
//  vkMapMemory() calls unnecessarily using full buffer size (#3957). Fixed
//  MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added
//  MinAllocationSize field in ImGui_ImplVulkan_InitInfo to workaround zealous
//  "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stopped
//  creating command pools with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT
//  as we don't reset them. 2023-11-29: Vulkan: Fixed mismatching allocator
//  passed to vkCreateCommandPool() vs vkDestroyCommandPool(). (#7075)
//  2023-11-10: *BREAKING CHANGE*: Removed parameter from
//  ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own
//  command-buffer to upload fonts.
//              *BREAKING CHANGE*: Removed
//              ImGui_ImplVulkan_DestroyFontUploadObjects() which is now
//              unecessary as we create and destroy those objects in the
//              backend. ImGui_ImplVulkan_CreateFontsTexture() is automatically
//              called by NewFrame() the first time. You can call
//              ImGui_ImplVulkan_CreateFontsTexture() again to recreate the font
//              atlas texture. Added ImGui_ImplVulkan_DestroyFontsTexture() but
...
//  to ImGui_ImplVulkan_AddTexture() not being honored + removed a bunch of
//  duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our
//  switch to C++11. 2022-10-04: Vulkan: Added experimental
//  ImGui_ImplVulkan_RemoveTexture() for api symetry. (#914, #5738). 2022-01-20:
//  Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call
//  ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define
//  ImTextureID ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at
...
//  Added support for custom function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES
//  by using ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support
//  for specifying which subpass to reference during VkPipeline creation.
//  2020-09-07: Vulkan: Added VkPipeline parameter to
//  ImGui_ImplVulkan_RenderDrawData (default to one passed to
//  ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial frame has
//  no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks
//  wouldn't be called if the ImDrawData didn't have vertices. 2019-08-01:
//  Vulkan: Added support for specifying multisample count. Set
//  ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits
//  values to use, default is non-multisampled as before. 2019-05-29: Vulkan:
//  Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added
...
//  fields in ImGui_ImplVulkan_InitInfo, required for initialization (was
//  previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added
//  ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added VkInstance
//  argument to ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04:
...
//  buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan: Viewport and
...
//  parameters to ImGui_ImplVulkan_RenderDrawData() to be consistent with other
//  backends. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the
...
//  couple of ImGui_ImplVulkanH_XXX helper that the example can use and that
//  viewport support will use. 2018-03-01: Vulkan: Renamed
//  ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields to match
//  more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the
//  io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls
//  ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06: Misc: Removed call to
...
void ImGui_ImplVulkan_DestroyFrameRenderBuffers(
    VkDevice device, ImGui_ImplVulkan_FrameRenderBuffers *buffers,
    const VkAllocationCallbacks *allocator);
void ImGui_ImplVulkan_DestroyWindowRenderBuffers(
    VkDevice device, ImGui_ImplVulkan_WindowRenderBuffers *buffers,
    const VkAllocationCallbacks *allocator);
...
void ImGui_ImplVulkanH_DestroyFrameSemaphores(
    ...
    const VkAllocationCallbacks *allocator);
void ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(
...
struct ImGui_ImplVulkan_ViewportData {
  ...
  bool WindowOwned;
  bool SwapChainNeedRebuild; // Flag when viewport swapchain resized in the
                             // middle of processing a frame
...
static ImGui_ImplVulkan_Data *ImGui_ImplVulkan_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplVulkan_Data *)ImGui::GetIO().BackendRendererUserData
             : nullptr;
}
...
static void CreateOrResizeBuffer(VkBuffer &buffer,
  ...
  VkDeviceSize buffer_size_aligned = AlignBufferSize(
      IM_MAX(v->MinAllocationSize, new_size), bd->BufferMemoryAlignment);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData *draw_data,
  ...
  ImGui_ImplVulkan_WindowRenderBuffers *wrb =
      &viewport_renderer_data->RenderBuffers;
  if (wrb->FrameRenderBuffers == nullptr) {
    wrb->Index = 0;
    wrb->Count = v->ImageCount;
    wrb->FrameRenderBuffers = (ImGui_ImplVulkan_FrameRenderBuffers *)IM_ALLOC(
        sizeof(ImGui_ImplVulkan_FrameRenderBuffers) * wrb->Count);
    memset(wrb->FrameRenderBuffers, 0,
           sizeof(ImGui_ImplVulkan_FrameRenderBuffers) * wrb->Count);
  }
  IM_ASSERT(wrb->Count == v->ImageCount);
  wrb->Index = (wrb->Index + 1) % wrb->Count;
  ImGui_ImplVulkan_FrameRenderBuffers *rb =
      &wrb->FrameRenderBuffers[wrb->Index];

  if (draw_data->TotalVtxCount > 0) {
    // Create or resize the vertex/index buffers
    size_t vertex_size =
        AlignBufferSize(draw_data->TotalVtxCount * sizeof(ImDrawVert),
                        bd->BufferMemoryAlignment);
    size_t index_size =
        AlignBufferSize(draw_data->TotalIdxCount * sizeof(ImDrawIdx),
                        bd->BufferMemoryAlignment);
    ...
    VkResult err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0,
                               vertex_size, 0, (void **)&vtx_dst);
    check_vk_result(err);
    err = vkMapMemory(v->Device, rb->IndexBufferMemory, 0, index_size, 0,
                      (void **)&idx_dst);
    check_vk_result(err);
  ...
  ImGui_ImplVulkan_SetupRenderState(draw_data, pipeline, command_buffer, rb,
                                    fb_width, fb_height);
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
          ImGui_ImplVulkan_SetupRenderState(draw_data, pipeline, command_buffer,
...
bool ImGui_ImplVulkan_CreateFontsTexture() {
  ...
  bd->FontDescriptorSet = (VkDescriptorSet)ImGui_ImplVulkan_AddTexture(
...
// ImGui_ImplVulkan_CreateFontsTexture() and ImGui_ImplVulkan_Shutdown().
...
static void
ImGui_ImplVulkan_CreateShaderModules(VkDevice device,
                                     const VkAllocationCallbacks *allocator) {
...
static void ImGui_ImplVulkan_CreatePipeline(
    ...
    VkSampleCountFlagBits MSAASamples, VkPipeline *pipeline, uint32_t subpass) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  ...
  if (bd->VulkanInitInfo.UseDynamicRendering) {
    IM_ASSERT(bd->VulkanInitInfo.PipelineRenderingCreateInfo.sType ==
                  VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR &&
              "PipelineRenderingCreateInfo sType must be "
              "VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR");
    IM_ASSERT(bd->VulkanInitInfo.PipelineRenderingCreateInfo.pNext == nullptr &&
              "PipelineRenderingCreateInfo pNext must be NULL");
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache,
                                  v->RenderPass, v->MSAASamples, &bd->Pipeline,
                                  v->Subpass);
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo *info) {
  IM_ASSERT(g_FunctionsLoaded &&
            "Need to call ImGui_ImplVulkan_LoadFunctions() if "
...
#else
    IM_ASSERT(0 && "Can't use dynamic rendering when neither VK_VERSION_1_3 or "
...
void ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count) {
  ...
  IM_ASSERT(
      0); // FIXME-VIEWPORT: Unsupported. Need to recreate all swap chains!
  ImGui_ImplVulkan_InitInfo *v = &bd->VulkanInitInfo;
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler,
...
int ImGui_ImplVulkanH_GetMinImageCountFromPresentMode(
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
  ...
  // Create Swapchain
  {
    ...
    wd->Frames = (ImGui_ImplVulkanH_Frame *)IM_ALLOC(
        sizeof(ImGui_ImplVulkanH_Frame) * wd->ImageCount);
    wd->FrameSemaphores = (ImGui_ImplVulkanH_FrameSemaphores *)IM_ALLOC(
        sizeof(ImGui_ImplVulkanH_FrameSemaphores) * wd->SemaphoreCount);
    memset(wd->Frames, 0, sizeof(wd->Frames[0]) * wd->ImageCount);
    memset(wd->FrameSemaphores, 0,
           sizeof(wd->FrameSemaphores[0]) * wd->SemaphoreCount);
    for (uint32_t i = 0; i < wd->ImageCount; i++)
  ...
  if (wd->UseDynamicRendering == false) {
    ...
    // wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, v->Subpass);
  }
...
void ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n++)
    if (ImGui_ImplVulkan_ViewportData *vd =
            (ImGui_ImplVulkan_ViewportData *)platform_io.Viewports[n]
                ->RendererUserData)
      ImGui_ImplVulkan_DestroyWindowRenderBuffers(device, &vd->RenderBuffers,
                                                  allocator);
}
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport *viewport) {
  ...
  VkResult err = (VkResult)platform_io.Platform_CreateVkSurface(
      viewport, (ImU64)v->Instance, (const void *)v->Allocator,
      (ImU64 *)&wd->Surface);
  check_vk_result(err);
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
  for (uint32_t n = 0; n < v->PipelineRenderingCreateInfo.colorAttachmentCount;
       n++)
    requestSurfaceImageFormats.push_back(
        v->PipelineRenderingCreateInfo.pColorAttachmentFormats[n]);
#endif
  const VkFormat defaultFormats[] = {
  ...
  wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
      v->PhysicalDevice, wd->Surface, requestSurfaceImageFormats.Data,
      (size_t)requestSurfaceImageFormats.Size, requestSurfaceColorSpace);
  ...
  wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
      v->PhysicalDevice, wd->Surface, &present_modes[0],
  ...
  wd->ClearEnable =
      (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? false : true;
  wd->UseDynamicRendering = v->UseDynamicRendering;
  ImGui_ImplVulkanH_CreateOrResizeWindow(
      v->Instance, v->PhysicalDevice, v->Device, wd, v->QueueFamily,
      v->Allocator, (int)viewport->Size.x, (int)viewport->Size.y,
      v->MinImageCount);
  vd->WindowOwned = true;

  // Create pipeline (shared by all secondary viewports)
  if (bd->PipelineForViewports == VK_NULL_HANDLE)
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, VK_NULL_HANDLE,
                                    wd->RenderPass, VK_SAMPLE_COUNT_1_BIT,
...
static void ImGui_ImplVulkan_DestroyWindow(ImGuiViewport *viewport) {
  ...
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  if (ImGui_ImplVulkan_ViewportData *vd =
          (ImGui_ImplVulkan_ViewportData *)viewport->RendererUserData) {
    ImGui_ImplVulkan_InitInfo *v = &bd->VulkanInitInfo;
    if (vd->WindowOwned)
      ImGui_ImplVulkanH_DestroyWindow(v->Instance, v->Device, &vd->Window,
                                      v->Allocator);
    ImGui_ImplVulkan_DestroyWindowRenderBuffers(v->Device, &vd->RenderBuffers,
                                                v->Allocator);
    IM_DELETE(vd);
  }
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport *viewport,
                                           ImVec2 size) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  ImGui_ImplVulkan_ViewportData *vd =
      (ImGui_ImplVulkan_ViewportData *)viewport->RendererUserData;
  ...
  vd->Window.ClearEnable =
      (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? false : true;
  ImGui_ImplVulkanH_CreateOrResizeWindow(
      v->Instance, v->PhysicalDevice, v->Device, &vd->Window, v->QueueFamily,
      v->Allocator, (int)size.x, (int)size.y, v->MinImageCount);
}
  ...
  {
    {
      ...
      if (err == VK_ERROR_OUT_OF_DATE_KHR) {
        // Since we are not going to swap this frame anyway, it's ok that
        // recreation happens on next frame.
    ...
    if (v->UseDynamicRendering) {
      ...
      ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR(fd->CommandBuffer,
                                                  &renderingInfo);
    } else
#endif
    {
      ...
      info.clearValueCount =
          (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? 0 : 1;
      info.pClearValues = (viewport->Flags & ImGuiViewportFlags_NoRendererClear)
                              ? nullptr
                              : &wd->ClearValue;
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport *viewport, void *) {
  ...
  if (vd->SwapChainNeedRebuild) // Frame data became invalid in the middle of
                                // rendering
    return;
...
void ImGui_ImplVulkan_InitPlatformInterface() {
  ...
  if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    IM_ASSERT(platform_io.Platform_CreateVkSurface != nullptr &&
