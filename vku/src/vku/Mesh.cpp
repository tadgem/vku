...
static vku::StaticVector<uint32_t> g_ScreenSpaceQuadIndexData = {0, 1, 2,
                                                                 2, 3, 0};

void vku::Mesh::InitBuiltInMeshes(vku::VkState &vk) {
  Buffer vertexBuffer = buffers::CreateVertexBuffer<VertexDataPosUv>(
      vk, g_ScreenSpaceQuadVertexData.data(),
      g_ScreenSpaceQuadVertexData.size());
  Buffer indexBuffer = buffers::CreateIndexBuffer(
      vk, g_ScreenSpaceQuadIndexData.data(), g_ScreenSpaceQuadIndexData.size());
...
vku::VertexDescription
vku::VertexDataPosColUv::GetVertexDescription(IAllocator &alloc) {
  GET_VERTEX_DESCRIPTION_IMPL;
}
vku::VertexDescription
vku::VertexDataPos4::GetVertexDescription(IAllocator &alloc) {
  GET_VERTEX_DESCRIPTION_IMPL;
}
vku::VertexDescription
vku::VertexDataPosUv::GetVertexDescription(IAllocator &alloc) {
  GET_VERTEX_DESCRIPTION_IMPL;
}
vku::VertexDescription
vku::VertexDataPosNormalUv::GetVertexDescription(IAllocator &alloc) {
  GET_VERTEX_DESCRIPTION_IMPL;
}
