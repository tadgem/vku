...
void vku::init::CleanupNanoVG(VkState &vk) { nvgDeleteVk(vk.m_NanoVG); }
...
bool vku::init::CheckDeviceExtensionSupport(VkState &vk,
...
void vku::init::SetupDebugOutput(VkState &vk) {
  ...
  PFN_vkVoidFunction rawFunction =
      vkGetInstanceProcAddr(vk.m_Instance, "vkCreateDebugUtilsMessengerEXT");
  PFN_vkCreateDebugUtilsMessengerEXT function =
      reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(rawFunction);
...
void vku::init::CleanupDebugOutput(VkState &vk) {
  PFN_vkVoidFunction rawFunction =
      vkGetInstanceProcAddr(vk.m_Instance, "vkDestroyDebugUtilsMessengerEXT");
  auto function =
      reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(rawFunction);
...
void vku::init::ListDeviceExtensions(VkState &vk,
  ...
  VKU_LOG_INFO("Selected device %s supports the following extensions:",
...
void vku::init::InitImGui(VkState &vk) {
  ...
  init_info.QueueFamily =
      vk.m_QueueFamilyIndices
          .m_QueueFamilies[QueueFamilyType::GraphicsAndCompute];
  init_info.Queue = vk.m_GraphicsQueue;
  init_info.PipelineCache = VK_NULL_HANDLE;
  // TODO: this is a bit shit, need to clean this pool some wheere
  init_info.DescriptorPool = vk.m_DescriptorSetAllocator.CreatePool(
      *vk.m_CPUAllocator, vk.m_LogicalDevice, 4096);
...
void vku::init::InitNanoVG(VkState &vk) {
  ...
  nvgCreateInfo.swapchainImageCount =
      static_cast<uint32_t>(vk.m_SwapChainImages.size());
  ...
  nvgCreateInfo.ext.sampleCount =
      vk.m_UseSwapchainMsaa ? vk.m_MaxMsaaSamples : VK_SAMPLE_COUNT_1_BIT;
...
void vku::init::CreateInstance(VkState &vk) {
  ...
  Vector<const char *> extensionNames =
      vk.m_Backend->GetRequiredInstanceExtensions(vk);
  ...
  // setup an m_Instance create info with our extensions & app info to create a
  // vulkan m_Instance
...
void vku::init::CleanupVulkan(VkState &vk) {
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(vk.m_LogicalDevice, vk.m_ImageAvailableSemaphores[i],
                       nullptr);
    vkDestroySemaphore(vk.m_LogicalDevice, vk.m_RenderFinishedSemaphores[i],
                       nullptr);
    vkDestroyFence(vk.m_LogicalDevice, vk.m_FrameInFlightFences[i], nullptr);
  }

  vkDestroyCommandPool(vk.m_LogicalDevice, vk.m_GraphicsComputeQueueCommandPool,
                       nullptr);
  vkDestroyRenderPass(vk.m_LogicalDevice, vk.m_SwapchainImageRenderPass,
                      nullptr);
...
vku::QueueFamilyIndices
vku::init::FindQueueFamilies(VkState &vk, VkPhysicalDevice m_PhysicalDevice) {
  ...
  for (int i = 0; i < queueFamilyProperties.size(); i++) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(m_PhysicalDevice, i, vk.m_Surface,
                                         &presentSupport);
...
vku::SwapChainSupportDetais
vku::init::GetSwapChainSupportDetails(VkState &vk,
                                      VkPhysicalDevice physicalDevice) {
  SwapChainSupportDetais details(*vk.m_CPUAllocator);

  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, vk.m_Surface,
                                            &details.m_Capabilities);

  uint32_t supportedNumberFormats = 0;
  vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, vk.m_Surface,
                                       &supportedNumberFormats, nullptr);
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, vk.m_Surface,
                                            &supportedPresentModes, nullptr);

  if (supportedNumberFormats > 0) {
    details.m_SupportedPresentModes.resize(supportedPresentModes);
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        physicalDevice, vk.m_Surface, &supportedPresentModes,
        details.m_SupportedPresentModes.data());
  }
...
bool vku::init::IsDeviceSuitable(VkState &vk, VkPhysicalDevice physicalDevice) {
  ...
  if (extensionsSupported) {
    SwapChainSupportDetais swapChainDetails =
        GetSwapChainSupportDetails(vk, physicalDevice);
    swapChainSupport = swapChainDetails.m_SupportedFormats.size() > 0 &&
                       swapChainDetails.m_SupportedPresentModes.size() > 0;
  }
...
void vku::init::PickPhysicalDevice(VkState &vk) {
  ...
  vkEnumeratePhysicalDevices(vk.m_Instance, &deviceCount,
...
void vku::init::CreateLogicalDevice(VkState &vk) {
  ...
  if (vkCreateDevice(vk.m_PhysicalDevice, &createInfo, nullptr,
                     &vk.m_LogicalDevice)) {
...
void vku::init::GetQueueHandles(VkState &vk) {
  vkGetDeviceQueue(vk.m_LogicalDevice,
                   vk.m_QueueFamilyIndices
                       .m_QueueFamilies[QueueFamilyType::GraphicsAndCompute],
                   0, &vk.m_GraphicsQueue);
  vkGetDeviceQueue(vk.m_LogicalDevice,
                   vk.m_QueueFamilyIndices
                       .m_QueueFamilies[QueueFamilyType::GraphicsAndCompute],
                   0, &vk.m_ComputeQueue);
  vkGetDeviceQueue(
      vk.m_LogicalDevice,
      vk.m_QueueFamilyIndices.m_QueueFamilies[QueueFamilyType::Present], 0,
      &vk.m_PresentQueue);
}
...
VkSurfaceFormatKHR vku::init::ChooseSwapChainSurfaceFormat(
  ...
  if (availableFormats.size() == 0) {
    VKU_LOG_ERR("Could not find any suitable Swapchain Surface Format in "
                "provided collection!");
  }
  ...
  VKU_LOG_ERR("Could not find suitable Swapchain Surface Format in provided "
              "collection!");
  return availableFormats[0];
}

VkPresentModeKHR
vku::init::ChooseSwapChainPresentMode(VkState &vk,
...
void vku::init::CreateSwapChain(VkState &vk) {
  SwapChainSupportDetais swapChainDetails =
      GetSwapChainSupportDetails(vk, vk.m_PhysicalDevice);

  VkSurfaceFormatKHR format =
      ChooseSwapChainSurfaceFormat(swapChainDetails.m_SupportedFormats);
  VkPresentModeKHR presentMode =
      ChooseSwapChainPresentMode(vk, swapChainDetails.m_SupportedPresentModes);
  VkExtent2D surfaceExtent =
      ChooseSwapExtent(vk, swapChainDetails.m_Capabilities);
  ...
  // Therefore it is recommended to request at least one more image than the
  // minimum"
  ...
  uint32_t queueFamilyIndices[] = {
      vk.m_QueueFamilyIndices
          .m_QueueFamilies[QueueFamilyType::GraphicsAndCompute],
      vk.m_QueueFamilyIndices.m_QueueFamilies[QueueFamilyType::Present]};

  if (vk.m_QueueFamilyIndices
          .m_QueueFamilies[QueueFamilyType::GraphicsAndCompute] !=
      vk.m_QueueFamilyIndices.m_QueueFamilies[QueueFamilyType::Present]) {
  ...
  // the alpha channel, hence OPAQUE_BIT_KHR.
  createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
  createInfo.oldSwapchain = VK_NULL_HANDLE;

  if (vkCreateSwapchainKHR(vk.m_LogicalDevice, &createInfo, nullptr,
                           &vk.m_SwapChain) != VK_SUCCESS) {
    VKU_LOG_ERR("Failed to create swapchain.");
  }

  vkGetSwapchainImagesKHR(vk.m_LogicalDevice, vk.m_SwapChain,
                          &swapChainImageCount, nullptr);
  vk.m_SwapChainImages.resize(swapChainImageCount);
  vkGetSwapchainImagesKHR(vk.m_LogicalDevice, vk.m_SwapChain,
                          &swapChainImageCount, vk.m_SwapChainImages.data());
...
void vku::init::CreateSwapChainFramebuffers(VkState &vk) {
  ...
  for (uint32_t i = 0; i < vk.m_SwapChainImageViews.size(); i++) {
    ...
    textures::CreateFramebuffer(vk, attachments, vk.m_SwapchainImageRenderPass,
                                vk.m_SwapChainImageExtent,
                                vk.m_SwapChainFramebuffers[i]);
  }
}
...
void vku::init::CreateSwapChainImageViews(VkState &vk) {
  ...
  for (uint32_t i = 0; i < vk.m_SwapChainImages.size(); i++) {
    textures::CreateImageView(
        vk, vk.m_SwapChainImages[i], vk.m_SwapChainImageFormat, 1,
        VK_IMAGE_ASPECT_COLOR_BIT, vk.m_SwapChainImageViews[i]);
  }
}

void vku::init::CleanupSwapChain(VkState &vk) {
  for (int i = 0; i < vk.m_SwapChainFramebuffers.size(); i++) {
    vkDestroyFramebuffer(vk.m_LogicalDevice, vk.m_SwapChainFramebuffers[i],
                         nullptr);
  }
  for (int i = 0; i < vk.m_SwapChainImageViews.size(); i++) {
    vkDestroyImageView(vk.m_LogicalDevice, vk.m_SwapChainImageViews[i],
                       nullptr);
  }

  vkDestroyImageView(vk.m_LogicalDevice, vk.m_SwapChainColourImageView,
                     nullptr);
...
void vku::init::CreateSwapChainColourTexture(VkState &vk, bool enableMsaa) {
  ...
  textures::CreateImage(
      vk, vk.m_SwapChainImageExtent.width, vk.m_SwapChainImageExtent.height, 1,
      sampleCount, vk.m_SwapChainImageFormat, VK_IMAGE_TILING_OPTIMAL,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
      vk.m_SwapChainColourImage, vk.m_SwapChainColourImageMemory);

  textures::CreateImageView(
      vk, vk.m_SwapChainColourImage, vk.m_SwapChainImageFormat, 1,
      VK_IMAGE_ASPECT_COLOR_BIT, vk.m_SwapChainColourImageView);
}
...
void vku::init::CreateSwapChainDepthTexture(VkState &vk, bool enableMsaa) {
  ...
  textures::CreateImage(
      ...
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vk.m_SwapChainDepthImage,
      vk.m_SwapChainDepthImageMemory);

  textures::CreateImageView(vk, vk.m_SwapChainDepthImage, depthFormat, 1,
                            VK_IMAGE_ASPECT_DEPTH_BIT,
                            vk.m_SwapChainDepthImageView);
  textures::TransitionImageLayout(
      vk, vk.m_SwapChainDepthImage, depthFormat, 1, VK_IMAGE_LAYOUT_UNDEFINED,
      VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
}

VkExtent2D
vku::init::ChooseSwapExtent(VkState &vk,
...
void vku::init::CreateCommandPool(VkState &vk) {
  ...
  if (vkCreateCommandPool(vk.m_LogicalDevice, &createInfo, nullptr,
                          &vk.m_GraphicsComputeQueueCommandPool) !=
      VK_SUCCESS) {
...
void vku::init::CreateDescriptorSetAllocator(VkState &vk) {
  STLAllocator<DescriptorSetAllocator::PoolSizeRatio> ratioAlloc(
      *vk.m_CPUAllocator);
  ...
  vk.m_DescriptorSetAllocator.Init(*vk.m_CPUAllocator, vk.m_LogicalDevice,
                                   MAX_FRAMES_IN_FLIGHT * 128, ratios);
}
...
void vku::init::CreateSemaphores(VkState &vk) {
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(vk.m_LogicalDevice, &createInfo, nullptr,
                          &vk.m_ImageAvailableSemaphores[i]) != VK_SUCCESS ||
        vkCreateSemaphore(vk.m_LogicalDevice, &createInfo, nullptr,
                          &vk.m_RenderFinishedSemaphores[i]) != VK_SUCCESS ||
        vkCreateSemaphore(vk.m_LogicalDevice, &createInfo, nullptr,
                          &vk.m_ComputeFinishedSemaphores[i]) != VK_SUCCESS) {
...
void vku::init::CreateFences(VkState &vk) {
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateFence(vk.m_LogicalDevice, &createInfo, nullptr,
                      &vk.m_FrameInFlightFences[i]) != VK_SUCCESS ||
        vkCreateFence(vk.m_LogicalDevice, &createInfo, nullptr,
                      &vk.m_ComputeInFlightFences[i]) != VK_SUCCESS) {
...
void vku::init::CreateCommandBuffers(VkState &vk) {
  ...
  VK_CHECK(vkAllocateCommandBuffers(vk.m_LogicalDevice, &allocateInfo,
                                    vk.m_GraphicsCommandBuffers.data()))
  VK_CHECK(vkAllocateCommandBuffers(vk.m_LogicalDevice, &allocateInfo,
                                    vk.m_ComputeCommandBuffers.data()))
}
...
void vku::init::GetMaxUsableSampleCount(VkState &vk) {
  ...
  if (counts & VK_SAMPLE_COUNT_64_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_64_BIT;
    return;
  }
  if (counts & VK_SAMPLE_COUNT_32_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_32_BIT;
    return;
  }
  if (counts & VK_SAMPLE_COUNT_16_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_16_BIT;
    return;
  }
  if (counts & VK_SAMPLE_COUNT_8_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_8_BIT;
    return;
  }
  if (counts & VK_SAMPLE_COUNT_4_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_4_BIT;
    return;
  }
  if (counts & VK_SAMPLE_COUNT_2_BIT) {
    vk.m_MaxMsaaSamples = VK_SAMPLE_COUNT_2_BIT;
    return;
  }
...
void vku::init::CreateBuiltInRenderPasses(vku::VkState &vk) {
  STLAllocator<VkAttachmentDescription> ada(*vk.m_CPUAllocator);
  {
    ...
    depthAttachmentDescription.samples =
        vk.m_UseSwapchainMsaa ? vk.m_MaxMsaaSamples : VK_SAMPLE_COUNT_1_BIT;
    ;
    ...
    render_passes::CreateRenderPass(
        vk, vk.m_SwapchainImageRenderPass, colourAttachmentDescriptions,
        resolveAttachmentDescriptions, true, depthAttachmentDescription,
  ...
  }
  {
    ...
    render_passes::CreateRenderPass(
        vk, vk.m_ImGuiRenderPass, colourAttachmentDescriptions,
        resolveAttachmentDescriptions, true, depthAttachmentDescription,
...
vku::init::GetDeviceAvailableExtensions(IAllocator &alloc,
                                        VkPhysicalDevice physicalDevice) {
  using namespace vku;
  uint32_t extensionCount;
...
void vku::init::Quit(vku::VkState &vk) { vk.m_ShouldRun = false; }
