...
ViewData CreateView(VkState &vk, VkuIm3dState im3dState,
                    ShaderProgram gbufferProg, ShaderProgram lightPassProg) {
  Framebuffer gbuffer(*vk.m_CPUAllocator);
  gbuffer.AddColourAttachment(
      vk, ResolutionScale::Full, 1, VK_FORMAT_R16G16B16A16_SFLOAT,
  ...
  gbuffer.AddDepthAttachment(
      vk, ResolutionScale::Full, 1,
      VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SAMPLE_COUNT_1_BIT);
  gbuffer.Build(vk);

  Framebuffer finalImage(*vk.m_CPUAllocator);
  finalImage.AddColourAttachment(
      vk, ResolutionScale::Full, 1, VK_FORMAT_R8G8B8A8_UNORM,
      VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SAMPLE_COUNT_1_BIT);
  finalImage.Build(vk);
  ...
  StaticVector<uint32_t> screenQuadIndices = {0, 1, 2, 2, 3, 0};

  Buffer vertexBuffer = buffers::CreateVertexBuffer<VertexDataPosUv>(
      vk, screenQuadVerts.data(), screenQuadVerts.size());
  Buffer indexBuffer = buffers::CreateIndexBuffer(vk, screenQuadIndices.data(),
                                                  screenQuadIndices.size());
  ...
  return {gbuffer,
          ...
          lightPassMat,
          im3dViewState,
          deferredIm3dViewState,
          {1920, 1080},
          {},
          screenQuad};
}

RenderData CreateRenderData(VkState &vk, ShaderProgram gbufferProg,
  ...
  auto vertexDescription =
      VertexDataPosNormalUv::GetVertexDescription(*vk.m_CPUAllocator);
  VkPipelineData gbufferPipeline = vku::pipelines::CreateDynamicRasterPipeline(
      vk, gbufferProg, vertexDescription, defaults::DefaultRasterState,
  ...
  auto presentVertexDescription =
      VertexDataPosUv::GetVertexDescription(*vk.m_CPUAllocator);
  RasterizationState lightPassRasterState{
      VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, false, VK_COMPARE_OP_NEVER,
      VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST};
  VkPipelineData pipeline = vku::pipelines::CreateDynamicRasterPipeline(
      vk, lightPassProg, presentVertexDescription, lightPassRasterState,
...
void UpdateViewData(VkState &vk, ViewData *view, DeferredLightData &lightData) {
  glm::quat qPitch = glm::angleAxis(glm::radians(-view->m_Camera.Rotation.x),
                                    glm::vec3(1, 0, 0));
  glm::quat qYaw = glm::angleAxis(glm::radians(view->m_Camera.Rotation.y),
                                  glm::vec3(0, 1, 0));
  // omit roll
  ...
  if (vk.m_SwapChainImageExtent.width > 0 || vk.m_SwapChainImageExtent.height) {
    view->m_Camera.Proj =
        glm::perspective(glm::radians(45.0f),
                         (float)view->m_CurrentResolution.width /
                             (float)view->m_CurrentResolution.height,
                         0.1f, 10000.0f);
...
void RecordCommandBuffersV2(VkState &vk, Vector<ViewData *> views,
                            RenderData &renderData, RenderModel &model,
                            Mesh &screenQuad, VkuIm3dState &im3dState,
  ...
  static StaticVector<VertexDataPosUv> originalScreenQuadData = {
      ...
      {{1.0f, 1.0f, 0.0f}, {1.0, 1.0}},
      {{-1.0f, 1.0f, 0.0f}, {0.0f, 1.0}}};
  vku::commands::RecordGraphicsCommands(vk, [&](VkCommandBuffer &commandBuffer,
                                                uint32_t frameIndex) {
    {
    ...
    for (auto &view : views) {
      ...
      // update screen quad
      {
        ...
        float w =
            static_cast<float>((float)viewExtent.width / (float)max.width);
        float h =
            static_cast<float>((float)viewExtent.height / (float)max.height);
        ...
        vkCmdUpdateBuffer(commandBuffer,
                          view->m_ViewQuad.m_VertexBuffer.m_GpuBuffer, 0,
                          4 * sizeof(VertexDataPosUv), &newScreenQuadData[0]);
      }

      {
        debug::BeginDebugMarker(commandBuffer, "GBuffer",
                                {0.0f, 1.0f, 0.0f, 1.0f});
        view->m_GBuffer.BeginDynamicRendering(vk, commandBuffer, frameIndex);
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          renderData.m_GBufferPipeline.m_Pipeline);
        VkViewport viewport{};
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        ...
        vkCmdPushConstants(
            commandBuffer, renderData.m_GBufferPipeline.m_PipelineLayout,
            VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PCViewData), &pcData);
        ...
        for (int i = 0; i < model.m_RenderItems.size(); i++) {
          ...
          vkCmdBindIndexBuffer(commandBuffer,
                               mesh.m_Mesh.m_IndexBuffer.m_GpuBuffer, 0,
                               VK_INDEX_TYPE_UINT32);
          vkCmdBindDescriptorSets(
              commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
              renderData.m_GBufferPipeline.m_PipelineLayout, 0, 1,
              &model.m_RenderItems[i]
                   .m_Material.m_DescriptorSets[0]
                   .m_Sets[frameIndex],
              0, nullptr);
      ...
      debug::BeginDebugMarker(commandBuffer, "Lighting Pass",
                              {1.0f, 1.0f, 0.0f, 1.0f});
      // vkCmdBeginRenderingKHR(commandBuffer,
      // &view->m_LightPassFB.m_DynamicRenderingInfo.m_RenderingInfos[frameIndex]);
      view->m_LightPassFB.BeginDynamicRendering(vk, commandBuffer, frameIndex);

      vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        renderData.m_LightPassPipeline.m_Pipeline);
      VkViewport viewport{};
      viewport.x = 0.0f;
      viewport.y = 0.0f;
      viewport.width = static_cast<float>(viewExtent.width);
      ...
      vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
      vkCmdPushConstants(
          commandBuffer, renderData.m_LightPassPipeline.m_PipelineLayout,
          VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PCViewData), &pcData);
      VkDeviceSize sizes[] = {0};
      vkCmdBindVertexBuffers(commandBuffer, 0, 1,
                             &view->m_ViewQuad.m_VertexBuffer.m_GpuBuffer,
                             sizes);
      vkCmdBindIndexBuffer(commandBuffer,
                           view->m_ViewQuad.m_IndexBuffer.m_GpuBuffer, 0,
                           VK_INDEX_TYPE_UINT32);
      vkCmdBindDescriptorSets(
          commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
          renderData.m_LightPassPipeline.m_PipelineLayout, 0, 1,
          &view->m_LightPassMaterial.m_DescriptorSets[0].m_Sets[frameIndex], 0,
          nullptr);
      ...
      {
        debug::BeginDebugMarker(commandBuffer, "Im3D Deferred Pass",
                                {0.0f, 0.0f, 1.0f, 1.0f});
        VkRenderingAttachmentInfoKHR colourInfo{};
        colourInfo.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
        colourInfo.imageView = view->m_LightPassFB.m_ColourAttachments[0]
                                   .m_AttachmentSwapchainImages[frameIndex]
                                   .m_ImageView;
        ...
        depthInfo.imageView = view->m_GBuffer.m_DepthAttachments[0]
                                  .m_AttachmentSwapchainImages[frameIndex]
                                  .m_ImageView;
        ...
        DrawIm3d(vk, commandBuffer, frameIndex, im3dState,
                 view->m_DeferredIm3dState,
                 *reinterpret_cast<glm::mat4 *>(&viewProjDeferred),
                 viewExtent.width, viewExtent.height);
...
RenderModel CreateRenderModelGbuffer(VkState &vk, const char *modelPath,
  ...
  for (auto &mesh : model.m_Meshes) {
    ...
    int materialIndex =
        std::min(mesh.m_MaterialIndex, (uint32_t)model.m_Materials.size() - 1);
    item.m_Mesh = mesh;
...
void OnImGui(VkState &vk, DeferredLightData &lightDataCpu,
  ...
  if (ImGui::Begin("View 1")) {
    ...
    auto &image = views[0]
                      ->m_LightPassFB.m_ColourAttachments[0]
  ...
  if (ImGui::Begin("Lights Menu")) {
    ImGui::Text("Frametime: %f", (1.0 / vk.m_DeltaTime));
    ImGui::DragFloat3("Directional Light Dir",
                      &lightDataCpu.m_DirectionalLight.Direction[0]);
    ImGui::DragFloat4("Directional Light Colour",
                      &lightDataCpu.m_DirectionalLight.Colour[0]);
    ImGui::DragFloat4("Directional Light Ambient Colour",
                      &lightDataCpu.m_DirectionalLight.Ambient[0]);
      ...
      for (int i = 0; i < NUM_LIGHTS; i++) {
        ...
        if (ImGui::TreeNode("Point Light")) {
          ImGui::DragFloat3("Position",
                            &lightDataCpu.m_PointLights[i].PositionRadius[0]);
          ImGui::DragFloat("Radius",
                           &lightDataCpu.m_PointLights[i].PositionRadius[3]);
          ImGui::DragFloat4("Colour", &lightDataCpu.m_PointLights[i].Colour[0]);
          ImGui::DragFloat4("Ambient Colour",
                            &lightDataCpu.m_PointLights[i].Ambient[0]);
    ...
    if (ImGui::TreeNode("Spot Lights")) {
      for (int i = 0; i < NUM_LIGHTS; i++) {
        ...
        if (ImGui::TreeNode("Spot Light")) {
          ImGui::DragFloat3("Position",
                            &lightDataCpu.m_SpotLights[i].PositionRadius[0]);
          ImGui::DragFloat("Radius",
                           &lightDataCpu.m_SpotLights[i].PositionRadius[3]);
          ImGui::DragFloat3("Direction",
                            &lightDataCpu.m_SpotLights[i].DirectionAngle[0]);
          ImGui::DragFloat("Angle",
                           &lightDataCpu.m_SpotLights[i].DirectionAngle[3]);
          ImGui::DragFloat4("Colour", &lightDataCpu.m_SpotLights[i].Colour[0]);
          ImGui::DragFloat4("Ambient Colour",
                            &lightDataCpu.m_SpotLights[i].Ambient[0]);
...
int main() {
  ...
  while (vk.m_Backend->ShouldRun(vk)) {
    ...
    RecordCommandBuffersV2(vk, views, renderData, m, *Mesh::g_ScreenSpaceQuad,
                           im3dState, lightDataCpu);
