...
glm::vec3 CalculateVec3Radians(glm::vec3 eulerDegrees) {
  return glm::vec3(glm::radians(eulerDegrees.x), glm::radians(eulerDegrees.y),
                   glm::radians(eulerDegrees.z));
}

glm::vec3 CalculateVec3Degrees(glm::vec3 eulerRadians) {
  return glm::vec3(glm::degrees(eulerRadians.x), glm::degrees(eulerRadians.y),
...
struct MeshEx {
  ...
  void Bind(VkCommandBuffer &cmd) {
    ...
    vkCmdBindVertexBuffers(cmd, 0, 1, &m_Mesh.m_VertexBuffer.m_GpuBuffer,
                           sizes);
    vkCmdBindIndexBuffer(cmd, m_Mesh.m_IndexBuffer.m_GpuBuffer, 0,
...
struct Model {
  ...
  Model(vku::IAllocator &alloc) : m_Meshes(alloc), m_Materials(alloc) {}
};
...
template <size_t _Size> struct FrameLightDataT {
...
void FillExampleLightData(FrameLightDataT<_Size> &lightData) {
  ...
  for (size_t i = 0; i < _Size; i++) {
    lightData.m_PointLights[i].PositionRadius = {
        centerPos.x + Random(areaSpread), centerPos.y + Random(areaSpread),
        centerPos.z + Random(areaSpread), Random(radiusSpread)};
    lightData.m_PointLights[i].Colour = {
        Random(colourSpread), Random(colourSpread), Random(colourSpread), 0.0f};

    lightData.m_SpotLights[i].PositionRadius = {
        centerPos.x + Random(areaSpread), centerPos.y + Random(areaSpread),
        centerPos.z + Random(areaSpread), Random(radiusSpread)};
    lightData.m_SpotLights[i].DirectionAngle = {
        Random(directionSpread), Random(directionSpread),
        -Random(directionSpread), Random(areaSpread)};
    lightData.m_SpotLights[i].Colour = {
        Random(colourSpread), Random(colourSpread), Random(colourSpread), 0.0f};
  }
}

static glm::vec2 AssimpToGLM(aiVector2D &aiVec) { return {aiVec.x, aiVec.y}; }
...
void FreeMesh(vku::VkState &vk, MeshEx &m) { m.m_Mesh.Free(vk); }
...
void ProcessMesh(vku::VkState &vk, Model &model, aiMesh *mesh, aiNode *node,
                 const aiScene *scene) {
  using namespace vku;
  ...
  if (hasPositions && hasUVs) {
    for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
      ...
      vert.UV = glm::vec2{mesh->mTextureCoords[0][i].x,
  ...
  if (hasIndices) {
    for (unsigned int i = 0; i < mesh->mNumFaces; i++) {
      ...
      if (currentFace.mNumIndices != 3) {
        VKU_LOG_ERR("Attempting to import a mesh with non triangular face "
                    "structure! cannot load this mesh.");
        return;
      }
      for (unsigned int index = 0; index < mesh->mFaces[i].mNumIndices;
           index++) {
        indices.push_back(
            static_cast<uint32_t>(mesh->mFaces[i].mIndices[index]));
      }
    }
  }
  ...
  MeshEx m{};
  m.m_Mesh.m_VertexBuffer =
      buffers::CreateVertexBuffer<VertexDataPosUv>(vk, verts);
...
AABB TransformAABB(AABB &in, glm::mat4 &m) {
  ...
  glm::mat3 rotationMatrix = {{m[0][0] * f, m[0][1] * f, m[0][2] * f},
...
void ProcessMeshWithNormals(vku::VkState &vk, Model &model, aiMesh *mesh,
  ...
  if (hasPositions && hasUVs && hasNormals) {
    for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
      ...
      vert.Position = AssimpToGLM(mesh->mVertices[i]);
      vert.UV = {mesh->mTextureCoords[0][i].x,
                 1.0f - mesh->mTextureCoords[0][i].y};
  ...
  MeshEx m{};
  m.m_Mesh.m_VertexBuffer =
      buffers::CreateVertexBuffer<VertexDataPosNormalUv>(vk, verts);
  m.m_Mesh.m_IndexBuffer = buffers::CreateIndexBuffer(vk, indices);
  m.m_IndexCount = static_cast<uint32_t>(indices.size());
...
void ProcessNode(vku::VkState &vk, Model &model, aiNode *node,
                 const aiScene *scene, bool withNormals = false) {
...
void LoadModelAssimp(vku::VkState &vk, Model &model, const char *path,
  ...
  //
  if (scene == nullptr) {
    VKU_LOG_ERR("AssimpModelAssetFactory : Failed to load asset at path : %s",
                path);
    return;
  }
  ProcessNode(vk, model, scene->mRootNode, scene, withNormals);

  // TODO: STL substr does not accept an allocator as param and so returned
  // string MUST be default allocated :( EASTL does not do this, uses allocator
  ...
  for (unsigned int i = 0; i < scene->mNumMaterials; i++) {
    ...
    uint32_t diffuseCount =
        aiGetMaterialTextureCount(meshMaterial, aiTextureType_DIFFUSE);
    if (diffuseCount == 0) {
      diffuseCount =
          aiGetMaterialTextureCount(meshMaterial, aiTextureType_BASE_COLOR);
    }

    if (diffuseCount == 0) {
      diffuseCount =
          aiGetMaterialTextureCount(meshMaterial, aiTextureType_UNKNOWN);
    }

    if (diffuseCount > 0) {
      aiString resultPath;
      aiGetMaterialTexture(meshMaterial, aiTextureType_DIFFUSE, 0, &resultPath);
      vku::String finalPath =
          directory + vku::String(resultPath.C_Str(), *vk.m_CPUAllocator);
      vku::Texture texture = vku::Texture::CreateTexture(
          vk, finalPath.c_str(), VK_FORMAT_R8G8B8A8_UNORM);
      model.m_Materials.push_back({texture});
    }
  }
}

MeshEx BuildScreenSpaceQuad(vku::VkState &vk,
                            vku::Vector<vku::VertexDataPosUv> &verts,
                            vku::Vector<uint32_t> &indices) {
  MeshEx m{};
  m.m_Mesh.m_VertexBuffer =
      vku::buffers::CreateVertexBuffer<vku::VertexDataPosUv>(vk, verts);
...
struct RenderModel {
  ...
  RenderModel(vku::IAllocator &alloc)
      : m_RenderItems(alloc), m_Original(alloc) {}
